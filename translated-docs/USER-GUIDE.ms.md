---
translation_last_updated: '2026-04-27T21:42:25.137Z'
...
- **Transformasi** - proses teks menggunakan arahan AI tersuai yang dikenali sebagai prompt.
...
> ℹ️ **NOTA**<br/>
> Transrewrt tersedia sebagai aplikasi desktop untuk Windows dan Linux, dan sebagai aplikasi web yang dihos sendiri. Panduan ini memberi tumpuan kepada penggunaan harian aplikasi. Di mana sesuatu hanya terpakai kepada satu versi, ia ditandakan dengan jelas.
...
- [Transformasi](#transform)
  - [Jalankan promp sedia ada](#run-an-existing-prompt)
  - [Jika anda belum mempunyai promp](#if-you-have-no-prompts-yet)
  - [Cipta promp dengan cepat](#create-a-prompt-quickly)
  - [Sunting promp](#edit-a-prompt)
  - [Uji promp sebelum menggunakannya](#test-a-prompt-before-using-it)
...
- [Tetapan](#settings)
  ...
  - [Promp transformasi](#transform-prompts)
  - [Pengguna](#users)
...
- [Isu lazim](#common-issues)
  ...
  - [Jumlah kos tidak sepadan dengan bil penyedia](#total-cost-does-not-match-my-provider-bill)
  ...
  - [Saya mengubah promp dan kehilangan suntingan](#i-changed-a-prompt-and-lost-the-edits)
...
Anda tidak perlu memilih model berbayar untuk bermula. Segera selepas anda menambah kunci API OpenRouter anda, aplikasi secara automatik mengaktifkan pilihan OpenRouter **percuma** terbina dalam. Ini membolehkan anda mula menterjemah, menulis semula, dan mengubah teks serta-merta. Sebagai alternatif, anda juga boleh mendapatkan kunci API percuma daripada Cerebras, Google, Groq, atau Mistral AI.

Dalam bahasa mudah:

- **Model** ialah enjin AI yang melakukan kerja. Model disenaraikan dengan **awalan penyedia** (contohnya `openrouter/…`, `openai/…`, `ollama/…`).
- **Kunci API** (atau, untuk Ollama, **URL asas**) adalah cara aplikasi mencapai penyedia tersebut.
...
Jika anda menggunakan **versi web**, pemilik pelayan mengkonfigurasi penyedia dengan pemboleh ubah persekitaran, jadi anda tidak boleh memasukkan kunci API secara langsung dalam aplikasi.
...
2. Cipta akaun atau daftar masuk.
...
Jika ini kali pertama anda menggunakan Transrewrt, ikuti susunan ini:
...
6. Pergi ke **Terjemah** dan jalankan terjemahan ringkas untuk mengesahkan segala-galanya berfungsi.
7. Setelah berjaya, cuba **Tulis semula** dan kemudian **Transformasi**.

Susunan ini penting. Ia mengelakkan masalah paling biasa ketika penggunaan pertama: cuba menjalankan tugas sebelum aplikasi mempunyai sambungan API yang berfungsi atau model yang dipilih.
  ...
  <tr>
    ...
    <td valign="top">
      ...
      <ul>
        ...
        <li><strong>Transformasi</strong> membuka ruang kerja prompt tersuai.</li><br/>
        ...
        <li><strong>Pengguna</strong> menunjukkan nama pengguna yang telah log masuk (versi web sahaja).</li>
      </ul>
    </td>
  </tr>
</table>
...
Sesetengah model percuma mungkin tidak sentiasa tersedia—kadangkala ia luar talian atau mempunyai had penggunaan. Jika ini berlaku, aplikasi akan secara automatik mengalih keluar model tersebut daripada senarai yang tersedia. Untuk mengawal model yang muncul, pergi ke [**Tetapan** > **Model**](#models) dan edit senarai model anda. 
...
Ikon **dunia + kod bahasa** mengubah bahasa antara muka aplikasi, seperti menu dan butang. Ia **tidak** mengubah bahasa terjemahan yang digunakan dalam **Terjemah**.
...
| **Input**                                                          | **Output**                                                                                                                  |
|--------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------|
| - Kiraan aksara <br/>- Kiraan perkataan <br/>- Kiraan perenggan   <br/> | - Berapa lama tugas mengambil masa<br/>- **TPS** (token per saat)<br/>- Kiraan aksara, perkataan, dan perenggan<br/>- Model yang digunakan |

Jika anda tertanya-tanya tentang istilah teknikal:

- **Token** bermaksud bahagian teks yang kecil. Anda boleh memikirkannya sebagai sebahagian daripada perkataan atau perkataan pendek.
...
- **Ke** adalah bahasa yang anda mahukan hasilnya.

**Bahasa Atas** yang anda pilih akan muncul di bahagian atas senarai. Anda boleh menetapkannya di [**Tetapan** > **Bahasa**](#languages).
...
- **Terjemah automatik semasa tampal** menjalankan terjemahan sebaik sahaja anda menampal teks.
- **Salin hasil ke papan keratan secara automatik** menyalin hasil secara automatik selepas pelaksanaan berjaya.
- **Terjemahan masa nyata (semasa menaip)** menjalankan terjemahan semasa anda menaip.
- **Had masa (ms)** mengawal berapa lama aplikasi menunggu sebelum menjalankan terjemahan masa nyata.
...
Gunakan **Tulis Semula** apabila anda ingin meningkatkan perkataan tanpa mengubah maksud utama.
...
- beberapa penyusunan semula berbeza dalam satu pelaksanaan (**Versi Alternatif**)
- menjadikan teks lebih formal atau tidak formal (**Formal** / **Tidak Formal**)
- memendekkan atau memanjangkan teks (**Pendekkan** / **Kembangkan**)
- menjadikan teks lebih teknikal (**Jadikan Teknikal**)

<br/>

> 💡 **TIP**<br/>
> Apabila anda menggunakan mod "**Semak Ejaan & Tatabahasa**", suis **Tunjukkan perubahan** akan muncul di panel output (bersebelahan **Salin**).
> Hidupkan atau matikan untuk memaparkan atau menyembunyikan pembetulan khusus yang dikenakan ke atas teks anda.
...
Gunakan **Transformasi** apabila anda mahu AI mengikuti satu set arahan tersuai.
...
- merumuskan nota
- mengubah teks kasar kepada emel yang sempurna
...
1. Buka **Transform**.
2. Pilih satu arahan dari senarai arahan.
3. Jika kotak **Sasaran** muncul, pilih bahasa jika anda mahukannya.
4. Taip atau tampal teks ke dalam **Input**.
5. Klik **Transform**.
6. Baca hasil dalam **Output**.

<br/>

<a id="if-you-have-no-prompts-yet"></a>
### Jika anda belum ada prompt

Jika senarai prompt anda kosong, klik **Muat prompt contoh** dalam ruang kerja Transformasi. Kawalan yang sama sentiasa tersedia di [**Tetapan** > **Gesaan transformasi**](#transform-prompts) pada baris eksport/import. Kedua-duanya menambah contoh binaan supaya anda boleh mula dengan cepat.

<br/>

> ℹ️ **NOTA**<br/>
> Prompt contoh disediakan dalam bahasa Inggeris. Selepas memuatnya, anda boleh mengedit prompt dan gunakan **Terjemah prompt** untuk menterjemahkannya ke dalam bahasa anda.
...
Cara terpantas untuk mencipta prompt ialah:
...
- **Arahan arahan (pilihan)**: petua ringkas yang dipaparkan kepada pengguna semasa melaksanakan arahan.
...
- **Penerangan hasil**: perkataan ringkas yang menerangkan hasil, seperti 'ringkasan' atau 'tulis semula'.
- **Suhu (0.0 → 1.0)**: bagaimana model akan berkelakuan; lihat di bawah.
- **Minta bahasa sasaran**: menambah pemilih bahasa sasaran apabila arahan dilaksanakan.

Jika istilah teknikal **Suhu** adalah baru bagi anda, fikirkan seperti ini:

- **Suhu** yang lebih rendah memberikan hasil yang lebih stabil dan boleh diramal.
...
- **`Improve prompt`** untuk membaik sempurna prompt sedia ada
- **`Translate prompt`** untuk menterjemah medan prompt
...
- anda sedang membina prompt baharu
- anda sedang membandingkan dua versi prompt
- anda ingin menyemak nada, panjang, atau format output

<br/>

> ℹ️ **NOTA**<br/>
> Anda boleh mengeksport dan mengimport prompt yang disimpan dalam [**Tetapan** > **Gesaan transformasi**](#transform-prompts).
...
> ℹ️ **NOTA**<br/>
> Penapis **Pengguna** hanya kelihatan kepada pentadbir dalam versi web. Pengguna biasa tidak akan melihat penapis ini, dan ia tidak tersedia dalam apl desktop.
...
- **Ringkasan** memberi gambaran keseluruhan penggunaan dan kos. Ia termasuk **Penggunaan dari masa ke masa** (**bilangan panggilan** kumulatif bertindih setiap hari untuk terjemah, tulis semula, dan transform) dan **Penggunaan mengikut model** (**jumlah panggilan per model**, termasuk transform).
- **Mengikut Penggunaan** membahagikan aktiviti mengikut bahasa terjemahan, mod tulis semula, dan arahan transform.
...
Ini berguna jika anda ingin mengkaji aktiviti di luar apl atau berkongsi laporan.
...
Di **Mengikut Model** atau **Semua Panggilan**, anda boleh membuang rekod tersimpan untuk model dengan mengklik ikon "tong sampah".

> ⚠️ **AMARAN**<br/>
> Pemadaman rekod tersimpan tidak boleh diterbalikkan. Gunakan hanya jika anda pasti tidak lagi memerlukan sejarah tersebut.

Untuk memadam semua data atau mengalih keluar rekod berdasarkan umur mereka, pergi ke [**Tetapan** > **Penjejakan Kos**](#cost-tracking). Di sana anda akan menjumpai pilihan untuk memadam semua data tersimpan atau hanya data yang lebih lama daripada tarikh tertentu.
...
Halaman sejarah boleh mengeksport data yang ditapis dalam:
...
Buka **Tetapan** daripada bar sisi untuk menyesuaikan cara apl berkelakuan.
...
> ℹ️ **NOTA**<br/>
> Dalam versi web, setiap pengguna mempunyai konfigurasinya sendiri. Tetapan seperti model dipilih, bahasa, pilihan umum, dan gesaan transformasi disimpan mengikut pengguna. Perubahan yang anda buat tidak akan menjejaskan pengguna lain.
...
Gunakan **Tetapan Umum** untuk mengawal tingkah laku menaip, sama ada butiran pelaksanaan disimpan untuk **Sejarah**, dan penampilan.

**Tingkah Laku**

- **Kelakuan untuk ENTER** menentukan sama ada `Enter` melaksanakan tugas atau memasukkan baris baharu.
...
- **Kekalkan sejarah pelaksanaan** mengawal sama ada setiap terjemah, tulis semula, dan transformasi menyimpan **teks input dan output** untuk paparan sisi [**Sejarah**](#history). Mematikannya meminta pengesahan; jika anda mengesahkan, teks sejarah yang disimpan akan dihapuskan dari pangkalan data.
- **Padam data sejarah** membolehkan anda menghapus teks yang disimpan mengikut usia (contohnya lebih tua daripada beberapa bulan, atau **semua data (kosongkan)**) menggunakan **Padam data**. Itu hanya mempengaruhi teks pelaksanaan yang disimpan untuk paparan **Sejarah**; ia tidak **menghapus** jumlah kos atau penggunaan. Untuk menghapus atau memotong data **kos**, gunakan [**Tetapan** > **Penjejakan Kos**](#cost-tracking).
...
- **Web sahaja:** **tunjukkan margin di sekeliling aplikasi** menambah ruang tambahan di sekeliling antara muka.
...
- **Sandarkan konfigurasi** - mencipta satu fail ZIP (`transrewrt-config-backup-YYYY-MM-DD_HHMMSS.zip` dalam UTC secara lalai) dengan `config.json`, `state.json`, kunci penyulitan pilihan, pengguna, keutamaan, arahan tersuai, dan data penggunaan jika anda memilih untuk menyertakannya. Selepas sandaran berjaya, pengesahan menunjukkan nama fail yang disimpan.
- **Pulih dari sandaran** - membuka **dialog pengesahan dahulu**. Pilih fail ZIP sandaran di dalam dialog tersebut (**Browse** / pemilih fail atau seret-dan-lepas jika disokong), kemudian semak semula pilihan:
  - **Pulihkan data penggunaan** - import penggunaan/sejarah dari ZIP apabila sandaran dibuat dengan data penggunaan disertakan; tinggalkan jika anda hanya mahu tetapan dan arahan.
  - **Kosongkan data penggunaan lama sebelum memulihkan** - buang penggunaan/sejarah sedia ada pada pemasangan ini sebelum memohon sandaran (pilihan; gunakan apabila anda mahu penggantian bersih).
...
Gunakan **Tetapan** > **Model** untuk memilih model yang muncul di bar alat.
...
Id model termasuk awalan penyedia (contohnya `openrouter/…` vs `openai/…`). Lencana seperti **OpenAI (OpenRouter)** vs **OpenAI (langsung)** menunjukkan bagaimana trafik dihalakan.

> ℹ️ **NOTA**<br/>
> **OpenRouter Body Builder** (`openrouter/bodybuilder`) adalah model penghala, bukan model sembang umum: balasannya adalah JSON yang menerangkan badan permintaan API OpenRouter (contohnya satu `requests` array dengan `model` dan `messages`). Jika anda menggunakannya untuk **Terjemah**, **Tulis Semula**, atau **Transformasi**, panel output akan menunjukkan JSON itu dan bukannya teks siap. Pilih model teks biasa untuk tugas-tugas tersebut. Lihat halaman [model Body Builder](https://openrouter.ai/openrouter/bodybuilder) di OpenRouter.

Tindakan:

- Untuk menambah model, klik **Tambah** atau mana-mana tempat dalam entri.

- Untuk menghapus model, klik **X** di sebelahnya dalam **Model Dipilih** atau **Dipilih** pada entri dalam Model Tersedia.
...
> ℹ️ **NOTA**<br/>
> Jika anda tidak mahu menambah kredit ke OpenRouter dengan segera, mulakan dengan mengaktifkan **Percuma Sahaja** dan memilih model-model percuma (tiada kad kredit diperlukan). Anda juga boleh menggunakan Ollama untuk menjalankan model secara tempatan tanpa sebarang kunci API.
...
- **Bahasa utama** ditetapkan berhampiran bahagian atas senarai bahasa dalam **Terjemah** dan **Transformasi**.
- **Bahasa pilihan** membolehkan anda menambah bahasa yang tidak terdapat dalam senarai terbina dalam.

Jika anda menambah bahasa pilihan, ia akan muncul dalam pemilih bahasa bersama pilihan terbina dalam.
...
Gunakan **Tetapan** > **Penjejakan Kos** untuk menguruskan maklumat kos.

- **Jumlah Kos** menunjukkan jumlah terkumpul.
- **Salin Nilai** menyalin jumlah ke papan keratan.
- **Tetapkan Semula Kos** menetapkan semula jumlah yang disimpan kepada sifar.
- **Sejajar dengan penggunaan kunci API** menetapkan jumlah supaya sepadan dengan penggunaan yang dilaporkan oleh akaun OpenRouter anda (OpenRouter sahaja).
...
> ⚠️ **AMARAN**<br/>
> Penghapusan data tidak boleh dibatalkan. Sebelum memadam, pastikan untuk menyandarkan data anda atau mengeksportnya melalui [**Sejarah**](#history) 
> atau [**Papan Pemuka** > **Semua Panggilan**](#dashboard-tabs), jika tidak, ia akan hilang secara kekal. 
...
Gunakan **Tetapan** > **Gesaan Transformasi** untuk menguruskan gesaan secara pukal.

Anda boleh:

- semak imbas arahan tersimpan anda
- padam arahan
- import arahan dari fail
- eksport arahan untuk sandaran atau perkongsian
- muat arahan sampel ke senarai arahan
...
Gunakan **Pengguna** untuk menguruskan akaun pengguna dalam versi web. Anda boleh menambah pengguna, mengemas kini butiran mereka, menetapkan semula kata laluan, dan memadam akaun.
...
Penyedia yang disokong adalah: OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras, dan **Ollama** (model tempatan melalui URL asas). Anda hanya perlu mengkonfigurasi penyedia yang anda gunakan.

**Aplikasi web: pentadbir sahaja**

Kunci API dikonfigurasikan melalui pemboleh ubah persekitaran sistem atau Docker - ia tidak dimasukkan dalam UI web. Halaman ini menunjukkan penyedia yang mempunyai kunci dikonfigurasikan dan membolehkan anda menguji setiap satu dengan mengklik butang **`Test`**.
...
Jika sesuatu tidak berfungsi seperti dijangka, semak perkara berikut dahulu.
...
2. Semak sekurang-kurangnya satu kunci API telah disimpan.
...
Cuba satu atau lebih daripada ini:
...
Klik ikon globe dalam [toolbar](#toolbar) dan pilih **Bahasa antara muka** yang anda suka.
...
- anda hanya menggunakan **model percuma** dan anda melihat angka **kos** (mungkin sifar); carta jumlah **penggunaan** pada **Ringkasan** masih memerlukan data dari tempoh yang dipilih
- **penapis masa** yang dipilih tidak merangkumi tempoh ketika panggilan dibuat - cuba **Semua** untuk memeriksa

Jika carta masih kosong selepas memilih **Semua**, sahkan bahawa panggilan muncul dalam [**Sejarah**](#history) atau dalam tab **Semua Panggilan**.

<br/>

<a id="cost-shows-not-available-or-seems-wrong"></a>
### Kos menunjukkan "tidak tersedia" atau nampak salah

Apabila anda menggunakan model melalui **OpenRouter**, aplikasi menunjukkan perbelanjaan sebenar anda yang dilaporkan oleh OpenRouter.

Untuk **penyedia lain** (OpenAI langsung, Anthropic langsung, dll.), kos dianggarkan daripada data harga yang diterbitkan oleh OpenRouter. Jika tiada harga yang sepadan ditemui untuk model, kos akan muncul sebagai **tidak tersedia** dan tidak akan ditambah kepada jumlah keseluruhan anda.
...
Untuk membuat jumlah ini lebih hampir dengan perbelanjaan OpenRouter sebenar anda, buka [**Tetapan** > **Penjejakan Kos**](#cost-tracking) dan klik **Segerak dengan penggunaan kunci API**.
...
**Kekalkan sejarah pelaksanaan** mungkin dimatikan. Buka [**Tetapan** > **Tetapan Umum**](#general-settings) dan dayakannya. Perhatikan bahawa menghidupkannya tidak memulihkan data sejarah yang telah dipadamkan sebelumnya.
...
Sesi anda mungkin telah tamat masa. Log masuk semula. Jika ini berlaku kerap, semak konfigurasi pelayan untuk tetapan tempoh sesi.
...
- Jika anda **terkunci keluar** tetapi mempunyai **akses shell** kepada mesin atau bekas, tetapkan semula kata laluan dengan alat bantu yang disertakan bersama imej (gantikan `transrewrt` jika anda menukar nama lalai, dan letakkan kata laluan dalam tanda petik jika ia mengandungi ruang atau aksara khas):

```bash
docker exec transrewrt reset-web-password '<username>' '<new-password>'
```

Nama pengguna pentadbir lalai ialah `admin` jika anda tidak pernah mencipta akaun lain. Apabila anda memberi hanya satu argumen, ia akan dianggap sebagai kata laluan baharu untuk `admin`.

Jika anda menjalankannya daripada **cek keluar sumber** dan bukannya Docker, gunakan:
...
- Mula dengan [**Terjemah**](#translate) untuk memastikan konfigurasi anda berfungsi sebelum anda teruskan ke [**Tulis Semula**](#rewrite) atau [**Transform**](#transform).
- Gunakan [**Tulis Semula**](#rewrite) untuk penambahbaikan perkataan harian.
- Gunakan [**Transform**](#transform) apabila anda memerlukan alur kerja berulang untuk tugas tertentu.
- Gunakan [**Papan Pemuka**](#dashboard) jika anda ingin memantau penggunaan dan kos.
- Gunakan [**Sejarah**](#history) untuk menyemak semula operasi lampau dan teks input/output penuh.
- Eksport arahan secara berkala jika anda membina perpustakaan arahan yang ingin anda simpan dengan selamat (rujuk [Transform Prompts](#transform-prompts)) atau jika anda ingin berkongsi dengannya.
