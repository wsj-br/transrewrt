---
translation_last_updated: '2026-05-17T23:35:08.894Z'
...
За замовчуванням додаток працює в режимі **Простий**: ви обираєте **навичку** (наприклад, Безкоштовно, Швидко або Технічна) та **постачальника** в Налаштуваннях, не вибираючи ідентифікатори моделей. Перейдіть до **Розширений** в [**Налаштування** > **Загальні налаштування**](#general-settings), якщо хочете класичний список моделей з [**Налаштування** > **Моделі**](#models).
...
- [Налаштування](#settings)
  ...
  - [Перетворити (вкладка налаштувань)](#transform-settings)
  - [Користувачі](#users)
...
- У режимі **Простий** **навичка** є попередньо налаштованою (Безкоштовно, Швидко, Розширений, Технічна, Юридична), яка відповідає моделі для обраного вами **постачальника** (OpenRouter, OpenAI, Ollama та інші). Ви вибираєте навичку на панелі інструментів у Перекладі, Переписуванні та Перетворенні.
- У режимі **Розширений** **модель** є AI-двигуном, який ви вибираєте безпосередньо. Ідентифікатори моделей використовують **префікс постачальника** (наприклад, `openrouter/…`, `openai/…`, `ollama/…`).
- **API-ключ** (або, для Ollama, **базовий URL**) — це спосіб, яким додаток підключається до цього постачальника.

Якщо ви використовуєте **десктопний додаток**, додайте ключі в [**Налаштування** > **Налаштування API**](#api-config) для кожного постачальника, якого ви використовуєте. Для використання тільки OpenRouter дивіться [Як отримати безкоштовний API-ключ OpenRouter](#how-to-get-an-api-key-desktop-app) нижче. Якщо ви не хочете використовувати API-ключ, ви можете встановити Ollama (з [ollama.com](https://ollama.com)) і використовувати локальні моделі замість цього, такі як `translategemma:4b`.
...
У режимі **Простий** панель інструментів показує **вибір навички** (Безкоштовно, Швидко, Розширений, Технічна, Юридична та подібні попередньо налаштовані). Навички залежать від **Постачальника**, якого ви вибрали в [**Налаштування** > **Загальні налаштування**](#general-settings). Якщо **Постачальник** — це **Ollama**, панель інструментів замість навичок перераховує ваші встановлені локальні моделі.
...
- **Простий** (за замовчуванням): виберіть **Постачальника** (OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras або Ollama). Хмарні постачальники використовують вбудовані пресети навичок на панелі інструментів. **Ollama** перераховує моделі, встановлені на вашій машині, замість навичок.
- **Розширений**: виберіть окремі моделі на панелі інструментів; керуйте списком у [**Налаштуваннях** > **Моделі**](#models).
