---
translation_last_updated: '2026-05-24T17:53:12.762Z'
...
За замовчуванням додаток працює в **Простому** режимі: ви вибираєте **навичку** (наприклад, Безкоштовно (OpenRouter), Lite або Technical) та **постачальника** в розділі Налаштування, не вказуючи ідентифікатори моделей. Перемкніться на **Розширений** режим у [**Налаштування** > **Загальні налаштування**](#general-settings), якщо хочете використовувати класичний список моделей із [**Налаштування** > **Моделі**](#models).
...
- [Налаштування](#settings)
  ...
  - [Перетворити (вкладка налаштувань)](#transform-settings-tab)
  - [Користувачі](#users)
...
- [Поширені проблеми](#common-issues)
  ...
  - [Підсумок панелі інструментів виглядає порожнім](#dashboard-summary-looks-empty)
...
- У **Простому** режимі **навичка** — це попередньо встановлений набір (Безкоштовно (OpenRouter), Lite, Advanced або Technical), який відповідає моделі обраного **постачальника** (OpenRouter, OpenAI, Ollama та інші). У панелі інструментів відображаються лише ті навички, які мають відповідне призначення для поточного постачальника. Ви обираєте навичку при виконанні дій Перекласти, Переписати та Перетворити.
- У **Розширеному** режимі **модель** — це движок ШІ, який ви обираєте безпосередньо. Ідентифікатори моделей використовують **префікс постачальника** (наприклад, `openrouter/…`, `openai/…`, `ollama/…`).
...
5. На **Перекладі** виберіть **навичку** (Простий) або **модель** (Розширений) на панелі інструментів.
6. Відкрийте [**Налаштування** > **Мови**](#languages) і виберіть свої **Основні мови**, якщо ви хочете, щоб ваші найвикористовуваніші мови з'являлися першими.
7. Виконайте простий переклад, щоб підтвердити, що все працює, а потім спробуйте **Переписати** та **Перетворити**.

Ця послідовність важлива. Вона запобігає найпоширенішій проблемі при першому використанні: спробі виконати задачу до того, як додаток має працююче API-з'єднання або вибрану навичку/модель.
...
- Зліва показується назва поточної сторінки.
- Справа показується **вибір навички або моделі** та елемент управління **Мовою інтерфейсу**.

У **Простому** режимі панель інструментів містить **вибір навички** з вбудованими попередніми наборами: **Безкоштовно (OpenRouter)**, **Lite**, **Advanced** та **Technical**. Які навички відображаються, залежить від **постачальника**, якого ви обрали в [**Налаштування** > **Загальні налаштування**](#general-settings). Наприклад, **Безкоштовно (OpenRouter)** відображається лише тоді, коли постачальник — OpenRouter. Якщо **постачальник** — **Ollama**, панель інструментів відображає ваші встановлені локальні моделі замість навичок.
...
2. Виберіть мову в **З**.
3. Виберіть мову в **До**.
4. Виберіть навичку (Простий) або модель (Розширений) на панелі інструментів.
...
- **Поведінка для ENTER** визначає, що відбувається при натисканні `Enter`:
  - **Enter** виконує переклад або переписування (за замовчуванням).
  - **Shift + Enter** виконує переклад або переписування; звичайний **Enter** вставляє новий рядок.
...
4. Виберіть навичку (Простий режим) або модель (Розширений режим).
...
Коли ви використовуєте **Створити запит**, **Покращити запит** або **Перекласти запит** в редакторі запитів, режим **Простий** пропонує той самий селектор навичок, що й Переклад та Переписування; режим **Розширений** використовує список моделей.
...
У режимі **Простий** вибір моделі відбувається через навички на панелі інструментів та **Постачальника** в Загальних налаштуваннях; вкладка **Моделі** прихована.

<br/>

> ℹ️ **ПРИМІТКА**<br/>
> У веб-версії кожен користувач має свою власну конфігурацію. Налаштування, такі як досвід роботи з ШІ, постачальник, обрані моделі або навички, мови, загальні параметри та перетворені підказки зберігаються для кожного користувача. Зміни, які ви вносите, не впливають на інших користувачів.
...
- **Простий** (за замовчуванням): виберіть **постачальника** (OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras або Ollama). Хмарні постачальники використовують вбудовані попередні набори навичок у панелі інструментів. **Ollama** відображає моделі, встановлені на вашому комп’ютері, замість навичок. У Простому режимі **Каталог навичок** показує версію каталогу та час останнього оновлення; натисніть **Оновити каталог навичок**, щоб отримати найновіший список навичок із репозиторію проекту (додаток також періодично перевіряє оновлення у фоновому режимі).
- **Розширений**: вибирайте окремі моделі в панелі інструментів; керуйте списком у розділі [**Налаштування** > **Моделі**](#models).

У **веб-додатку** те, які постачальники з'являються, залежить від API-ключів, встановлених у середовищі сервера. У **десктопному додатку** налаштуйте ключі в [**Налаштуваннях API**](#api-config).
...
**Резервне копіювання конфігурації** (лише для настільного додатку та веб-адміністраторів)
...
- ви вибрали **навичку** (Простий) або **модель** (Розширений) на панелі інструментів
- в режимі **Простий**, [**Налаштування** > **Загальні налаштування**](#general-settings) має **Постачальника** з робочим ключем (або URL Ollama) та принаймні одну навичку для цього постачальника
- в режимі **Розширений**, принаймні одна модель перерахована в [**Налаштування** > **Моделі**](#models)
- ваша налаштування API працює
...
- вибрати іншу навичку (Простий режим) або модель (Розширений режим)
- використовувати коротший вхідний текст
- вимкнути **Переклад у реальному часі (під час введення)** в [**Налаштування** > **Загальні налаштування**](#general-settings)
- використовувати безкоштовні моделі для простих завдань (див. [Моделі](#models))
