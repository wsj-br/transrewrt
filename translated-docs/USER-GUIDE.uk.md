...
- [Переклад](#translate)
  ...
  - [Удосконалення перекладу](#refining-your-translation)
  - [Використання глосарію](#using-the-glossary)
- [Перезапис](#rewrite)
  - [Перезапис тексту](#rewrite-text)
  - [Уточнення перезапису](#refining-your-rewrite)
- [Трансформація](#transform)
  - [Запуск існуючої підказки](#run-an-existing-prompt)
  - [Якщо у вас ще немає підказок](#if-you-have-no-prompts-yet)
  - [Швидке створення підказки](#create-a-prompt-quickly)
  - [Редагування підказки](#edit-a-prompt)
  - [Тестування підказки перед використанням](#test-a-prompt-before-using-it)
...
- [Історія](#history)
  - [Фільтрування історії](#filter-the-history)
...
- [Налаштування](#settings)
  ...
  - [Глосарій (вкладка налаштувань)](#glossary-settings-tab)
  - [Користувачі](#users)
  - [Налаштування API](#api-config)
  - [Про](#about)
- [Поширені проблеми](#common-issues)
  - [Додаток не перекладає, не переписує та не трансформує текст](#the-app-will-not-translate-rewrite-or-transform-text)
  ...
  - [Інтерфейс відображається неправильною мовою](#the-interface-is-in-the-wrong-language)
  - [Текст занадто малий або важко читається](#the-text-is-too-small-or-hard-to-read)
  - [Підсумок панелі керування виглядає порожнім](#dashboard-summary-looks-empty)
  - [Вартість відображається як «недоступно» або здається неправильною](#cost-shows-not-available-or-seems-wrong)
  ...
  - [Я змінив підказку та втратив зміни](#i-changed-a-prompt-and-lost-the-edits)
...
![Селектор моделі](../images/screenshots/uk/preset-selector.png)
...
![Селектор мови інтерфейсу](../images/screenshots/uk/language-selector.png)
...
1. **Перефразувати…** — якщо в результаті не вибрано текст, отримайте ще один повний переклад того ж вхідного тексту з іншим формулюванням. Модель отримує кожну версію, яку ви вже маєте, тому нове формулювання може відрізнятися від усіх попередніх. Ви можете зберігати до **п’яти** версій і перемикатися між ними у спадному меню версій. Якщо вибрано текст, **Перефразувати…** відкриває варіанти слів біля виділення (так само, як клацання правою кнопкою миші). Без виділення **Перефразувати…** вимикається, коли ви досягаєте п’яти версій; з виділенням воно все ще працює на п’яти версіях (лише варіанти слів, оновлення версії 5). Поки виконується повне перефразування, натисніть **Зупинити переклад**, щоб скасувати; результат повернеться до версії, яка була активною на початку перефразування.
2. **Варіанти слів** — виберіть одне або кілька слів чи коротку фразу в результаті (якщо ви виберете лише частину слова, додаток розширить виділення до повних слів), потім клацніть правою кнопкою миші або натисніть **Перефразувати…**. Короткий список варіантів з’явиться біля виділення; клацніть один, щоб замінити його. Кожен варіант може замінити трохи ширший діапазон, ніж ваше виділення (наприклад, сусідній прийменник або артикль), щоб речення залишалося граматично правильним. Якщо у вас менше п’яти версій, відредагований результат зберігається як нова версія; на п’яти версіях оновлюється лише **версія 5**. Клацання правою кнопкою миші без виділення виділяє слово під курсором (або нічого не робить, якщо там немає слова). Натисніть **Esc** або клацніть за межами списку, щоб скасувати без зміни результату.
3. **Витрати** — кожне повне **Перефразування…** (без виділення) та кожен запит на варіант слова знову використовує модель і може збільшити вартість використання (так само, як звичайний запуск перекладу).
...
**Глосарій** — це список пар термінів мовою оригіналу та цільовою мовою для конкретної мовної пари. Коли глосарій увімкнено, Transrewrt надсилає відповідні терміни до моделі, щоб ваші бажані формулювання залишалися послідовними в перекладах (наприклад, назва продукту, термін бренду або посада, яку завжди слід перекладати однаково).

Щоб використовувати його на сторінці **Переклад**:

1. Увімкніть перемикач **Глосарій** на панелі введення (поряд із перемикачами автовиконання та автокопіювання).
2. Виберіть мови **Джерело** та **Ціль** і перекладайте як зазвичай. Збережені терміни для цієї мовної пари застосовуються автоматично.
3. Щоб додати нову пару на льоту, натисніть **Додати до глосарію** (поряд із селектором мови **Джерело:**). Діалогове вікно буде попередньо заповнено вашими поточними мовами, тому вам залишиться лише заповнити **Термін мовою оригіналу** та **Цільовий термін**.
4. Використовуйте посилання **Глосарій** у нижньому колонтитулі виводу (або посилання **Керування глосарієм** у діалоговому вікні), щоб перейти до [**Налаштування** > **Глосарій**](#glossary-settings) та переглянути всі свої терміни.

Ви додаєте, редагуєте, імпортуєте та експортуєте терміни на вкладці [**Налаштування** > **Глосарій**](#glossary-settings) — див. нижче.

<br/>

> ℹ️ **ПРИМІТКА**<br/>
> Терміни глосарію зіставляються за **мовною парою**, тому термін, збережений для англійської → французької, не застосовується під час перекладу з англійської → німецької. Глосарій не можна використовувати з опцією **Визначити мову** як джерелом, оскільки для зіставлення термінів потрібна конкретна мова оригіналу.
...
Використовуйте **Перезапис**, коли ви хочете покращити формулювання, не змінюючи основного значення. Текст залишається тією ж мовою (він не перекладається).

![Робоча область перезапису](../images/screenshots/uk/rewrite.png)
...
1. Відкрийте **Перезапис**.
2. Виберіть **Режим** (наприклад, **Покращити ясність** або **Зробити формальним**).
3. За бажанням встановіть **Від** на мову вашого тексту (або залиште **Визначити мову**).
4. Введіть або вставте текст у **Введення**.
5. Натисніть **Перезапис**.
6. Прочитайте результат у **Вивід**.
7. За бажанням уточніть результат за допомогою **Перефразувати…** або варіантів слів — див. [Уточнення перезапису](#refining-rewrite).
...
> ℹ️ **ПРИМІТКА**<br/>
> Режим перезапису **Альтернативні версії** повертає кілька переформулювань за **один** запуск, розділених `----` у виводі. Це відрізняється від **Перефразувати…**, який створює історію версій з часом (один новий варіант за клік). Див. [Уточнення перезапису](#refining-rewrite).

<br/>

<a id="refining-rewrite"></a>
### Уточнення перезапису

Після успішного перезапису, **Перефразувати…** та спадне меню версій з'являються на стороні виводу робочої області (у розділеному макеті, на верхній панелі інструментів над стовпцем виводу, поруч із метриками запуску; у стековому макеті, над панеллю виводу поруч із **Від:**). Ви можете уточнити результат там — та сама ідея, що й [Уточнення перекладу](#refining-translation), але текст залишається тією ж мовою та зберігає поточний **Режим** перезапису:

1. **Перефразувати…** — якщо в виводі не вибрано текст, отримайте ще один повний перезапис того ж вводу з іншим формулюванням, все ще застосовуючи вибраний режим (наприклад, чіткіший, коротший або більш формальний). Модель отримує кожну версію, яку ви вже маєте, тому нове формулювання може відрізнятися від усіх них. Ви можете зберігати до **п'яти** версій і перемикатися між ними у спадному меню версій. Якщо вибрано текст, **Перефразувати…** відкриває альтернативи слів біля виділення (так само, як клацання правою кнопкою миші). Без виділення **Перефразувати…** вимикається, коли ви досягаєте п'яти версій; з виділенням він все ще працює на п'яти версіях (лише альтернативи слів, оновлюючи версію 5). Під час повного перефразування натисніть **Зупинити перезапис**, щоб скасувати; вивід повертається до версії, яка була активною на початку перефразування.
2. **Варіанти слів** — виберіть одне або кілька слів або коротку фразу у виводі (якщо ви виберете лише частину слова, програма розширить виділення до повних слів), потім клацніть правою кнопкою миші або натисніть **Перефразувати…**. Короткий список альтернатив з'явиться біля виділення; клацніть одну, щоб замінити її. Кожен варіант може замінити трохи ширший діапазон, ніж ваше виділення, щоб речення залишалося граматично правильним. Якщо у вас менше п'яти версій, відредагований вивід зберігається як нова версія; на п'яти версіях оновлюється лише **версія 5**. Клацання правою кнопкою миші без виділення вибирає слово під курсором (або нічого не робить, якщо там немає слова). Натисніть **Esc** або клацніть за межами списку, щоб скасувати без зміни виводу.
3. **Вартість** — кожен повний **Перефразувати…** (без виділення) та кожен запит на альтернативи слів знову використовує модель і може збільшити вартість використання (так само, як звичайний запуск перезапису).
...
![Редактор запитів трансформації](../images/screenshots/uk/transform-prompt-edit.png)
...
![Підсумок панелі керування](../images/screenshots/uk/dashboard-summary.png)
...
| Вкладка           | Десктоп | Веб (адміністратор) | Веб (звичайний користувач) | Примітки                                        |
  ...
  | Глосарій         |   так   |     так     |        так         | Пари термінів, застосовані під час перекладу        |
  | Користувачі      |    -    |     так     |         -          |                                              |
...
![Вкладка «Моделі» в налаштуваннях](../images/screenshots/uk/settings-general.png)
...
### Глосарій (вкладка налаштувань)

Використовуйте **Налаштування** > **Глосарій**, щоб керувати парами термінів, які застосовуються під час перекладу (див. [Використання глосарію](#using-the-glossary)). Кожен термін має **мову оригіналу**, **цільову мову**, **термін мовою оригіналу** та **цільовий термін**.

Ви можете:

- **Додати термін** — заповніть рядок унизу таблиці (виберіть мови, введіть термін мовою оригіналу та цільовий термін) і натисніть кнопку **+**.
- **Знайти терміни** — фільтруйте список за **мовою оригіналу**, **цільовою мовою** або за вільним **текстом**; натисніть **Очистити фільтри**, щоб скинути.
- **Видалити термін** — натисніть піктограму кошика в його рядку.
- **Імпорт** — завантажте терміни з файлу `.csv`, `.xlsx` або `.xls`. Файл повинен мати стовпці `source_language`, `target_language`, `source_text` та `target_text`.
- **Експорт CSV** / **Експорт XLSX** — завантажте всі свої терміни для резервного копіювання або обміну.
- **Шаблон CSV** / **Шаблон XLSX** — завантажте порожній файл із правильними заголовками стовпців для заповнення та імпорту.

<br/>

> ℹ️ **ПРИМІТКА**<br/>
> У **настільній програмі** глосарій зберігається локально. У **веб-версії** кожен користувач має власний глосарій, тому ваші терміни не впливають на інших користувачів.
