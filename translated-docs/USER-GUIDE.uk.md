...
Після успішного перекладу ви можете уточнити результат у панелі виводу:

1. **Перефразувати…** — без вибраного тексту у виводі отримайте ще один повний переклад того ж введення з іншим формулюванням. Ви можете зберігати до **п'яти** версій і перемикатися між ними у випадаючому списку версій. При вибраному тексті **Перефразувати…** відкриває альтернативи слів поблизу вибору (так само, як і правий клік). Без вибору **Перефразувати…** вимкнено, коли ви досягаєте п'яти версій; з вибором воно все ще працює на п'яти версіях (тільки альтернативи слів, оновлюючи версію 5).
2. **Альтернативи слів** — виберіть одне або кілька слів у виводі (якщо ви виберете лише частину слова, програма розширить вибір до повних слів), потім клацніть правою кнопкою миші або натисніть **Перефразувати…**. Невеликий список альтернатив з'являється поблизу вибору; натисніть одну, щоб замінити її. Якщо у вас менше ніж п'ять версій, відредагований вивід зберігається як нова версія; при п'яти версіях оновлюється лише **версія 5**. Клацання правою кнопкою без вибору нічого не робить. Натисніть **Esc** або клацніть поза списком, щоб скасувати без зміни виводу.
3. **Вартість** — кожне повне **Перефразувати…** (без вибору) та кожен запит альтернативи слів використовує модель знову і може додати до вартості використання (так само, як звичайний переклад).
