---
translation_last_updated: '2026-05-27T10:08:32.438Z'
...
За замовчуванням додаток працює в **Простому** режимі: ви вибираєте **попереднє налаштування** (наприклад, Безкоштовно (OpenRouter), Lite або Technical) та **постачальника** в розділі Налаштування, не вказуючи ідентифікатори моделей. Перемкніться на **Розширений** режим у [**Налаштування** > **Загальні налаштування**](#general-settings), якщо хочете отримати класичний список моделей у [**Налаштування** > **Моделі**](#models).
...
- У **Простому** режимі **попереднє налаштування** — це заздалегідь визначене налаштування (Безкоштовно (OpenRouter), Lite, Розширений або Technical), яке відповідає моделі для обраного **постачальника** (OpenRouter, OpenAI, Ollama та інші). У панелі інструментів відображаються лише ті навички, які мають відповідне призначення для поточного постачальника. Ви вибираєте навичку в Перекласти, Переписати та Перетворити.
...
У **Простому** режимі панель інструментів показує **вибір попередніх налаштувань** із вбудованими варіантами: **Безкоштовно (OpenRouter)**, **Lite**, **Розширений** та **Technical**. Які саме попередні налаштування відображаються, залежить від **постачальника**, якого ви обрали в [**Налаштування** > **Загальні налаштування**](#general-settings). Наприклад, **Безкоштовно (OpenRouter)** відображається лише тоді, коли постачальник — OpenRouter. Якщо **постачальник** — **Ollama**, на панелі інструментів замість попередніх налаштувань відображаються ваші встановлені локальні моделі.
...
- **Простий** (за замовчуванням): виберіть **Постачальника** (OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras або Ollama). Хмарні постачальники використовують вбудовані попередні налаштування на панелі інструментів. **Ollama** замість попередніх налаштувань відображає моделі, встановлені на вашому комп'ютері. У Простому режимі **Каталог попередніх налаштувань** показує версію каталогу та час останнього оновлення; натисніть **Оновити каталог попередніх налаштувань**, щоб отримати найновіший список навичок із репозиторію проекту (додаток також періодично перевіряє оновлення у фоновому режимі).
...
- виберіть інше попереднє налаштування (Простий) або модель (Розширений)
- скоротіть вхідні дані
