...
- [แปล](#translate)
  ...
  - [การปรับแต่งการแปลของคุณ](#refining-your-translation)
  - [การใช้คำศัพท์](#using-the-glossary)
- [การแก้ไขใหม่](#rewrite)
  - [แก้ไขข้อความใหม่](#rewrite-text)
  - [ปรับปรุงการแก้ไขใหม่ของคุณ](#refining-your-rewrite)
- [การแปลง](#transform)
  - [เรียกใช้พร้อมต์ที่มีอยู่](#run-an-existing-prompt)
  - [หากคุณยังไม่มีพร้อมต์](#if-you-have-no-prompts-yet)
...
- [แดชบอร์ด](#dashboard)
  ...
  - [ลบระเบียนที่จัดเก็บไว้สำหรับโมเดล](#delete-stored-records-for-a-model)
- [ประวัติ](#history)
  - [กรองประวัติ](#filter-the-history)
...
- [การตั้งค่า](#settings)
  ...
  - [อภิธานศัพท์ (แท็บการตั้งค่า)](#glossary-settings-tab)
  - [ผู้ใช้](#users)
  - [การกำหนดค่า API](#api-config)
  - [เกี่ยวกับ](#about)
- [ปัญหาทั่วไป](#common-issues)
  - [แอปไม่แปล แก้ไขใหม่ หรือแปลงข้อความ](#the-app-will-not-translate-rewrite-or-transform-text)
  - [รายการโมเดลว่างเปล่า](#the-model-list-is-empty)
  - [ผลลัพธ์ช้าเกินไปหรือแพงเกินไป](#the-result-is-too-slow-or-too-expensive)
  - [อินเทอร์เฟซเป็นภาษาที่ไม่ถูกต้อง](#the-interface-is-in-the-wrong-language)
  - [ข้อความเล็กเกินไปหรืออ่านยาก](#the-text-is-too-small-or-hard-to-read)
  - [สรุปแดชบอร์ดดูว่างเปล่า](#dashboard-summary-looks-empty)
  - [ค่าใช้จ่ายแสดงว่า "ไม่พร้อมใช้งาน" หรือดูเหมือนผิดปกติ](#cost-shows-not-available-or-seems-wrong)
  - [ค่าใช้จ่ายรวมไม่ตรงกับบิลผู้ให้บริการของฉัน](#total-cost-does-not-match-my-provider-bill)
  ...
  - [แดชบอร์ดไม่แสดงข้อมูลสำหรับผู้ใช้รายอื่น (เว็บ)](#dashboard-shows-no-data-for-other-users-web)
  - [ฉันเปลี่ยนพร้อมต์และทำส่วนที่แก้ไขหายไป](#i-changed-a-prompt-and-lost-the-edits)
...
![ตัวเลือกโมเดล](../images/screenshots/th/preset-selector.png)
...
1. **แก้ไขใหม่…** — เมื่อไม่มีข้อความที่เลือกในเอาต์พุต ให้รับการแปลฉบับเต็มอีกครั้งของอินพุตเดียวกันด้วยถ้อยคำที่แตกต่างกัน โมเดลจะได้รับทุกเวอร์ชันที่คุณมีอยู่แล้ว ดังนั้นถ้อยคำใหม่จึงสามารถแตกต่างจากทั้งหมดได้ คุณสามารถจัดเก็บได้สูงสุด **ห้า** เวอร์ชันและสลับไปมาระหว่างเวอร์ชันเหล่านั้นในเมนูแบบเลื่อนลงของเวอร์ชัน เมื่อเลือกข้อความแล้ว **แก้ไขใหม่…** จะเปิดคำทางเลือกใกล้กับการเลือก (เช่นเดียวกับการคลิกขวา) หากไม่มีการเลือก **แก้ไขใหม่…** จะถูกปิดใช้งานเมื่อคุณถึงห้าเวอร์ชัน; หากมีการเลือก มันยังคงทำงานที่ห้าเวอร์ชัน (เฉพาะคำทางเลือก อัปเดตเวอร์ชัน 5) ในขณะที่การแก้ไขใหม่ฉบับเต็มกำลังทำงานอยู่ ให้คลิก **หยุดการแปล** เพื่อยกเลิก; เอาต์พุตจะกลับไปเป็นเวอร์ชันที่ใช้งานอยู่เมื่อการแก้ไขใหม่เริ่มต้นขึ้น
2. **คำทางเลือก** — เลือกหนึ่งคำขึ้นไปหรือวลีสั้นๆ ในเอาต์พุต (หากคุณเลือกเพียงบางส่วนของคำ แอปจะขยายการเลือกเป็นคำเต็ม) จากนั้นคลิกขวาหรือคลิก **แก้ไขใหม่…** รายการทางเลือกสั้นๆ จะปรากฏขึ้นใกล้กับการเลือก; คลิกหนึ่งรายการเพื่อแทนที่ แต่ละตัวเลือกอาจแทนที่ช่วงที่กว้างกว่าการเลือกของคุณเล็กน้อย (เช่น คำบุพบทหรือคำนำหน้าคำนามที่อยู่ติดกัน) เพื่อให้ประโยคยังคงถูกต้องตามหลักไวยากรณ์ หากคุณมีเวอร์ชันน้อยกว่าห้าเวอร์ชัน เอาต์พุตที่แก้ไขจะถูกบันทึกเป็นเวอร์ชันใหม่; ที่ห้าเวอร์ชัน เฉพาะ **เวอร์ชัน 5** เท่านั้นที่จะได้รับการอัปเดต คลิกขวาโดยไม่มีการเลือกจะเลือกคำที่อยู่ใต้เคอร์เซอร์ (หรือไม่มีอะไรเกิดขึ้นหากไม่มีคำอยู่ตรงนั้น) กด **Esc** หรือคลิกนอกรายการเพื่อยกเลิกโดยไม่เปลี่ยนแปลงเอาต์พุต
3. **ค่าใช้จ่าย** — การ **แก้ไขใหม่…** ฉบับเต็มแต่ละครั้ง (ไม่มีการเลือก) และคำขอคำทางเลือกแต่ละครั้งจะใช้โมเดลอีกครั้งและอาจเพิ่มค่าใช้จ่ายในการใช้งาน (เช่นเดียวกับการแปลปกติ)
...
**คำศัพท์** คือรายการคู่คำศัพท์ต้นทาง/ปลายทางสำหรับคู่ภาษาที่ระบุ เมื่อเปิดใช้งานคำศัพท์ Transrewrt จะส่งคำที่ตรงกันไปยังโมเดลเพื่อให้คำที่คุณต้องการมีความสอดคล้องกันในการแปล (เช่น ชื่อผลิตภัณฑ์ คำศัพท์แบรนด์ หรือตำแหน่งงานที่ควรแปลเหมือนกันเสมอ)

วิธีใช้ในหน้า **แปล**:

1. เปิดสวิตช์ **คำศัพท์** ในแผงข้อมูลนำเข้า (ถัดจากสวิตช์ทำงานอัตโนมัติและคัดลอกอัตโนมัติ)
2. เลือกภาษา **จาก** และ **ถึง** ของคุณ แล้วแปลตามปกติ คำศัพท์ที่บันทึกไว้สำหรับคู่ภาษานั้นจะถูกนำไปใช้โดยอัตโนมัติ
3. หากต้องการบันทึกคู่คำศัพท์ใหม่ทันที คลิก **เพิ่มลงในอภิธานศัพท์** (ถัดจากตัวเลือกภาษา **จาก:**) กล่องโต้ตอบจะเติมข้อมูลล่วงหน้าด้วยภาษาปัจจุบันของคุณ ดังนั้นคุณเพียงแค่กรอก **คำศัพท์ต้นทาง** และ **คำศัพท์ปลายทาง**
4. ใช้ลิงก์ **คำศัพท์** ในส่วนท้ายของข้อมูลส่งออก (หรือลิงก์ **จัดการอภิธานศัพท์** ภายในกล่องโต้ตอบ) เพื่อไปยัง [**การตั้งค่า** > **คำศัพท์**](#glossary-settings) และตรวจสอบคำศัพท์ทั้งหมดของคุณ

คุณสามารถเพิ่ม แก้ไข นำเข้า และส่งออกคำศัพท์ได้ในแท็บ [**การตั้งค่า** > **คำศัพท์**](#glossary-settings) — ดูด้านล่าง

<br/>

> ℹ️ **หมายเหตุ**<br/>
> คำศัพท์จะถูกจับคู่ตาม **คู่ภาษา** ดังนั้นคำศัพท์ที่บันทึกไว้สำหรับภาษาอังกฤษ → ฝรั่งเศส จะไม่ถูกนำไปใช้เมื่อแปลภาษาอังกฤษ → เยอรมัน คำศัพท์ไม่สามารถใช้กับ **ตรวจสอบภาษา** เป็นภาษาต้นทางได้ เนื่องจากจำเป็นต้องมีภาษาต้นทางที่เฉพาะเจาะจงเพื่อจับคู่คำศัพท์
...
ใช้ **การแก้ไขใหม่** เมื่อคุณต้องการปรับปรุงถ้อยคำโดยไม่เปลี่ยนความหมายหลัก ข้อความยังคงอยู่ในภาษาเดียวกัน (ไม่ได้แปล)

![พื้นที่ทำงานการแก้ไขใหม่](../images/screenshots/th/rewrite.png)
...
1. เปิด **Rewrite**
2. เลือก **Mode** (เช่น **Improve Clarity** หรือ **Make Formal**)
3. เลือกภาษาของข้อความของคุณใน **From** (หรือปล่อยให้เป็น **Detect Language**) (ไม่บังคับ)
4. พิมพ์หรือวางข้อความลงใน **Input**
5. คลิก **Rewrite**
6. อ่านผลลัพธ์ใน **Output**
7. ปรับแต่งผลลัพธ์ด้วย **Rephrase…** หรือคำอื่นๆ (ไม่บังคับ) — ดู [การปรับแต่งการแก้ไขใหม่ของคุณ](#refining-rewrite)
...
> ℹ️ **หมายเหตุ**<br/>
> โหมดการแก้ไขใหม่ **Alternative versions** จะส่งคืนการปรับปรุงหลายรูปแบบในการเรียกใช้ **ครั้งเดียว** โดยคั่นด้วย `----` ในเอาต์พุต ซึ่งแตกต่างจาก **Rephrase…** ที่สร้างประวัติเวอร์ชันเมื่อเวลาผ่านไป (หนึ่งตัวแปรใหม่ต่อการคลิก) ดู [การปรับแต่งการแก้ไขใหม่ของคุณ](#refining-rewrite)

<br/>

<a id="refining-rewrite"></a>
### การปรับแต่งการแก้ไขใหม่ของคุณ

หลังจากแก้ไขใหม่สำเร็จ **Rephrase…** และเมนูดรอปดาวน์เวอร์ชันจะปรากฏที่ด้านเอาต์พุตของพื้นที่ทำงาน (ในเค้าโครงแบบแยก ในแถบเครื่องมือด้านบนเหนือคอลัมน์เอาต์พุต ถัดจากเมตริกการเรียกใช้; ในเค้าโครงแบบซ้อนกัน เหนือแผงเอาต์พุตถัดจาก **From:**) คุณสามารถปรับแต่งผลลัพธ์ได้ที่นั่น — แนวคิดเดียวกันกับ [การปรับแต่งการแปลของคุณ](#refining-translation) แต่ข้อความยังคงเป็นภาษาเดียวกันและคง **Mode** การแก้ไขใหม่ปัจจุบันไว้:

1. **Rephrase…** — เมื่อไม่มีข้อความที่เลือกในเอาต์พุต ให้แก้ไขใหม่ทั้งหมดของอินพุตเดียวกันด้วยถ้อยคำที่แตกต่างกัน โดยยังคงใช้โหมดที่เลือก (เช่น ชัดเจนขึ้น สั้นลง หรือเป็นทางการมากขึ้น) โมเดลจะได้รับทุกเวอร์ชันที่คุณมีอยู่แล้ว ดังนั้นถ้อยคำใหม่จึงอาจแตกต่างจากทั้งหมด คุณสามารถจัดเก็บได้สูงสุด **ห้า** เวอร์ชันและสลับไปมาระหว่างเวอร์ชันเหล่านั้นในเมนูดรอปดาวน์เวอร์ชัน เมื่อเลือกข้อความแล้ว **Rephrase…** จะเปิดคำอื่นๆ ใกล้กับส่วนที่เลือก (เช่นเดียวกับการคลิกขวา) หากไม่มีการเลือก **Rephrase…** จะถูกปิดใช้งานเมื่อคุณถึงห้าเวอร์ชัน; หากมีการเลือก จะยังคงทำงานที่ห้าเวอร์ชัน (เฉพาะคำอื่นๆ โดยอัปเดตเวอร์ชัน 5) ในขณะที่การแก้ไขใหม่ทั้งหมดกำลังทำงาน ให้คลิก **Stop Rewrite** เพื่อยกเลิก; เอาต์พุตจะกลับไปเป็นเวอร์ชันที่ใช้งานอยู่เมื่อการแก้ไขใหม่เริ่มต้นขึ้น
2. **Word alternatives** — เลือกหนึ่งคำหรือมากกว่า หรือวลีสั้นๆ ในเอาต์พุต (หากคุณเลือกเพียงส่วนหนึ่งของคำ แอปจะขยายส่วนที่เลือกให้เป็นคำเต็ม) จากนั้นคลิกขวาหรือคลิก **Rephrase…** รายการคำอื่นๆ สั้นๆ จะปรากฏขึ้นใกล้กับส่วนที่เลือก; คลิกหนึ่งคำเพื่อแทนที่ ตัวเลือกแต่ละตัวเลือกอาจแทนที่ช่วงที่กว้างกว่าส่วนที่คุณเลือกเล็กน้อยเพื่อให้ประโยคยังคงเป็นไปตามหลักไวยากรณ์ หากคุณมีน้อยกว่าห้าเวอร์ชัน เอาต์พุตที่แก้ไขจะถูกบันทึกเป็นเวอร์ชันใหม่; ที่ห้าเวอร์ชัน เฉพาะ **เวอร์ชัน 5** เท่านั้นที่จะได้รับการอัปเดต คลิกขวาโดยไม่มีการเลือกจะเลือกคำที่อยู่ใต้เคอร์เซอร์ (หรือไม่มีอะไรเกิดขึ้นหากไม่มีคำอยู่ตรงนั้น) กด **Esc** หรือคลิกนอกรายการเพื่อยกเลิกโดยไม่เปลี่ยนเอาต์พุต
3. **Costs** — การ **Rephrase…** เต็มรูปแบบแต่ละครั้ง (ไม่มีการเลือก) และคำขอคำอื่นๆ แต่ละครั้งจะใช้โมเดลอีกครั้งและอาจเพิ่มค่าใช้จ่ายในการใช้งาน (เช่นเดียวกับการเรียกใช้การแก้ไขใหม่ปกติ)
...
![สร้างคำแนะนำ](../images/screenshots/th/transform-generate.png)
...
![ตัวแก้ไขคำแนะนำการแปลง](../images/screenshots/th/transform-prompt-edit.png)
...
| แปลง | ใช่ | ใช่ | ใช่ | นำเข้า/ส่งออกพรอมต์แปลงแบบกลุ่ม |
  | คำศัพท์         |   ใช่   |     ใช่     |        ใช่         | คู่คำศัพท์ที่ใช้ระหว่างการแปล        |
| ผู้ใช้ | - | ใช่ | - | |
...
### คำศัพท์ (แท็บการตั้งค่า)

ใช้ **การตั้งค่า** > **อภิธานศัพท์** เพื่อจัดการคู่คำศัพท์ที่ใช้ในการแปล (ดู [การใช้อภิธานศัพท์](#using-the-glossary)) แต่ละคำศัพท์มี **ภาษาต้นทาง**, **ภาษาปลายทาง**, **คำศัพท์ต้นทาง** และ **คำศัพท์ปลายทาง**

คุณสามารถ:

- **เพิ่มคำศัพท์** — กรอกข้อมูลในแถวที่ด้านล่างของตาราง (เลือกภาษา พิมพ์คำศัพท์ต้นทางและปลายทาง) แล้วคลิกปุ่ม **+**
- **ค้นหาคำศัพท์** — กรองรายการตาม **ภาษาต้นทาง**, **ภาษาปลายทาง** หรือ **ข้อความ** ที่ต้องการค้นหา; คลิก **ล้างตัวกรอง** เพื่อรีเซ็ต
- **ลบคำศัพท์** — คลิกไอคอนถังขยะในแถวของคำศัพท์นั้น
- **นำเข้า** — โหลดคำศัพท์จากไฟล์ `.csv`, `.xlsx` หรือ `.xls` ไฟล์ควรมีคอลัมน์ `source_language`, `target_language`, `source_text` และ `target_text`
- **ส่งออก CSV** / **ส่งออก XLSX** — ดาวน์โหลดคำศัพท์ทั้งหมดของคุณเพื่อสำรองข้อมูลหรือแชร์
- **เทมเพลต CSV** / **เทมเพลต XLSX** — ดาวน์โหลดไฟล์เปล่าที่มีหัวคอลัมน์ที่ถูกต้องเพื่อกรอกและนำเข้า

<br/>

> ℹ️ **หมายเหตุ**<br/>
> ใน **แอปพลิเคชันเดสก์ท็อป** อภิธานศัพท์จะถูกจัดเก็บไว้ในเครื่อง ใน **เวอร์ชันเว็บ** ผู้ใช้แต่ละคนจะมีอภิธานศัพท์ของตนเอง ดังนั้นคำศัพท์ของคุณจะไม่ส่งผลกระทบต่อผู้ใช้รายอื่น
...
คีย์ API จะถูกกำหนดค่าผ่านตัวแปรสภาพแวดล้อมของระบบหรือ Docker - จะไม่ป้อนใน UI เว็บ สำหรับผู้ให้บริการที่กำหนดเอง ให้ตั้งค่า `CUSTOM_PROVIDER_NAME`, `CUSTOM_PROVIDER_URL`, และ `CUSTOM_PROVIDER_API_KEY` (ทั้งสามรายการจำเป็น) หน้านี้จะแสดงว่าผู้ให้บริการรายใดมีคีย์ที่กำหนดค่าไว้ และให้คุณทดสอบแต่ละรายการได้โดยคลิกปุ่ม `Test`
...
> 💡 **เคล็ดลับ** <br/>
> หากคุณไม่ต้องการใช้คีย์ API หรือชำระค่าบริการ คุณสามารถ [ดาวน์โหลด Ollama](https://ollama.com) และรันโมเดล (เช่น `translategemma:4b`) ภายในเครื่องบนเครื่องของคุณได้ฟรี หรือคุณสามารถสร้างบัญชี OpenRouter ฟรี (ไม่ต้องใช้บัตรเครดิต) เพื่อใช้โมเดลฟรี หรือรับคีย์ API ฟรีจาก Cerebras, Google, Groq, Mistral AI หรือ [NVIDIA](https://build.nvidia.com/)
