...
Para usar o Transrewrt, você precisa de acesso a pelo menos um provedor de IA. Os provedores suportados são: [OpenRouter](https://openrouter.ai) (que agrega muitos modelos), OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras, NVIDIA, Alibaba Cloud, apikey.fun, qualquer provedor compatível com OpenAI e [Ollama](https://ollama.com) para modelos locais.

Não precisa selecionar um modelo pago para começar. Assim que adicionar a sua chave de API OpenRouter, a aplicação ativa automaticamente uma opção **grátis** integrada da OpenRouter. Isto permite-lhe começar a traduzir, reescrever e transformar texto imediatamente. Alternativamente, também pode obter uma chave de API gratuita da Cerebras, Google, Groq, Mistral AI ou [NVIDIA](https://build.nvidia.com/) (API compatível com OpenAI).
...
![Seletor de modelo](../images/screenshots/pt/preset-selector.png)
...
9. Opcionalmente refine o resultado com **Reformular…** ou alternativas de palavras — veja [Refinando sua tradução](#refining-translation).
...
2. **Alternativas de palavras** — selecione uma ou mais palavras ou uma frase curta na saída (se você selecionar apenas parte de uma palavra, o aplicativo expande a seleção para palavras completas), então clique com o botão direito ou clique em **Refrasear…**. Uma pequena lista de alternativas aparece perto da seleção; clique em uma para substituí-la. Cada opção pode substituir um trecho ligeiramente mais amplo do que sua seleção para que a frase permaneça gramatical. Se você tiver menos de cinco versões, a saída editada é salva como uma nova versão; em cinco versões, apenas a **versão 5** é atualizada. Clicar com o botão direito sem seleção seleciona a palavra sob o cursor (ou não faz nada se não houver palavra lá). Pressione **Esc** ou clique fora da lista para cancelar sem alterar a saída.
...
![Espaço de trabalho de transformação](../images/screenshots/pt/transform.png)
...
- **Descrição da saída (p. ex. transformada, resumida, etc.)**: uma palavra curta descrevendo o resultado.
...
- **Dígitos fracionários de custo** altera a forma como os decimais de custo são exibidos.
...
**Backup de configuração** (apenas para administradores de aplicativos de desktop e web)
...
- **Restaurar do backup** - abre um **diálogo de confirmação primeiro**. Escolha o ZIP de backup dentro do diálogo (**Procurar** / seletor de arquivos ou arrastar e soltar onde suportado), depois revise as opções:
...
![Guia Modelos de configurações](../images/screenshots/pt/settings-general.png)
...
Os provedores suportados são: OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras, NVIDIA, Alibaba Cloud, apikey.fun, **Ollama** (modelos locais via URL base) e um **provedor personalizado compatível com OpenAI** opcional (nome, URL e chave de API — apenas no modo Avançado). Você só precisa configurar os provedores que usa.

**Aplicação web: apenas administrador**

As chaves de API são configuradas através de variáveis de ambiente do sistema ou Docker — não são inseridas na interface web. Para o provedor personalizado, defina `CUSTOM_PROVIDER_NAME`, `CUSTOM_PROVIDER_URL` e `CUSTOM_PROVIDER_API_KEY` (todos os três são necessários). Esta página mostra quais os provedores que têm uma chave configurada e permite testar cada um clicando no botão `Test`.
...
Use **Configuração da API** para armazenar chaves de API para cada provedor que você usa. Para Ollama, insira a **URL base** em vez de uma chave de API. Para um provedor personalizado compatível com OpenAI (qualquer endpoint que não esteja na lista integrada, como um servidor auto-hospedado ou gateway), insira um **nome de provedor**, **URL base** (como `https://my-llm.example.com/v1`) e **chave de API**; todos os três são necessários. A URL e o nome são editados inline; use **Editar** para substituir a chave de API. Os modelos de provedor personalizado aparecem apenas no modo **Avançado** (Configurações → Modelos).

<br/>

> 💡 **Dica** <br/>
> Se não quiser usar uma chave de API ou pagar pelo uso, pode [descarregar Ollama](https://ollama.com) e executar modelos (como `translategemma:4b`) localmente na sua máquina gratuitamente. Alternativamente, pode criar uma conta gratuita na OpenRouter (sem necessidade de cartão de crédito) para usar os seus modelos gratuitos, ou obter uma chave de API gratuita da Cerebras, Google, Groq, Mistral AI ou [NVIDIA](https://build.nvidia.com/).

<br/>

- Adicione apenas os provedores de que você precisa. Em **Configurações** > **Modelos**, cada ID de modelo começa com o provedor (por exemplo, `openrouter/openrouter/free`, `openai/gpt-4o`, `nvidia/nvidia/nemotron-nano-3-30b-a3b`, `ollama/llama3`, `MyProvider/…` para um endpoint personalizado chamado `MyProvider`).
...
- o nome da aplicação e o slogan
- o número da versão e a data de compilação
- informações de licença e direitos autorais, com uma ligação para abrir **Avisos de terceiros**
