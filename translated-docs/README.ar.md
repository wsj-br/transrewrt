...
- **تحويل** - مطالبات ذكاء اصطناعي مخصصة؛ إنشاء وإدارة المطالبات، لغة هدف اختيارية لكل مطالبة
- **المسرد** - تخزين أزواج مصطلحات المصدر/الهدف لكل زوج لغات وتطبيقها أثناء الترجمة لضمان ثبات المصطلحات المختارة؛ إدارة المصطلحات في الإعدادات (إضافة/تحرير/حذف، استيراد CSV/XLSX وتصدير القوالب)
- **السجل** - سجل تنفيذ كامل مع نص الإدخال/الإخراج، والتصفية، والتصدير
- **سهل ومتقدم** - الوضع السهل (افتراضي): إعدادات مسبقة منسقة لكل مزود (**مجاني (OpenRouter)**، **قياسي**، **متقدم**، **تقني**؛ تظهر فقط الإعدادات المسبقة التي لها تعيين للمزود المحدد) دون اختيار معرفات النماذج؛ الوضع المتقدم: قائمة نماذج كاملة من المزودين الذين تم تكوينهم
- **النماذج والتكلفة** - لوحات معلومات التكلفة والاستخدام (ملخص، حسب النموذج، جميع المكالمات) مع التصدير؛ يعرض OpenRouter الإنفاق الفعلي، ويستخدم المزودون الآخرون تقديرات
- **واجهة المستخدم** - واجهة متعددة اللغات (أكثر من 30 لغة، دعم RTL)، خطوط، ...
- **وضع الويب** - دعم متعدد المستخدمين مع أدوار المسؤول
- **سطح المكتب** - تطبيق Electron لنظامي التشغيل Windows و Linux
- **مستضاف ذاتيًا** - صورة Docker لمعماريات amd64 و arm64 (جاهزة لـ Raspberry Pi)
