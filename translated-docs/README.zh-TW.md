...
- **翻譯** - 在數十種語言之間進行翻譯，並自動偵測來源語言
- **重寫** - 修復語法、提高清晰度、正式/非正式、縮短、擴展、技術性
- **轉換** - 自訂 AI 提示；建立和管理提示，每個提示可選目標語言
- **詞彙表** - 為每種語言對儲存來源/目標術語配對，並在翻譯期間套用，以確保選定的術語保持一致；在設定中管理術語（新增/編輯/刪除、CSV/XLSX 匯入和範本匯出）
- **歷史記錄** - 完整的執行歷史記錄，包含輸入/輸出文字、篩選和匯出
- **簡易與進階** - 簡易模式（預設）：每個提供者都有精選的預設（**免費 (OpenRouter)**、**標準**、**進階**、**技術性**；僅顯示有為所選提供者設定對應的預設），無需選擇模型 ID；進階模式：來自您已設定提供者的完整模型列表
- **模型與成本** - 成本和使用量儀表板（摘要、依模型、所有通話），並可匯出；OpenRouter 顯示實際支出，其他提供者使用估計值
- **使用者介面** - 多語言介面（30 多種語言，支援 RTL），字型，...
- **網頁模式** - 支援多使用者及管理員角色
- **桌面版** - 適用於 Windows 和 Linux 的 Electron 應用程式
- **自行託管** - 適用於 amd64 和 arm64（適用於 Raspberry Pi）的 Docker 映像檔
