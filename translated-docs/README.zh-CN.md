---
translation_last_updated: '2026-06-02T15:30:40.377Z'
...
- **简易 & 高级** - 简易模式（默认）：按提供商精选的预设（**免费 (OpenRouter)**、**标准**、**高级**、**技术**；仅显示所选提供商支持映射的预设），无需选择模型 ID；高级模式：显示您已配置提供商的全部模型列表
- **模型与费用** - 费用和使用情况仪表板（摘要、按模型、所有调用），支持导出；OpenRouter 显示实际支出，其他提供商使用估算值
- **用户界面** - 多语言界面（30 多种语言，支持从右到左排版）、字体等
- **网页模式** - 支持多用户及管理员角色
