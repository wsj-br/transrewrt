---
translation_last_updated: '2026-03-30T00:45:57.419Z'
...
> ℹ️ **注意**<br/>
> 在 Docker 中，LLM 凭据通过环境变量设置，例如 `OPENROUTER_API_KEY`、`OPENAI_API_KEY`、`CEREBRAS_API_KEY`……（不在 Web 界面中设置）。在桌面端（Electron）中，您需在 **设置 → API** 中配置密钥。
...
在 Debian/Ubuntu 上，您可能需要先安装额外依赖：
...
> ℹ️ **注意**<br/>
> Windows 可能会显示以下安全警告之一（对于未签名/独立应用属正常情况）：
>   - **用户账户控制 (UAC)**：“您要允许此来自未知发布者的应用对设备进行更改吗？” → 点击 **是**。
>   - **Microsoft Defender SmartScreen**：“Windows 保护了您的 PC” → 点击 **更多信息** → **仍然运行**。
>
> 此情况发生是因为该应用未由 Microsoft 或主要出版商签名——只要从我们官方的 GitHub 发布页面下载即为安全
>  （请核对下方的 SHA256 校验和）。
...
请勿对翻译、改写或转换任务使用 OpenRouter 的 **Body Builder** 模型（[`openrouter/bodybuilder`](https://openrouter.ai/openrouter/bodybuilder)）：它返回的是 JSON 请求负载，而不是这些任务所需的完整文本。详见用户指南中的 [设置 → 模型](USER-GUIDE.zh-CN.md#models)。
...
> ⚠️ **警告**<br/>
> 如果您从其他设备、容器或服务使用 Ollama，请记得配置 Ollama 以允许外部连接（不能仅限于本地主机）。
...
| `TZ`             | `Europe/London`         | 服务器端 IANA 时区（用于日志等）；UI 仍遵循浏览器设置。参见 [Docker → 时区](#docker-timezone) |
