---
translation_last_updated: '2026-05-27T10:06:30.408Z'
...
アプリはデフォルトで **かんたん** モードで実行されます。設定で **プリセット**（例：無料 (OpenRouter)、Lite、Technicalなど）と **プロバイダー** を選択しますが、モデルIDは選択しません。[**設定** > **一般設定**](#general-settings) で **高度** モードに切り替えると、[**設定** > **モデル**](#models) にある従来のモデル一覧を使用できます。
...
- **かんたん** モードでは、**プリセット** とは、選択した **プロバイダー**（OpenRouter、OpenAI、Ollama など）に対応するモデルにマッピングされたあらかじめ設定された構成（無料 (OpenRouter)、Lite、高度、Technicalなど）です。現在のプロバイダーにマッピングがあるスキルのみがツールバーに表示されます。翻訳、書き換え、変換の各スキルはツールバーから選択します。
- **高度** モードでは、**モデル** とは、直接選択するAIエンジンを指します。モデルIDには **プロバイダー接頭辞** が使用されます（例：`openrouter/…`、`openai/…`、`ollama/…`）。
...
**かんたん** モードでは、ツールバーに組み込みプリセット **無料 (OpenRouter)**、**Lite**、**高度**、**Technical** を含む **プリセット選択** が表示されます。表示されるプリセットは、[**設定** > **一般設定**](#general-settings) で選択した **プロバイダー** によって異なります。たとえば、**無料 (OpenRouter)** はプロバイダーがOpenRouterの場合にのみ表示されます。**プロバイダー** が **Ollama** の場合、プリセットの代わりにローカルにインストール済みのモデルがツールバーに表示されます。
...
- **かんたん**（デフォルト）：**プロバイダー**（OpenRouter、OpenAI、Anthropic、Google Gemini、DeepSeek、Groq、Mistral、xAI、Cerebras、またはOllama）を選択します。クラウドプロバイダーでは、ツールバーの組み込みプリセットを使用します。**Ollama** はプリセットの代わりに、マシンにインストールされたモデルを一覧表示します。かんたんモードでは、**プリセットカタログ** にカタログのバージョンと最終更新時刻が表示されます。**プリセットカタログを更新** をクリックすると、プロジェクトリポジトリから最新のスキルリストを取得できます（アプリは定期的にバックグラウンドでも確認します）。
- **高度**：ツールバーで個別のモデルを選択します。[**設定** > **モデル**](#models) で一覧を管理します。
...
- 異なるプリセット（かんたん）またはモデル（高度）を選択する
- 入力内容を短くする
- [**設定** > **一般設定**](#general-settings) で **入力中にリアルタイム翻訳** をオフにする
