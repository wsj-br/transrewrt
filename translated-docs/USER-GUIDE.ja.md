---
translation_last_updated: '2026-05-17T23:33:00.673Z'
...
アプリはデフォルトで **かんたん**モードで実行されます。設定で**プロバイダー**（OpenRouter、OpenAI、Ollamaなど）を選択すると、モデルIDを指定せずに**スキル**（無料、高速、技術的など）を選べます。[**設定** > **一般設定**](#general-settings)で**高度**モードに切り替えると、[**設定** > **モデル**](#models)の従来のモデル一覧が利用できます。
...
- **かんたん**モードでは、**スキル**は（無料、高速、高度、技術的、法的など）あらかじめ設定されたもので、選択した**プロバイダー**（OpenRouter、OpenAI、Ollamaなど）に対応するモデルにマッピングされます。翻訳、書き換え、変換の各ツールバーからスキルを選択します。
- **高度**モードでは、**モデル**は直接選択するAIエンジンです。モデルIDは**プロバイダー接頭辞**を使用します（例: `openrouter/…`、`openai/…`、`ollama/…`）。
- **APIキー**（Ollamaの場合は**ベースURL**）は、アプリがそのプロバイダーに接続するための手段です。

**デスクトップアプリ**を使用している場合は、使用する各プロバイダーのAPIキーを[**設定** > **API設定**](#api-config)に追加してください。OpenRouterのみを使用する場合は、下記の「無料のOpenRouter APIキーの取得方法」[How to get a free OpenRouter API key](#how-to-get-an-api-key-desktop-app)を参照してください。APIキーを使いたくない場合は、[ollama.com](https://ollama.com)からOllamaをインストールして、`translategemma:4b`などのローカルモデルを使用できます。
...
**かんたん**モードでは、ツールバーに**スキルセレクター**（無料、高速、高度、技術的、法的などのプリセット）が表示されます。スキルは[**設定** > **一般設定**](#general-settings)で選択した**プロバイダー**に依存します。**プロバイダー**が**Ollama**の場合は、スキルの代わりにインストール済みのローカルモデルがツールバーに表示されます。
...
- **かんたん**（デフォルト）：**プロバイダー**（OpenRouter、OpenAI、Anthropic、Google Gemini、DeepSeek、Groq、Mistral、xAI、Cerebras、またはOllama）を選択します。クラウドプロバイダーはツールバーの組み込みスキルプリセットを使用します。**Ollama** はスキルの代わりに、ローカルマシンにインストールされたモデルをリストします。
- **高度**：ツールバーで個別のモデルを選択します。リストは [**設定** > **モデル**](#models) で管理します。
