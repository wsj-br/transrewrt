---
translation_last_updated: '2026-05-17T23:32:43.503Z'
...
ডিফল্টভাবে অ্যাপটি **সহজ** মোডে চলে: আপনি **দক্ষতা** (যেমন ফ্রি, ফাস্ট, বা টেকনিক্যাল) এবং একটি **প্রদানকারী** নির্বাচন করেন সেটিংসে, মডেল আইডি নির্বাচন না করেই। ক্লাসিক মডেল তালিকা পেতে [**সেটিংস** > **সাধারণ সেটিংস**](#general-settings) এ **উন্নত** মোডে স্যুইচ করুন, [**সেটিংস** > **মডেলগুলি**](#models) থেকে।
...
- [সেটিংস](#settings)
  ...
  - [রূপান্তর (সেটিংস ট্যাব)](#transform-settings)
  - [ব্যবহারকারীরা](#users)
...
- **সহজ** মোডে, একটি **দক্ষতা** হল একটি প্রিসেট (ফ্রি, ফাস্ট, উন্নত, টেকনিক্যাল, আইনগত) যা আপনার নির্বাচিত **প্রদানকারী** (ওপেনরাউটার, ওপেনএআই, ওলামা, এবং অন্যান্য) এর জন্য একটি মডেলের সাথে মানানসই। আপনি অনুবাদ, পুনঃলেখন, এবং রূপান্তরে টুলবারে দক্ষতা নির্বাচন করেন।
- **উন্নত** মোডে, একটি **মডেল** হল AI ইঞ্জিন যা আপনি সরাসরি নির্বাচন করেন। মডেল আইডিগুলি একটি **প্রদানকারী প্রিফিক্স** ব্যবহার করে (যেমন `openrouter/…`, `openai/…`, `ollama/…`)।
- একটি **API কী** (অথবা, ওলামার জন্য, একটি **বেস URL**) হল কিভাবে অ্যাপটি সেই প্রদানকারীর সাথে যোগাযোগ করে।

যদি আপনি **ডেস্কটপ অ্যাপ** ব্যবহার করেন, তবে প্রতিটি প্রদানকারীর জন্য [**সেটিংস** > **API কনফিগ**](#api-config) এ কী যোগ করুন। শুধুমাত্র ওপেনরাউটার ব্যবহারের জন্য, নিচে [কিভাবে একটি ফ্রি ওপেনরাউটার API কী পেতে হয়](#how-to-get-an-api-key-desktop-app) দেখুন। যদি আপনি API কী ব্যবহার করতে না চান, তবে আপনি ওলামা ইনস্টল করতে পারেন ( [ollama.com](https://ollama.com) থেকে) এবং স্থানীয় মডেলগুলি ব্যবহার করতে পারেন, যেমন `translategemma:4b`।
...
5. **অনুবাদ** এ, টুলবারে একটি **দক্ষতা** (সহজ) বা **মডেল** (উন্নত) নির্বাচন করুন।
6. [**সেটিংস** > **ভাষাগুলি**](#languages) খুলুন এবং আপনার **শীর্ষ ভাষাগুলি** নির্বাচন করুন যদি আপনি চান আপনার সবচেয়ে ব্যবহৃত ভাষাগুলি প্রথমে প্রদর্শিত হোক।
7. সবকিছু কাজ করছে কিনা তা নিশ্চিত করতে একটি সহজ অনুবাদ চালান, তারপর **পুনঃলেখন** এবং **রূপান্তর** চেষ্টা করুন।

এই ক্রমটি গুরুত্বপূর্ণ। এটি সবচেয়ে সাধারণ প্রথম-ব্যবহার সমস্যা প্রতিরোধ করে: অ্যাপটির একটি কার্যকর API সংযোগ বা নির্বাচিত দক্ষতা/মডেল আগে একটি কাজ চালানোর চেষ্টা করা।
...
- ডান দিকে, এটি **দক্ষতা বা মডেল নির্বাচনকারী** এবং **ইন্টারফেস ভাষা** নিয়ন্ত্রণ দেখায়।

**সহজ** মোডে, টুলবারে একটি **দক্ষতা নির্বাচনকারী** (ফ্রি, ফাস্ট, উন্নত, টেকনিক্যাল, আইনগত, এবং অনুরূপ প্রিসেট) দেখায়। দক্ষতাগুলি আপনার নির্বাচিত **প্রদানকারী** এর উপর নির্ভর করে যা আপনি [**সেটিংস** > **সাধারণ সেটিংস**](#general-settings) এ নির্বাচন করেছেন। যদি **প্রদানকারী** **ওলামা** হয়, তবে টুলবারে দক্ষতার পরিবর্তে আপনার ইনস্টল করা স্থানীয় মডেলগুলি তালিকাবদ্ধ করা হয়।
...
4. টুলবারে একটি দক্ষতা (সহজ) বা মডেল (উন্নত) নির্বাচন করুন।
...
প্রম্পট এডিটরে **Generate prompt**, **Improve prompt**, বা **Translate prompt** ব্যবহার করার সময়, **সহজ** মোডটি অনুবাদ এবং পুনঃলেখনের মতো একই দক্ষতা নির্বাচক প্রদান করে; **উন্নত** মোডটি মডেল তালিকা ব্যবহার করে।
...
**Easy** মোডে, টুলবারে দক্ষতার মাধ্যমে এবং সাধারণ সেটিংসে **প্রদানকারী** ব্যবহার করে মডেল নির্বাচন করা হয়; **মডেলগুলি** ট্যাবটি লুকানো থাকে।

<br/>

> ℹ️ **নোট**<br/>
> ওয়েব সংস্করণে, প্রতিটি ব্যবহারকারীর নিজস্ব কনফিগারেশন থাকে। AI অভিজ্ঞতা, প্রদানকারী, নির্বাচিত মডেল বা দক্ষতা, ভাষা, সাধারণ বিকল্প এবং রূপান্তর প্রম্পটগুলির মতো সেটিংস প্রতি ব্যবহারকারী ভিত্তিক সংরক্ষিত হয়। আপনি যে পরিবর্তনগুলি করছেন তা অন্যান্য ব্যবহারকারীদের প্রভাবিত করে না।
...
- **সহজ** (ডিফল্ট): একটি **প্রদানকারী** নির্বাচন করুন (ওপেনরাউটার, ওপেনএআই, অ্যানথ্রোপিক, গুগল জেমিনি, ডিপসীক, গ্রোক, মিস্ট্রাল, এক্সএআই, সেরেব্রাস বা ওলামা)। ক্লাউড প্রদানকারীরা টুলবারে অন্তর্নির্মিত দক্ষতা প্রিসেট ব্যবহার করে। **ওলামা** দক্ষতার পরিবর্তে আপনার মেশিনে ইনস্টল করা মডেলগুলির তালিকা দেয়।
- **উন্নত**: টুলবারে আলাদা মডেল নির্বাচন করুন; [**সেটিংস** > **মডেলগুলি**](#models) এর অধীনে তালিকা পরিচালনা করুন।

**ওয়েব অ্যাপ**-এ, কোন প্রদানকারীরা দেখায় তা সার্ভার পরিবেশে সেট করা API কী-এর উপর নির্ভর করে। **ডেস্কটপ অ্যাপ**-এ, [**API কনফিগ**](#api-config) এর অধীনে কী কনফিগার করুন।
...
- আপনি টুলবারে একটি **দক্ষতা** (সহজ) বা **মডেল** (উন্নত) নির্বাচন করেছেন
- **সহজ** মোডে, [**সেটিংস** > **সাধারণ সেটিংস**](#general-settings) এ একটি **প্রদানকারী** রয়েছে যার একটি কার্যকরী কী (অথবা ওলামা URL) এবং সেই প্রদানকারীর জন্য অন্তত একটি দক্ষতা রয়েছে
- **উন্নত** মোডে, [**সেটিংস** > **মডেলগুলি**](#models) এ অন্তত একটি মডেল তালিকাবদ্ধ রয়েছে
