---
translation_last_updated: '2026-05-18T15:42:44.243Z'
...
기본적으로 앱은 **쉬움** 모드로 실행됩니다. 설정에서 모델 ID를 선택하지 않고 **기술**(예: 무료, 빠름 또는 기술적)과 **제공업체**를 선택합니다. [**설정** > **일반 설정**](#general-settings)에서 **고급** 모드로 전환하면 [**설정** > **모델**](#models)의 기존 모델 목록을 사용할 수 있습니다.
...
- **쉬움** 모드에서는 **기술**이 사전 설정된 값(무료, 빠름, 고급, 기술적, 법적 등)이며, 선택한 **제공업체**(OpenRouter, OpenAI, Ollama 등)에 해당하는 모델로 매핑됩니다. 번역, 다시 작성, 변환 화면의 툴바에서 기술을 선택합니다.
...
**데스크톱 앱**을 사용하는 경우, 사용하는 각 제공업체에 대해 [**설정** > **API 설정**](#api-config)에서 키를 추가하세요. OpenRouter만 사용하는 경우 아래의 [무료 OpenRouter API 키를 확보하는 방법](#how-to-get-an-api-key-desktop-app)을 참조하세요. API 키를 사용하고 싶지 않다면 [ollama.com](https://ollama.com)에서 Ollama를 설치하여 `translategemma:4b`과 같은 로컬 모델을 사용할 수 있습니다.
...
- 오른쪽에는 **기술 또는 모델 선택기**와 **인터페이스 언어** 조정 기능이 표시됩니다.

**쉬움** 모드에서는 툴바에 **기술 선택기**(무료, 빠름, 고급, 기술적, 법적 등 사전 설정)가 표시됩니다. 기술은 [**설정** > **일반 설정**](#general-settings)에서 선택한 **제공업체**에 따라 달라집니다. **제공업체**가 **Ollama**인 경우, 기술 대신 설치된 로컬 모델이 툴바에 표시됩니다.
...
프롬프트 편집기에서 **프롬프트 생성**, **프롬프트 개선** 또는 **프롬프트 번역**을 사용할 때, **쉬움** 모드는 번역 및 다시 작성과 동일한 기술 선택기를 제공하며, **고급** 모드는 모델 목록을 사용합니다.
...
**쉬움** 모드에서는 툴바의 기술과 일반 설정의 **제공업체**를 통해 모델을 선택하며, **모델** 탭은 숨겨집니다.

<br/>

> ℹ️ **참고**<br/>
> 웹 버전에서는 각 사용자가 자신의 설정을 갖습니다. AI 기능, 제공업체, 선택한 모델 또는 기술, 언어, 일반 옵션, 변환 프롬프트 등의 설정은 사용자별로 저장됩니다. 사용자가 변경한 사항은 다른 사용자에게 영향을 주지 않습니다.
...
- **쉬움**(기본값): **제공업체**(OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras, 또는 Ollama)를 선택합니다. 클라우드 제공업체는 도구 모음의 기본 제공 기술 사전 설정을 사용합니다. **Ollama**는 기술 대신 사용자의 기기에 설치된 모델을 나열합니다.
...
- 툴바에서 **기술** (쉬움) 또는 **모델** (고급)을 선택했습니다.
- **쉬움** 모드에서는 [**설정** > **일반 설정**](#general-settings)에서 작동하는 키(또는 Ollama URL)가 있는 **제공업체**를 설정하고, 해당 제공업체에 대해 하나 이상의 기술이 있어야 합니다.
- **고급** 모드에서는 [**설정** > **모델**](#models)에 하나 이상의 모델이 나열되어 있어야 합니다.
- API 설정이 정상적으로 작동 중입니다.
