---
translation_last_updated: '2026-05-27T10:06:46.822Z'
...
기본적으로 앱은 **쉬움** 모드에서 실행됩니다. 설정에서 모델 ID를 선택하지 않고 **사전 설정**(예: 무료(OpenRouter), Lite 또는 기술 전문)과 **제공업체**를 선택합니다. [**설정** > **일반 설정**](#general-settings)에서 **고급** 모드로 전환하면 [**설정** > **모델**](#models)의 기존 모델 목록을 사용할 수 있습니다.
...
- **쉬움** 모드에서는 **사전 설정**이 선택한 **제공업체**(OpenRouter, OpenAI, Ollama 등)에 매핑되는 사전 설정(Free (OpenRouter), Lite, Advanced, Technical 등)을 의미합니다. 현재 제공업체에 매핑된 기술만 도구 모음에 표시됩니다. 번역, 다시 작성, 변환 시 기술을 선택합니다.
- **고급** 모드에서는 **모델**이 직접 선택하는 AI 엔진을 의미합니다. 모델 ID는 **제공업체 접두사**를 사용합니다(예: `openrouter/…`, `openai/…`, `ollama/…`).
...
**쉬움** 모드에서는 도구 모음에 내장된 사전 설정 **무료(OpenRouter)**, **Lite**, **고급**, **기술 전문**을 포함하는 **사전 설정 선택기**가 표시됩니다. 표시되는 사전 설정은 [**설정** > **일반 설정**](#general-settings)에서 선택한 **제공업체**에 따라 달라집니다. 예를 들어 **무료(OpenRouter)** 는 제공업체가 OpenRouter일 때만 표시됩니다. **제공업체**가 **Ollama**인 경우 도구 모음에 사전 설정 대신 로컬에 설치된 모델이 나열됩니다.
...
- **쉬움**(기본값): **제공업체**(OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras 또는 Ollama)를 선택합니다. 클라우드 제공업체는 도구 모음의 내장 사전 설정을 사용합니다. **Ollama**는 사전 설정 대신 로컬에 설치된 모델을 나열합니다. 쉬움 모드에서 **사전 설정 카탈로그**는 카탈로그 버전과 마지막 업데이트 시간을 표시합니다. 프로젝트 저장소에서 최신 기술 목록을 가져오려면 **사전 설정 카탈로그 새로 고침**을 클릭합니다(앱은 주기적으로 백그라운드에서 확인도 수행함).
