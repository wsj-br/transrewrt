...
9. 선택적으로 **Rephrase…** 또는 단어 대안으로 결과를 다듬습니다 — [번역 다듬기](#refining-translation)를 참조하세요.
...
- **붙여넣기 시 자동 실행**는 텍스트를 붙여넣는 즉시 번역을 실행합니다.
- **결과를 클립보드에 자동 복사**는 성공적인 실행 후 결과를 자동으로 복사합니다.
- **타이핑 중 실시간 번역** (⚠️ 이로 인해 사용 비용이 증가할 수 있습니다)는 타이핑하는 동안 번역을 실행합니다.
- **타임아웃 (ms)** 는 앱이 실시간 번역을 실행하기 전에 대기하는 시간을 조절합니다.
...
3. **From** 언어 상자가 나타나면 원하는 언어를 선택합니다.
...
- **출력 설명(예: 변환, 요약 등)**: 결과를 설명하는 짧은 단어입니다.
- **온도(0.0 → 1.0)**: 모델의 동작 방식; 아래를 참조하세요.
- **대상 언어 요청**: 프롬프트가 실행될 때 언어 선택기를 추가합니다.
...
- **ENTER 동작 대상**는 `Enter`가 작업을 실행할지 또는 새 줄을 삽입할지를 선택합니다.
- **붙여넣기 시 자동 실행**는 텍스트를 붙여넣는 즉시 번역을 시작합니다.
- **결과를 클립보드에 자동 복사**는 성공적인 결과를 자동으로 복사합니다.
- **타이핑 중 실시간 번역** (⚠️ 이로 인해 사용 비용이 증가할 수 있습니다)는 타이핑하는 동안 번역을 실행합니다.
...
**구성 백업** (데스크톱 앱 및 웹 관리자 전용)
- **백업에 사용량 데이터 포함** - 활성화 시 ZIP에는 실행 기록 및 API 호출 데이터도 포함됩니다.
- **구성 백업** - `transrewrt-config-backup-YYYY-MM-DD_HHMMSS.zip`의 로컬 시간으로 단일 ZIP을 생성하며, `config.json`, `state.json`, 선택적 암호화 키, 사용자, 기본 설정, 사용자 정의 프롬프트 및 사용량 데이터가 포함됩니다. 성공적인 백업 후 확인 메시지에 저장된 파일 이름이 표시됩니다.
- **백업에서 복원** - 먼저 **확인 대화상자**가 열립니다. 대화상자에서 백업 ZIP을 선택합니다 (**찾아보기** / 파일 선택기 또는 지원되는 경우 드래그 앤 드롭), 그런 다음 옵션을 검토합니다:
  - **사용량 데이터 복원** - 사용량이 포함된 ZIP에서 사용량/역사 데이터를 가져옵니다; 설정 및 프롬프트만 원할 경우 선택 해제합니다.
  - **복원하기 전에 이전 사용량 데이터 지우기** - 백업을 적용하기 전에 이 설치에서 기존 사용량/역사 데이터를 제거합니다(선택 사항; 깨끗한 교체를 원할 때 사용합니다).
웹 또는 데스크톱 버전에서 생성된 백업은 서로 복원할 수 있습니다. 웹 버전에서 데스크톱 백업을 복원할 때 데이터는 관리자 사용자에게 복원됩니다.
...
- 다른 사전 설정(쉬움) 또는 모델(고급)을 선택합니다.
- 더 짧은 입력을 사용합니다.
- [**설정** > **일반 설정**](#general-settings)에서 **타이핑 중 실시간 번역**를 끕니다.
- 간단한 작업에 무료 모델을 사용합니다(자세한 내용은 [모델](#models) 참조).
<br/>
