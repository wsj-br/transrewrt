---
translation_last_updated: '2026-05-03T19:25:22.033Z'
...
- [일반적인 문제](#common-issues)
  ...
  - [비용이 "사용 가능하지 않음" 또는 잘못 표시됨](#cost-shows-not-available-or-seems-wrong)
...
**데스크톱 앱**을 사용하는 경우 사용하는 각 제공업체에 대해 [**설정** > **API 설정**](#api-config)에서 키를 추가하세요. OpenRouter만 사용하는 경우 아래의 [API 키를 얻는 방법](#how-to-get-an-api-key-desktop-app)을 참조하세요. API 키를 사용하지 않으려면 [ollama.com](https://ollama.com)에서 Ollama를 설치하고 `translategemma:4b`과 같은 로컬 모델을 사용할 수 있습니다.
...
3. **데스크톱 앱**을 사용하는 경우 [**설정** > **API 설정**](#api-config)을 열고 하나 이상의 제공업체(예: OpenRouter)에 대한 API 키를 추가한 후 **테스트**를 클릭하여 작동 여부를 확인합니다.
4. [**설정** > **모델**](#models)을 열고 하나 이상의 모델을 **선택한 모델**에 추가합니다.
5. [**설정** > **언어**](#languages)을 열고 자주 사용하는 언어를 맨 위에 표시하려면 **상위 언어**를 선택합니다.
6. **번역**으로 이동하여 간단한 번역을 실행하여 모든 것이 제대로 작동하는지 확인합니다.
7. 정상 작동하면 **다시 작성**을 시도하고 그 다음 **변환**을 시도합니다.

이 순서는 중요합니다. 가장 흔한 초기 사용 문제인 앱에 작동하는 API 연결이나 선택된 모델이 없을 때 작업을 실행하려는 시도를 방지할 수 있습니다.
...
일부 무료 모델은 항상 사용 가능한 것은 아닙니다. 오프라인이거나 사용량 제한이 있을 수 있습니다. 이런 경우 앱이 자동으로 해당 모델을 사용 가능한 목록에서 제거합니다. 표시할 모델을 제어하려면 [**설정** > **모델**](#models)로 이동하여 모델 목록을 편집하세요.
...
- **Enter**은(는) `Enter` 키를 누를 때 발생하는 동작을 제어합니다:
...
- 텍스트를 더 격식 있게 또는 비격식적으로 변경 (**Formal** / **Informal**)
- 텍스트 축약 또는 확장 (**축약** / **확장**)
...
프롬프트 목록이 비어 있으면 변환 작업 영역에서 **샘플 프롬프트 불러오기**를 클릭합니다. 동일한 기능은 내보내기/가져오기 행의 [**설정** > **변환 프롬프트**](#transform-prompts)에서도 항상 사용할 수 있습니다. 두 방법 모두 기본 제공되는 예제를 추가하여 빠르게 시작할 수 있습니다.
...
> ℹ️ **참고**<br/>
> [**설정** > **변환 프롬프트**](#transform-prompts)에서 저장된 프롬프트를 내보내고 가져올 수 있습니다.
...
> ℹ️ **참고**<br/>
> **무료** 모델만 사용하는 경우 **비용**은 0일 수 있으며, 비용 중심 요약은 비어 보일 수 있습니다. **요약**에서 **시간 경과에 따른 사용량** 및 **모델별 사용량**은 선택한 기간 동안 활동이 있을 경우 여전히 **호출 횟수**(번역, 다시 작성, 변환)를 표시합니다.
...
- **요약**은 사용량과 비용에 대한 개요를 제공합니다. 여기에는 **시간 경과에 따른 사용량**(번역, 다시 작성, 변환의 일별 누적 **호출 횟수**)과 **모델별 사용량**(**모델별 총 호출 횟수**, 변환 포함)이 포함됩니다.
- **사용량 기준**은 번역 언어, 다시 작성 모드, 변환 프롬프트별로 활동을 분류합니다.
- **모델 기준**은 사용한 모델과 그 비용을 보여줍니다.
- **일자 기준**은 일별 합계를 표시합니다.
...
**기록**은(는) **대시보드** 페이지와 동일한 필터를 사용합니다. 이를 사용하여 기간을 선택할 수 있습니다.
...
> ℹ️ **참고**<br/>
> 웹 버전에서는 각 사용자가 자신의 구성 정보를 가집니다. 선택한 모델, 언어, 일반 옵션 및 변환 프롬프트와 같은 설정은 사용자별로 저장됩니다. 귀하가 수행한 변경 사항은 다른 사용자에게 영향을 주지 않습니다.
...
**일반 설정**에서 입력 동작, 실행 세부 정보를 **기록**에 저장할지 여부, 외형 등을 제어할 수 있습니다.
...
- **실행 기록 유지**는 사이드바의 [**기록**](#history) 보기에서 각 번역, 다시 작성, 변환 작업의 **입력 및 출력 텍스트**를 저장할지 여부를 제어합니다. 이 기능을 끄면 확인을 요청하며, 확인 시 저장된 기록 텍스트가 데이터베이스에서 제거됩니다.
- **기록 데이터 삭제**를 사용하면 저장된 텍스트를 기간별(예: 몇 개월 이상된 항목) 또는 **모든 데이터 (지우기)** 옵션을 통해 **데이터 삭제** 기능으로 제거할 수 있습니다. 이는 **기록** 보기용으로 저장된 실행 텍스트에만 영향을 미치며, 비용 또는 사용량 총계는 **삭제하지 않습니다**. **비용** 데이터를 제거하거나 축소하려면 [**설정** > **비용 추적**](#cost-tracking)을 사용하세요.

**외형**

- **작업에 비용 정보 표시**는 작업당 비용(가능한 경우)과 번역, 다시 작성, 변환 출력 패널의 총 비용 표시 여부를 제어합니다.
...
**설정** > **변환 프롬프트**를 사용하여 프롬프트를 일괄 관리할 수 있습니다.
...
- 툴바에서 모델을 선택했는지
- [**설정** > **모델**](#models)에 적어도 하나의 모델이 나열되어 있는지
- API 설정이 작동하는지
...
[**설정** > **모델**](#models)을 열고 **새로고침**를 클릭합니다.
...
- 간단한 작업에 무료 모델 사용 (자세한 내용은 [모델](#models) 참조)
...
- **무료 모델**만 사용 중이고 **비용** 수치를 확인 중인 경우(비용이 0일 수 있음); **요약**의 **사용량** 호출 수 차트는 여전히 선택한 기간의 데이터가 필요함
- 선택한 **시간 필터**가 호출이 발생한 기간을 포함하지 않는 경우 - 확인하려면 **전체**를 시도해 보세요

**전체**를 선택한 후에도 차트가 비어 있으면 [**기록**](#history) 또는 **모든 호출** 탭에 호출이 표시되는지 확인하세요.
...
**실행 기록 유지**가 꺼져 있을 수 있습니다. [**설정** > **일반 설정**](#general-settings)을 열고 활성화하세요. 이 기능을 켜도 이전에 삭제된 기록 데이터는 복원되지 않습니다.
