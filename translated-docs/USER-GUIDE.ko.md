---
translation_last_updated: '2026-05-24T17:53:05.049Z'
...
기본적으로 앱은 **쉬움** 모드에서 실행됩니다. 설정에서 모델 ID를 선택하지 않고 **기술**(예: 무료(OpenRouter), 라이트 또는 기술적)과 **제공업체**를 선택합니다. [**설정** > **일반 설정**](#general-settings)에서 **고급** 모드로 전환하면 [**설정** > **모델**](#models)의 기존 모델 목록을 사용할 수 있습니다.
...
- **쉬움** 모드에서는 **기술**이 사전 설정된 값(Free (OpenRouter), Lite, Advanced 또는 Technical)이며, 선택한 **제공업체**(OpenRouter, OpenAI, Ollama 등)에 해당하는 모델로 매핑됩니다. 현재 제공업체에 매핑된 기술만 툴바에 표시됩니다. 번역, 다시 작성 및 변환 작업 시 기술을 선택합니다.
- **고급** 모드에서는 **모델**이 직접 선택하는 AI 엔진입니다. 모델 ID는 **제공업체 접두사**를 사용합니다(예: `openrouter/…`, `openai/…`, `ollama/…`).
...
**쉬움** 모드에서는 툴바에 내장된 사전 설정 **무료(OpenRouter)**, **라이트**, **고급**, **기술적**을 포함하는 **기술 선택기**가 표시됩니다. 표시되는 기술은 [**설정** > **일반 설정**](#general-settings)에서 선택한 **제공업체**에 따라 달라집니다. 예를 들어, 제공업체가 OpenRouter일 때만 **무료(OpenRouter)** 가 표시됩니다. **제공업체**가 **Ollama**인 경우, 기술 대신 설치된 로컬 모델이 툴바에 나열됩니다.
...
- **쉬움**(기본값): **제공업체**(OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras 또는 Ollama)를 선택합니다. 클라우드 제공업체는 툴바의 내장 기술 사전 설정을 사용합니다. **Ollama**는 기술 대신 사용자의 컴퓨터에 설치된 모델을 나열합니다. 쉬움 모드에서 **기술 카탈로그**는 카탈로그 버전과 마지막 업데이트 시간을 표시합니다. **기술 카탈로그 새로 고침**을 클릭하여 프로젝트 저장소에서 최신 기술 목록을 가져옵니다(앱은 주기적으로 백그라운드에서 자동 확인도 수행합니다).
- **고급**: 툴바에서 개별 모델을 선택하고 [**설정** > **모델**](#models)에서 목록을 관리합니다.
...
- 다른 기술(Easy) 또는 모델(고급) 선택
- 더 짧은 입력 사용
- [**설정** > **일반 설정**](#general-settings)에서 **실시간 번역(입력 중)** 끄기
- 간단한 작업에는 무료 모델 사용 (참고: [모델](#models))
