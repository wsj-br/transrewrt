---
translation_last_updated: '2026-05-17T23:31:49.893Z'
...
默认情况下，应用程序以**简易**模式运行：您可以在设置中选择一个**技能**（例如免费、快速或技术）和一个**提供商**，而无需选择模型 ID。如果您想要从[**设置** > **常规设置**](#general-settings)中切换到**高级**，请访问经典模型列表，从[**设置** > **模型**](#models)中查看。
...
- 在 **简易** 模式下，**技能** 是预设（如 免费、快速、高级、技术性、法律性），会映射到您所选 **提供商**（OpenRouter、OpenAI、Ollama 等）的某个模型。您可以在“翻译”、“重写”和“转换”功能的工具栏中选择技能。
- 在 **高级** 模式下，**模型** 是您直接选择的 AI 引擎。模型 ID 使用 **提供商前缀**（例如 `openrouter/…`、`openai/…`、`ollama/…`）。
- **API 密钥**（对于 Ollama 则为 **基础 URL**）是应用连接该提供商的方式。

如果您使用的是 **桌面应用**，请在 [**设置** > **API 配置**](#api-config) 中为每个使用的提供商添加密钥。如果仅使用 OpenRouter，请参见下方的 [如何获取免费的 OpenRouter API 密钥](#how-to-get-an-api-key-desktop-app)。如果您不想使用 API 密钥，可以安装 Ollama（来自 [ollama.com](https://ollama.com)）并使用本地模型，例如 `translategemma:4b`。
...
在 **简易** 模式下，工具栏显示一个 **技能选择器**（如 免费、快速、高级、技术性、法律性 等预设）。技能取决于您在 [**设置** > **常规设置**](#general-settings) 中选择的 **提供商**。如果 **提供商** 为 **Ollama**，工具栏将列出您已安装的本地模型，而非技能。
...
- **简易**（默认）：选择一个 **提供商**（OpenRouter、OpenAI、Anthropic、Google Gemini、DeepSeek、Groq、Mistral、xAI、Cerebras 或 Ollama）。云提供商使用工具栏中的内置技能预设。**Ollama** 则列出您机器上安装的模型，而非技能。
