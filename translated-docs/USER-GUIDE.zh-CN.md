---
translation_last_updated: '2026-05-27T10:06:00.231Z'
...
默认情况下，应用以 **简易** 模式运行：您只需在“设置”中选择一个 **预设**（例如 免费 (OpenRouter)、精简版 或 技术版）和一个 **提供商**，无需选择模型 ID。如果您希望使用 [**设置** > **模型**](#models) 中的经典模型列表，请在 [**设置** > **常规设置**](#general-settings) 中切换到 **高级** 模式。
...
- 在 **简易** 模式下，**预设** 是一个预设（如 免费 (OpenRouter)、Lite、高级 或 Technical），它会映射到您所选 **提供商**（OpenRouter、OpenAI、Ollama 等）的某个模型。只有当前提供商支持的功能才会出现在工具栏中。您可在“翻译”、“重写”和“转换”中选择功能。
...
在 **简易** 模式下，工具栏会显示一个 **预设选择器**，包含内置预设 **免费 (OpenRouter)**、**Lite**、**高级** 和 **Technical**。显示哪些预设取决于您在 [**设置** > **常规设置**](#general-settings) 中选择的 **提供商** —— 例如，只有当提供商为 OpenRouter 时，才会列出 **免费 (OpenRouter)**。如果 **提供商** 为 **Ollama**，工具栏将列出您本地安装的模型，而非预设。
...
- **简易**（默认）：选择一个 **提供商**（OpenRouter、OpenAI、Anthropic、Google Gemini、DeepSeek、Groq、Mistral、xAI、Cerebras 或 Ollama）。云提供商使用工具栏中的内置预设。**Ollama** 则列出您机器上已安装的本地模型，而非预设。在简易模式下，**预设目录** 显示目录版本和上次更新时间；点击 **刷新预设目录** 可从项目仓库获取最新的功能列表（应用也会在后台定期自动检查）。
