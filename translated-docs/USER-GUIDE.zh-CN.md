...
默认情况下，应用以 **简易** 模式运行：您只需在“设置”中选择一个 **预设**（例如 免费 (OpenRouter)、标准、高级 或 技术）和一个 **提供商**，而无需选择模型 ID。如果您想要从 [**设置** > **模型**](#models) 中使用经典的模型列表，请在 [**设置** > **常规设置**](#general-settings) 中切换到 **高级** 模式。
...
- 在 **简易** 模式下，一个 **预设**（免费 (OpenRouter)、标准、高级 或 技术）会映射到您所选 **提供商**（OpenRouter、OpenAI、Ollama 等）的一个模型。只有当前提供商支持的预设才会出现在工具栏中。您可在 翻译、重写 和 转换 功能中选择预设。
...
5. 在 **翻译** 功能中，从工具栏选择一个 **预设**（简易）或 **模型**（高级）。
6. 打开 [**设置** > **语言**](#languages)，选择您的 **常用语言**，以便最常用的语言优先显示。
7. 运行一次简单翻译以确认一切正常，然后尝试 **重写** 和 **转换**。

此顺序很重要。它可避免最常见的初次使用问题：在应用尚未建立有效的 API 连接或尚未选择预设/模型时尝试执行任务。
...
在 **简易** 模式下，工具栏显示一个 **预设选择器**，包含内置预设 **免费 (OpenRouter)**、**标准**、**高级** 和 **技术**。显示哪些预设取决于您在 [**设置** > **常规设置**](#general-settings) 中选择的 **提供商** —— 例如，只有当提供商为 OpenRouter 时，**免费 (OpenRouter)** 才会列出。如果 **提供商** 为 **Ollama**，工具栏将列出您本地安装的模型而非预设。

在 **高级** 模式下，**模型选择器** 允许您为当前任务选择使用的 AI 引擎。

![Model selector](../images/screenshots/zh-CN/preset-selector.png)
...
7. 在 **输出**中查看结果。
8. 如果需要复制结果，请使用复制按钮。
9. 可选地使用 **翻译…** 或单词替代来细化结果 — 请参见 [细化您的翻译](#refining-translation)。
...
- **粘贴时自动执行** 在您粘贴文本后立即运行翻译。
- **将结果自动复制到剪贴板** 在成功运行后自动复制结果。
- **输入时实时翻译** (⚠️ 这可能会增加使用成本) 在您输入时运行翻译。
- **超时 (毫秒)** 控制应用程序在运行实时翻译之前等待的时间。
...
在成功翻译后，**翻译…** 和版本下拉菜单出现在输出标题中，位于 **到：** 语言选择器旁边。您可以在此处细化结果：
...
- **输出描述 (例如: 转换, 总结等)**: 描述结果的简短词语。
- **温度 (0.0 → 1.0)**: 模型的行为；见下文。
- **询问目标语言**: 在运行提示时添加语言选择器。
...
当您在提示编辑器中使用 **生成提示**、**优化提示** 或 **翻译提示** 时，**简易** 模式会提供与“翻译”和“重写”相同的预设选择器；而 **高级** 模式则使用模型列表。
...
在 **简易** 模式下，模型选择通过工具栏中的预设和“常规设置”中的 **提供商** 完成；**模型** 选项卡会被隐藏。

<br/>

> ℹ️ **注意**<br/>
> 在网页版本中，每位用户都有自己的配置。AI 体验、提供商、已选模型或预设、语言、常规选项和转换提示词等设置均按用户存储。您所做的更改不会影响其他用户。
...
- **简易**（默认）：选择一个 **提供商**（OpenRouter、OpenAI、Anthropic、Google Gemini、DeepSeek、Groq、Mistral、xAI、Cerebras 或 Ollama）。云提供商使用工具栏中的内置预设。**Ollama** 则列出您机器上安装的模型而非预设。在简易模式下，**预设目录** 显示目录版本和上次更新时间；点击 **刷新预设目录** 可从项目仓库获取最新的预设列表（应用也会在后台定期自动检查更新）。
...
- **ENTER的行为** 选择`Enter`是运行任务还是插入新行。
- **粘贴时自动执行** 在您粘贴文本后开始翻译。
- **将结果自动复制到剪贴板** 自动复制成功的结果。
- **输入时实时翻译** (⚠️ 这可能会增加使用成本) 在您输入时翻译。
- **超时时间（毫秒）** 设置实时翻译的等待时间。
...
**配置备份**（仅限桌面应用程序和网络管理员）
- **在备份中包含使用数据** - 启用时，ZIP还包含执行历史和API调用数据。
- **备份配置** - 创建一个包含`transrewrt-config-backup-YYYY-MM-DD_HHMMSS.zip`（本地时间）的单个ZIP，内含`config.json`、`state.json`、可选的加密密钥、用户、偏好设置、自定义提示和使用数据（如果您选择了）。成功备份后，确认将显示保存的文件名。
- **从备份中恢复** - 首先打开**确认对话框**。在对话框中选择备份ZIP（**浏览** / 文件选择器或在支持的地方拖放），然后查看选项：
  - **恢复使用数据** - 从ZIP中导入使用/历史记录，当它是包含使用数据的备份时；如果您只想要设置和提示，请不选。
  - **在恢复之前清除旧的使用数据** - 在应用备份之前删除此安装上的现有使用/历史记录（可选；当您想要干净替换时使用）。
在网络或桌面版本中创建的备份可以在另一个版本中恢复。当在网络版本中恢复桌面备份时，数据将恢复到管理员用户。
...
![Settings Models tab](../images/screenshots/zh-CN/settings-general.png)
...
- 您已在工具栏中选择了一个 **预设**（简易）或 **模型**（高级）
- 在 **简易** 模式下，[**设置** > **常规设置**](#general-settings) 中已为 **提供商** 配置了有效的密钥（或 Ollama URL），并且该提供商至少有一个可用预设
...
- 选择不同的预设（简单）或模型（高级）
- 使用更短的输入
- 在[**设置** > **常规设置**](#general-settings)中关闭**输入时实时翻译**
- 对于简单任务使用免费模型（见[模型](#models)）
<br/>
