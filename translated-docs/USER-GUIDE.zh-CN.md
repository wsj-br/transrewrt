---
translation_last_updated: '2026-05-02T22:34:39.760Z'
...
- [常见问题](#common-issues)
  ...
  - [总成本与提供商账单不符](#total-cost-does-not-match-my-provider-bill)
...
- 一个 **模型** 是执行任务的 AI 引擎。模型会以 **提供商前缀** 列出（例如 `openrouter/…`、`openai/…`、`ollama/…`）。
- 一个 **API 密钥**（对于 Ollama 而言是 **基础 URL**）是应用程序连接该提供商的方式。

如果您使用的是 **桌面应用**，请在 [**设置** > **API 配置**](#api-config) 中为每个您使用的提供商添加密钥。如果仅使用 OpenRouter，请参阅下方的 [如何获取 API 密钥](#how-to-get-an-api-key-desktop-app)。如果您不想使用 API 密钥，可以安装 Ollama（来自 [ollama.com](https://ollama.com)）并使用本地模型，例如 `translategemma:4b`。
...
3. 如果您使用的是 **桌面应用**，请打开 [**设置** > **API 配置**](#api-config)，为至少一个提供商添加 API 密钥（例如 OpenRouter），然后点击 **测试** 以验证其是否有效。
4. 打开 [**设置** > **模型**](#models)，并将一个或多个模型添加到 **已选模型** 中。
5. 打开 [**设置** > **语言**](#languages)，如果希望常用语言优先显示，请设置您的 **常用语言**。
6. 进入 **翻译** 功能并运行一次简单的翻译任务，确认一切正常。
7. 成功后，尝试使用 **重写** 和 **转换** 功能。

此顺序很重要。它可以避免最常见的首次使用问题：在应用尚未建立有效的 API 连接或未选择模型时就尝试执行任务。
...
如果您的提示列表为空，请在转换工作区中单击 **加载示例提示**。此控件始终可在[**设置** > **转换提示词**](#transform-prompts)的导出/导入行中找到。两者都会添加内置示例，以便您快速开始。
...
> ℹ️ **注意**<br/>
> 您可以在[**设置** > **转换提示词**](#transform-prompts)中导出和导入已保存的提示。
...
> ℹ️ **注意**<br/>
> 如果您仅使用**免费**模型，**费用**金额可能为零，且以成本为重点的摘要可能显示为空。在**摘要**中，只要所选时间段内有活动，**使用情况随时间变化**和**按模型使用情况**仍会显示**调用次数**（翻译、重写和转换）。
...
- **摘要**提供使用情况和费用的概览。包括**使用情况随时间变化**（按天累计堆叠的翻译、重写和转换的**调用次数**）和**按模型使用情况**（每个模型的总**调用次数**，包括转换）。
- **按使用量**按翻译语言、重写模式和转换提示词分解活动。
- **按模型**显示您使用的模型及其费用。
- **按天**显示每日总计。
...
> ℹ️ **注意**<br/>
> 在网页版本中，每个用户都有自己的配置。所选模型、语言、常规选项和转换提示词等设置均按用户存储。您所做的更改不会影响其他用户。
...
- **保留执行历史** 控制每次翻译、重写和转换是否将 **输入和输出文本** 保存到侧边栏的 [**历史记录**](#history) 视图中。关闭此功能会提示确认；若确认，已存储的历史文本将从数据库中删除。
- **删除历史数据** 允许您按时间（例如几个月前的数据，或 **所有数据（清除）**）使用 **删除数据** 功能移除已存储的文本。此操作仅影响 **历史记录** 视图中保存的执行文本；**不会** 删除费用或使用总量。如需删除或清理 **费用** 数据，请使用 [**设置** > **成本跟踪**](#cost-tracking)。

**外观**

- **在操作中显示成本信息** 控制是否显示每次操作的成本（如可用）以及翻译、重写和转换输出面板上的总成本。
- **费用小数位数** 更改费用小数的显示方式。
...
- 您仅使用 **免费模型**，并且正在查看 **费用** 数据（这些数据可能为零）；**摘要** 中的 **使用量** 调用次数图表仍需所选时间段内的数据
- 所选的 **时间筛选器** 未覆盖调用发生的时间段——请尝试 **全部** 进行检查

如果在选择 **全部** 后图表仍然为空，请确认调用是否出现在 [**历史记录**](#history) 或 **所有调用** 选项卡中。
...
**保留执行历史** 可能已关闭。请打开 [**设置** > **常规设置**](#general-settings) 并启用该选项。请注意，启用后无法恢复之前已删除的历史数据。
...
- 使用 [**历史记录](#history) 查看以往的操作及其完整的输入/输出文本。
- 如果您正在构建希望长期保存的提示词库（参见 [转换提示词](#transform-prompts)），或打算与他人共享，请定期导出提示词。
