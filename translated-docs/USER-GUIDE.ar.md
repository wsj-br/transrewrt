---
translation_last_updated: '2026-05-27T10:05:50.252Z'
...
بشكل افتراضي، يعمل التطبيق في وضع **سهل**: تختار **إعدادًا مسبقًا** (مثل مجاني (OpenRouter)، أو Lite، أو تقني) و**مزودًا** في الإعدادات، دون اختيار معرفات النماذج. قم بالتبديل إلى **متقدم** في [**الإعدادات** > **الإعدادات العامة**](#general-settings) إذا كنت ترغب في قائمة النماذج الكلاسيكية من [**الإعدادات** > **النماذج**](#models).
...
- في الوضع **سهل**، يكون **الإعداد المسبق** عبارة عن إعداد مسبق (مجاني (OpenRouter)، Lite، متقدم، أو تقني) يتوافق مع نموذج لمزود تختاره **(المزود)** (مثل OpenRouter، OpenAI، Ollama، وآخرين). لا تظهر في الشريط الجانبي إلا المهارات التي لها توافق مع المزود الحالي. وتقوم باختيار المهارة عند الترجمة أو إعادة الصياغة أو التحويل.
- في الوضع **متقدم**، يكون **النموذج** هو محرك الذكاء الاصطناعي الذي تختاره مباشرة. وتستخدم معرفات النماذج **بادئة المزود** (مثلاً `openrouter/…`، `openai/…`، `ollama/…`).
- يُستخدم **مفتاح API** (أو بالنسبة لـ Ollama، **عنوان URL الأساسي**) للوصول إلى هذا المزود من خلال التطبيق.
...
- **سهل** (الافتراضي): اختر **مزودًا** (OpenRouter، OpenAI، Anthropic، Google Gemini، DeepSeek، Groq، Mistral، xAI، Cerebras، أو Ollama). يستخدم المزودون السحابيون الإعدادات المسبقة المدمجة في شريط الأدوات. أما **Ollama** فيعرض النماذج المثبتة على جهازك بدلًا من الإعدادات المسبقة. في الوضع السهل، تعرض **كتالوج الإعدادات المسبقة** إصدار الكتالوج ووقت آخر تحديث؛ انقر على **تحديث كتالوج الإعدادات المسبقة** لجلب أحدث قائمة مهارات من مستودع المشروع (كما يقوم التطبيق أيضًا بالتحقق بشكل دوري في الخلفية).
- **متقدم**: اختر نماذج فردية في شريط الأدوات؛ وادِر القائمة من خلال [**الإعدادات** > **النماذج**](#models).
...
- اختر إعدادًا مسبقًا مختلفًا (سهل) أو نموذجًا (متقدم)
- استخدم مدخلًا أقصر
- قم بإيقاف **الترجمة الفورية (أثناء الكتابة)** في [**الإعدادات** > **الإعدادات العامة**](#general-settings)
