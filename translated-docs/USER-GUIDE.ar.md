---
translation_last_updated: '2026-05-24T17:52:57.273Z'
...
بشكل افتراضي، يعمل التطبيق في وضع **سهل**: حيث تختار **مهارة** (مثل مجاني (OpenRouter)، أو Lite، أو تقنية) و**مزودًا** في الإعدادات، دون اختيار معرفات النماذج. قم بالتبديل إلى **متقدم** في [**الإعدادات** > **الإعدادات العامة**](#general-settings) إذا كنت ترغب في قائمة النماذج الكلاسيكية من [**الإعدادات** > **النماذج**](#models).
...
- في الوضع **السهل**، تكون **المهارة** عبارة عن إعداد مسبق (مجاني (OpenRouter)، Lite، متقدم، أو تقني) يتم ربطه بنموذج لمزودك المختار **(المزود)** (OpenRouter، OpenAI، Ollama، وآخرين). لا تظهر في شريط الأدوات إلا المهارات التي لها رابط بالمزود الحالي. وتقوم باختيار المهارة عند الترجمة أو إعادة الصياغة أو التحويل.
- في الوضع **المتقدم**، يكون **النموذج** هو محرك الذكاء الاصطناعي الذي تختاره مباشرة. وتستخدم معرفات النماذج **بادئة المزود** (على سبيل المثال `openrouter/…`، `openai/…`، `ollama/…`).
- يتم استخدام **مفتاح API** (أو بالنسبة لـ Ollama، **عنوان URL الأساسي**) للوصول إلى هذا المزود من خلال التطبيق.
...
في وضع **سهل**، يعرض شريط الأدوات **محدد مهارة** مع الإعدادات المسبقة المدمجة **مجاني (OpenRouter)**، **Lite**، **متقدم**، و**تقني**. تعتمد المهارات الظاهرة على **المزود** الذي اخترته في [**الإعدادات** > **الإعدادات العامة**](#general-settings) — على سبيل المثال، تظهر **مجاني (OpenRouter)** فقط عندما يكون المزود هو OpenRouter. إذا كان **المزود** هو **Ollama**، فسيعرض شريط الأدوات النماذج المحلية المثبتة على جهازك بدلًا من المهارات.
...
- يتحكم **السلوك لـ ENTER** في ما يحدث عند الضغط على `Enter`:
  - يؤدي الضغط على **Enter** إلى تشغيل الترجمة أو إعادة الصياغة (الافتراضي).
  - يؤدي الضغط على **Shift + Enter** إلى تشغيل الترجمة أو إعادة الصياغة؛ بينما يؤدي الضغط العادي على **Enter** إلى إدراج سطر جديد.
...
3. وصف ما تريد أن يقوم به الموجه.
4. اختر مهارة (سهل) أو نموذجًا (متقدم).
...
- **سهل** (الافتراضي): اختر **مزودًا** (OpenRouter، OpenAI، Anthropic، Google Gemini، DeepSeek، Groq، Mistral، xAI، Cerebras، أو Ollama). يستخدم مقدمو الخدمات السحابية الإعدادات المسبقة المدمجة للمهارات في شريط الأدوات. أما **Ollama** فيسرد النماذج المثبتة على جهازك بدلًا من المهارات. في الوضع السهل، تعرض **مُخطط المهارات** إصدار المخطط ووقت آخر تحديث؛ انقر على **تحديث مُخطط المهارات** لجلب أحدث قائمة مهارات من مستودع المشروع (كما يقوم التطبيق أيضًا بالتحقق بشكل دوري في الخلفية).
...
**النسخ الاحتياطي للإعدادات** (مخصص لتطبيق سطح المكتب ومديري الويب فقط)
...
- اختيار مهارة مختلفة (سهل) أو نموذج (متقدم)
- استخدام مدخل أقصر
- إيقاف **الترجمة الفورية (أثناء الكتابة)** في [**الإعدادات** > **الإعدادات العامة**](#general-settings)
- استخدام النماذج المجانية للمهام البسيطة (انظر [النماذج](#models))
