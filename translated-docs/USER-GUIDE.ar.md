---
translation_last_updated: '2026-05-18T15:42:34.127Z'
...
بشكل افتراضي، يعمل التطبيق في وضع **سهل**: حيث تختار **مهارة** (مثل مجاني، سريع، أو تقني) و**مزودًا** في الإعدادات، دون اختيار معرفات النماذج. قم بالتبديل إلى وضع **متقدم** في [**الإعدادات** > **الإعدادات العامة**](#general-settings) إذا كنت ترغب في قائمة النماذج الكلاسيكية من [**الإعدادات** > **النماذج**](#models).
...
- [الإعدادات](#settings)
  ...
  - [تحويل (علامة التبويب الإعدادات)](#transform-settings)
  - [المستخدمون](#users)
...
- في وضع **سهل**، تكون **المهارة** إعدادًا مسبقًا (مجاني، سريع، متقدم، تقني، قانوني) يتوافق مع نموذج لمزود **المزود** الذي اخترته (OpenRouter، OpenAI، Ollama، وغيرهم). يمكنك اختيار المهارة من شريط الأدوات في ترجم، إعادة صياغة، وتحويل.
...
- مفتاح **واجهة برمجة التطبيقات (API)** (أو، بالنسبة لـ Ollama، **عنوان URL الأساسي**) هو الطريقة التي تصل بها التطبيق إلى ذلك المزود.

إذا كنت تستخدم **تطبيق سطح المكتب**، أضف المفاتيح في [**الإعدادات** > **إعدادات واجهة برمجة التطبيقات (API)**](#api-config) لكل مزود تستخدمه. إذا كنت تستخدم OpenRouter فقط، فانظر [كيفية الحصول على مفتاح API مجاني من OpenRouter](#how-to-get-an-api-key-desktop-app) أدناه. إذا كنت لا ترغب في استخدام مفتاح API، يمكنك تثبيت Ollama (من [ollama.com](https://ollama.com)) واستخدام نماذج محلية بدلًا من ذلك، مثل `translategemma:4b`.
...
5. في خيار **ترجم**، اختر **مهارة** (سهل) أو **نموذجًا** (متقدم) من شريط الأدوات.
6. افتح [**الإعدادات** > **اللغات**](#languages) واختر **أهم لغاتك** إذا كنت ترغب في ظهور لغاتك الأكثر استخدامًا أولاً.
7. قم بإجراء ترجمة بسيطة للتأكد من أن كل شيء يعمل، ثم جرّب خيارات **إعادة الصياغة** و**التحويل**.

يهم هذا الترتيب. فهو يمنع أكثر مشكلة شائعة عند أول استخدام: محاولة تنفيذ مهمة قبل أن يكون للتطبيق اتصال نشط بواجهة برمجة التطبيقات أو قبل اختيار مهارة/نموذج.
...
- على اليمين، يعرض **محدد المهارة أو النموذج** و**عنصر التحكم في لغة الواجهة**.

في وضع **سهل**، يعرض شريط الأدوات **محدد مهارة** (مثل مجاني، سريع، متقدم، تقني، قانوني، وإعدادات مسبقة مشابهة). تعتمد المهارات على **المزود** الذي اخترته في [**الإعدادات** > **الإعدادات العامة**](#general-settings). إذا كان **المزود** هو **Ollama**، فسيعرض شريط الأدوات النماذج المحلية المثبتة لديك بدلًا من المهارات.
...
2. اختر لغة في **من**.
3. اختر لغة في **إلى**.
4. اختر مهارة (سهل) أو نموذجًا (متقدم) في شريط الأدوات.
...
عند استخدامك لـ **توليد الموجه** أو **تحسين الموجه** أو **ترجم الأمر** في محرر الأوامر، يوفر الوضع **سهل** نفس محدد المهارة الموجود في الترجمة وإعادة الصياغة؛ بينما يستخدم الوضع **متقدم** قائمة النماذج.
...
في الوضع **سهل**، يتم اختيار النموذج من خلال المهارات في شريط الأدوات و**المزود** في الإعدادات العامة؛ بينما تكون علامة التبويب **النماذج** مخفية.

<br/>

> ℹ️ **ملاحظة**<br/>
> في النسخة الويب، يمتلك كل مستخدم تكوينه الخاص. يتم تخزين الإعدادات مثل تجربة الذكاء الاصطناعي، المزود، النماذج أو المهارات المحددة، اللغات، الخيارات العامة، وأوامر التحويل لكل مستخدم. التغييرات التي تقوم بها لا تؤثر على المستخدمين الآخرين.
...
- **سهل** (افتراضي): اختر **مزوّدًا** (OpenRouter، OpenAI، Anthropic، Google Gemini، DeepSeek، Groq، Mistral، xAI، Cerebras، أو Ollama). تستخدم موفرو الخدمات السحابية إعدادات المهارات المضمنة في شريط الأدوات. يعرض **Ollama** النماذج المثبتة على جهازك بدلًا من المهارات.
- **متقدم**: اختر نماذج فردية في شريط الأدوات؛ وقم بإدارة القائمة ضمن [**الإعدادات** > **النماذج**](#models).

على تطبيق الويب **web app**، تعتمد موفرو الخدمة الظاهرون على مفاتيح واجهة برمجة التطبيقات (API) المُعدَّة في بيئة الخادم. أما على تطبيق سطح المكتب **desktop app**، فيمكنك تهيئة المفاتيح ضمن [**API Config**](#api-config).
...
- قمت بتحديد **مهارة** (سهل) أو **نموذج** (متقدم) في شريط الأدوات
- في الوضع **سهل**، يجب أن يكون هناك **مزود** مُعدّ في [**الإعدادات** > **الإعدادات العامة**](#general-settings) مع مفتاح صالح (أو عنوان URL لـ Ollama) ومهارة واحدة على الأقل لهذا المزود
- في الوضع **متقدم**، يجب أن يكون هناك نموذج واحد على الأقل مدرج في [**الإعدادات** > **النماذج**](#models)
- إعداد واجهة برمجة التطبيقات (API) الخاص بك يعمل بشكل صحيح
