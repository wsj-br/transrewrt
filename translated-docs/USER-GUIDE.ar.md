![لافتة Transrewrt](../images/transrewrt_banner.png)
...
- [الترجمة](#translate)
  ...
  - [تحسين ترجمتك](#refining-your-translation)
  - [استخدام المسرد](#using-the-glossary)
- [إعادة كتابة](#rewrite)
  - [إعادة كتابة النص](#rewrite-text)
  - [تحسين إعادة الكتابة الخاصة بك](#refining-your-rewrite)
- [تحويل](#transform)
...
- [الإعدادات](#settings)
  ...
  - [تحويل (علامة تبويب الإعدادات)](#transform-settings-tab)
  - [المسرد (علامة تبويب الإعدادات)](#glossary-settings-tab)
  - [المستخدمون](#users)
  - [تكوين API](#api-config)
  - [حول](#about)
- [مشكلات شائعة](#common-issues)
  - [التطبيق لن يترجم أو يعيد كتابة أو يحول النص](#the-app-will-not-translate-rewrite-or-transform-text)
  ...
  - [تطبيق الويب: تم إعادة التوجيه إلى صفحة تسجيل الدخول بشكل غير متوقع](#web-app-redirected-to-the-login-page-unexpectedly)
  - [مسؤول الويب: نسيت أو فقدت كلمة مرور](#web-admin-forgot-or-lost-a-password)
  - [لوحة القيادة لا تظهر بيانات للمستخدمين الآخرين (الويب)](#dashboard-shows-no-data-for-other-users-web)
...
![محدد النموذج](../images/screenshots/ar/preset-selector.png)
...
1. **إعادة صياغة…** — بدون تحديد نص في الإخراج، احصل على ترجمة كاملة أخرى لنفس الإدخال بكلمات مختلفة. يتلقى النموذج كل إصدار لديك بالفعل حتى يمكن أن تختلف الكلمات الجديدة عن جميعها. يمكنك تخزين ما يصل إلى **خمسة** إصدارات والتبديل بينها في القائمة المنسدلة للإصدار. مع تحديد النص، يفتح **إعادة صياغة…** بدائل الكلمات بالقرب من التحديد (مثل النقر بزر الماوس الأيمن). بدون تحديد، يتم تعطيل **إعادة صياغة…** بمجرد وصولك إلى خمسة إصدارات؛ مع التحديد، لا يزال يعمل عند خمسة إصدارات (بدائل الكلمات فقط، تحديث الإصدار 5). أثناء تشغيل إعادة صياغة كاملة، انقر فوق **إيقاف الترجمة** للإلغاء؛ يعود الإخراج إلى الإصدار الذي كان نشطًا عند بدء إعادة الصياغة.
2. **بدائل الكلمات** — حدد كلمة واحدة أو أكثر أو عبارة قصيرة في الإخراج (إذا حددت جزءًا فقط من كلمة، يقوم التطبيق بتوسيع التحديد ليشمل الكلمات الكاملة)، ثم انقر بزر الماوس الأيمن أو انقر فوق **إعادة صياغة…**. تظهر قائمة قصيرة من البدائل بالقرب من التحديد؛ انقر فوق أحدها لاستبداله. قد يحل كل خيار محل نطاق أوسع قليلاً من التحديد الخاص بك (على سبيل المثال حرف جر أو أداة تعريف مجاورة) بحيث تظل الجملة نحوية. إذا كان لديك أقل من خمسة إصدارات، يتم حفظ الإخراج المحرر كإصدار جديد؛ عند خمسة إصدارات، يتم تحديث **الإصدار 5** فقط. يؤدي النقر بزر الماوس الأيمن بدون تحديد إلى تحديد الكلمة تحت المؤشر (أو لا يفعل شيئًا إذا لم تكن هناك كلمة هناك). اضغط على **Esc** أو انقر خارج القائمة للإلغاء دون تغيير الإخراج.
3. **التكاليف** — كل **إعادة صياغة…** كاملة (بدون تحديد) وكل طلب بديل للكلمة يستخدم النموذج مرة أخرى وقد يزيد من تكلفة الاستخدام (مثل تشغيل ترجمة عادي).
...
**المسرد** هو قائمة بأزواج مصطلحات المصدر/الهدف لزوج لغات معين. عندما يكون المسرد نشطًا، يرسل Transrewrt المصطلحات المتطابقة إلى النموذج حتى تظل صياغتك المفضلة متسقة عبر الترجمات (على سبيل المثال، اسم منتج، مصطلح علامة تجارية، أو مسمى وظيفي يجب ترجمته دائمًا بنفس الطريقة).

لاستخدامه في صفحة **الترجمة**:

1. قم بتشغيل مفتاح **المسرد** في لوحة الإدخال (بجانب مفاتيح التنفيذ التلقائي والنسخ التلقائي).
2. اختر لغات **المصدر** و **الهدف** وقم بالترجمة كالمعتاد. يتم تطبيق المصطلحات المحفوظة لزوج اللغات هذا تلقائيًا.
3. لالتقاط زوج جديد أثناء التنقل، انقر فوق **إضافة إلى المسرد** (بجانب محدد لغة **من:**). يتم ملء الحوار مسبقًا بلغتك الحالية، لذا ما عليك سوى ملء **مصطلح المصدر** و **مصطلح الهدف**.
4. استخدم رابط **المسرد** في تذييل الإخراج (أو رابط **إدارة المسرد** داخل الحوار) للانتقال إلى [**الإعدادات** > **المسرد**](#glossary-settings) ومراجعة جميع مصطلحاتك.

تقوم بإضافة وتحرير واستيراد وتصدير المصطلحات في علامة التبويب [**الإعدادات** > **المسرد**](#glossary-settings) — انظر أدناه.

<br/>

> ℹ️ **ملاحظة**<br/>
> تتم مطابقة مصطلحات المسرد حسب **زوج اللغات**، لذا فإن المصطلح المحفوظ للغة الإنجليزية → الفرنسية لا يتم تطبيقه عند الترجمة من الإنجليزية → الألمانية. لا يمكن استخدام المسرد مع **اكتشاف اللغة** كمصدر، لأن لغة مصدر محددة مطلوبة لمطابقة المصطلحات.
...
استخدم **إعادة كتابة** عندما تريد تحسين الصياغة دون تغيير المعنى الرئيسي. يبقى النص بنفس اللغة (لا تتم ترجمته).
...
1. افتح **إعادة كتابة**.
2. اختر **الوضع** (على سبيل المثال **تحسين الوضوح** أو **جعله رسميًا**).
3. اختياريًا، اضبط **من** إلى لغة النص الخاص بك (أو اترك **اكتشاف اللغة**).
4. اكتب أو الصق النص في **إدخال**.
5. انقر فوق **إعادة كتابة**.
6. اقرأ النتيجة في **إخراج**.
7. اختياريًا، قم بتحسين النتيجة باستخدام **إعادة الصياغة…** أو بدائل الكلمات — راجع [تحسين إعادة الكتابة](#refining-rewrite).
...
> ℹ️ **ملاحظة**<br/>
> يعرض وضع إعادة الكتابة **إصدارات بديلة** عدة صيغ في تشغيل **واحد**، مفصولة بـ `----` في الإخراج. يختلف هذا عن **إعادة الصياغة…**، الذي يبني سجل إصدارات بمرور الوقت (متغير جديد واحد لكل نقرة). راجع [تحسين إعادة الكتابة](#refining-rewrite).
...
بعد إعادة الكتابة الناجحة، يظهر **إعادة الصياغة…** والقائمة المنسدلة للإصدارات على جانب الإخراج من مساحة العمل (في التخطيط المقسم، في شريط الأدوات العلوي فوق عمود الإخراج، بجوار مقاييس التشغيل؛ في التخطيط المكدس، فوق لوحة الإخراج بجوار **من:**). يمكنك تحسين النتيجة هناك — نفس فكرة [تحسين الترجمة](#refining-translation)، ولكن النص يبقى بنفس اللغة ويحتفظ بـ **وضع** إعادة الكتابة الحالي:

1. **إعادة الصياغة…** — بدون تحديد نص في الإخراج، احصل على إعادة كتابة كاملة أخرى لنفس الإدخال بصياغة مختلفة، مع الاستمرار في تطبيق الوضع المحدد (على سبيل المثال أوضح، أقصر، أو أكثر رسمية). يتلقى النموذج كل إصدار لديك بالفعل حتى يمكن أن تختلف الصياغة الجديدة عن جميعها. يمكنك تخزين ما يصل إلى **خمسة** إصدارات والتبديل بينها في القائمة المنسدلة للإصدارات. مع تحديد نص، يفتح **إعادة الصياغة…** بدائل الكلمات بالقرب من التحديد (نفس النقر بزر الماوس الأيمن). بدون تحديد، يتم تعطيل **إعادة الصياغة…** بمجرد وصولك إلى خمسة إصدارات؛ مع تحديد، لا يزال يعمل عند خمسة إصدارات (بدائل الكلمات فقط، تحديث الإصدار 5). أثناء تشغيل إعادة صياغة كاملة، انقر فوق **إيقاف إعادة الكتابة** للإلغاء؛ يعود الإخراج إلى الإصدار الذي كان نشطًا عند بدء إعادة الصياغة.
2. **بدائل الكلمات** — حدد كلمة واحدة أو أكثر أو عبارة قصيرة في الإخراج (إذا حددت جزءًا فقط من كلمة، يقوم التطبيق بتوسيع التحديد ليشمل الكلمات الكاملة)، ثم انقر بزر الماوس الأيمن أو انقر فوق **إعادة الصياغة…**. تظهر قائمة قصيرة من البدائل بالقرب من التحديد؛ انقر فوق أحدها لاستبداله. قد يستبدل كل خيار نطاقًا أوسع قليلاً من تحديدك حتى تظل الجملة نحوية. إذا كان لديك أقل من خمسة إصدارات، يتم حفظ الإخراج المحرر كإصدار جديد؛ عند خمسة إصدارات، يتم تحديث **الإصدار 5** فقط. النقر بزر الماوس الأيمن بدون تحديد يحدد الكلمة تحت المؤشر (أو لا يفعل شيئًا إذا لم تكن هناك كلمة هناك). اضغط على **Esc** أو انقر خارج القائمة للإلغاء دون تغيير الإخراج.
3. **التكاليف** — كل **إعادة صياغة…** كاملة (بدون تحديد) وكل طلب بديل للكلمة يستخدم النموذج مرة أخرى وقد يزيد من تكلفة الاستخدام (نفس تشغيل إعادة الكتابة العادي).
...
![تحويل مساحة العمل](../images/screenshots/ar/transform.png)
...
![توليد النص المطلوب](../images/screenshots/ar/transform-generate.png)
...
![محرر تحويل النص](../images/screenshots/ar/transform-prompt-edit.png)
...
![ملخص لوحة القيادة](../images/screenshots/ar/dashboard-summary.png)
...
| علامة التبويب | سطح المكتب | الويب (المسؤول) | الويب (المستخدم العادي) | ملاحظات                                        |
  ...
  | المسرد         |   نعم   |     نعم     |        نعم         | أزواج المصطلحات المطبقة أثناء الترجمة        |
  | المستخدمون             |    -    |     نعم     |         -          | |
...
### المسرد (علامة تبويب الإعدادات)

استخدم **الإعدادات** > **المسرد** لإدارة أزواج المصطلحات المطبقة أثناء الترجمة (راجع [استخدام المسرد](#using-the-glossary)). لكل مصطلح **لغة مصدر** و **لغة هدف** و **مصطلح مصدر** و **مصطلح هدف**.

يمكنك:

- **إضافة مصطلح** — املأ الصف في أسفل الجدول (اختر اللغات، اكتب المصطلحات المصدر والهدف) وانقر على الزر **+**.
- **البحث عن مصطلحات** — قم بتصفية القائمة حسب **لغة المصدر** أو **لغة الهدف** أو **نص** حر؛ انقر على **مسح عوامل التصفية** لإعادة التعيين.
- **حذف مصطلح** — انقر على أيقونة سلة المهملات في صفه.
- **استيراد** — تحميل المصطلحات من ملف `.csv` أو `.xlsx` أو `.xls`. يجب أن يحتوي الملف على الأعمدة `source_language` و `target_language` و `source_text` و `target_text`.
- **تصدير CSV** / **تصدير XLSX** — قم بتنزيل كل مصطلحاتك للنسخ الاحتياطي أو المشاركة.
- **قالب CSV** / **قالب XLSX** — قم بتنزيل ملف فارغ مع رؤوس الأعمدة الصحيحة لملئها واستيرادها.

<br/>

> ℹ️ **ملاحظة**<br/>
> في **تطبيق سطح المكتب**، يتم تخزين المسرد محليًا. في **إصدار الويب**، لكل مستخدم مسرده الخاص، لذا فإن مصطلحاتك لا تؤثر على المستخدمين الآخرين.
...
يتم تكوين مفاتيح API من خلال متغيرات بيئة النظام أو Docker - لا يتم إدخالها في واجهة المستخدم الرسومية. بالنسبة للمزود المخصص، قم بتعيين `CUSTOM_PROVIDER_NAME` و `CUSTOM_PROVIDER_URL` و `CUSTOM_PROVIDER_API_KEY` (جميعها مطلوبة). تعرض هذه الصفحة المزودين الذين لديهم مفتاح تم تكوينه وتسمح لك باختبار كل واحد منهم بالنقر فوق الزر `Test`.
...
> 💡 **تلميح** <br/>
> إذا كنت لا ترغب في استخدام مفتاح API أو الدفع مقابل الاستخدام، يمكنك [تنزيل Ollama](https://ollama.com) وتشغيل النماذج (مثل `translategemma:4b`) محليًا على جهازك مجانًا. بدلاً من ذلك، يمكنك إنشاء حساب OpenRouter مجاني (لا يلزم بطاقة ائتمان) لاستخدام نماذجهم المجانية، أو الحصول على مفتاح API مجاني من Cerebras أو Google أو Groq أو Mistral AI أو [NVIDIA](https://build.nvidia.com/).
