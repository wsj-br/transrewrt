...
- [Configuración](#settings)
  ...
  - [Glosario (pestaña de configuración)](#glossary-settings-tab)
  - [Usuarios](#users)
...
Para usar Transrewrt, necesitas acceso a al menos un proveedor de IA. Los proveedores compatibles son: [OpenRouter](https://openrouter.ai) (que agrega muchos modelos), OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras, NVIDIA, Alibaba Cloud, apikey.fun, cualquier proveedor compatible con OpenAI y [Ollama](https://ollama.com) para modelos locales.

No es necesario seleccionar un modelo de pago para empezar. Tan pronto como agregues tu clave API de OpenRouter, la aplicación habilitará automáticamente una opción **gratuita** incorporada de OpenRouter. Esto te permite empezar a traducir, reescribir y transformar texto de inmediato. Alternativamente, también puedes obtener una clave API gratuita de Cerebras, Google, Groq, Mistral AI o [NVIDIA](https://build.nvidia.com/) (API compatible con OpenAI).
...
Un **glosario** es una lista de pares de términos de origen/destino para un par de idiomas específico. Cuando el glosario está activado, Transrewrt envía los términos coincidentes al modelo para que tu redacción preferida se mantenga consistente en las traducciones (por ejemplo, un nombre de producto, un término de marca o un título de trabajo que siempre debe traducirse de la misma manera).

Para usarlo en la página **Traducir**:

1. Activa el interruptor **Glosario** en el panel de entrada (junto a los interruptores de ejecución automática y copia automática).
2. Elige tus idiomas de **Origen** y **Destino** y traduce como de costumbre. Los términos guardados para ese par de idiomas se aplican automáticamente.
3. Para capturar un nuevo par sobre la marcha, haz clic en **Añadir al glosario** (junto al selector de idioma **De:**). El diálogo se rellena previamente con tus idiomas actuales, por lo que solo necesitas completar el **término de origen** y el **término de destino**.
4. Usa el enlace **Glosario** en el pie de página de salida (o el enlace **Gestionar glosario** dentro del diálogo) para ir a [**Configuración** > **Glosario**](#glossary-settings) y revisar todos tus términos.

Añades, editas, importas y exportas términos en la pestaña [**Configuración** > **Glosario**](#glossary-settings), consulta más abajo.

<br/>

> ℹ️ **NOTA**<br/>
> Los términos del glosario se emparejan por **par de idiomas**, por lo que un término guardado para inglés → francés no se aplica al traducir inglés → alemán. El glosario no se puede usar con **Detectar idioma** como origen, porque se necesita un idioma de origen específico para emparejar los términos.
...
| Pestaña              | Escritorio | Web (administrador) | Web (usuario normal) | Notas                                        |
  ...
  | Glosario         |   sí   |     sí     |        sí         | Pares de términos aplicados durante la traducción        |
  | Usuarios            |    -    |     sí     |         -          |                                              |
...
### Glosario (pestaña de configuración)

Use **Configuración** > **Glosario** para administrar los pares de términos aplicados durante la traducción (consulte [Uso del glosario](#using-the-glossary)). Cada término tiene un **idioma de origen**, un **idioma de destino**, un **término de origen** y un **término de destino**.

Puede:

- **Agregar un término** — complete la fila en la parte inferior de la tabla (seleccione los idiomas, escriba los términos de origen y destino) y haga clic en el botón **+**.
- **Buscar términos** — filtre la lista por **Idioma de origen**, **Idioma de destino** o **texto** libre; haga clic en **Borrar filtros** para restablecer.
- **Eliminar un término** — haga clic en el icono de la papelera en su fila.
- **Importar** — cargue términos desde un archivo `.csv`, `.xlsx` o `.xls`. El archivo debe tener las columnas `source_language`, `target_language`, `source_text` y `target_text`.
- **Exportar CSV** / **Exportar XLSX** — descargue todos sus términos para hacer una copia de seguridad o compartirlos.
- **Plantilla CSV** / **Plantilla XLSX** — descargue un archivo vacío con los encabezados de columna correctos para completar e importar.

<br/>

> ℹ️ **NOTA**<br/>
> En la **aplicación de escritorio**, el glosario se almacena localmente. En la **versión web**, cada usuario tiene su propio glosario, por lo que sus términos no afectan a otros usuarios.
...
Los proveedores compatibles son: OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras, NVIDIA, Alibaba Cloud, apikey.fun, **Ollama** (modelos locales a través de una URL base) y un **proveedor personalizado compatible con OpenAI** opcional (nombre, URL y clave API — solo modo Avanzado). Solo necesitas configurar los proveedores que utilizas.

**Aplicación web: solo administrador**

Las claves API se configuran a través de variables de entorno del sistema o de Docker; no se introducen en la interfaz web. Para el proveedor personalizado, configura `CUSTOM_PROVIDER_NAME`, `CUSTOM_PROVIDER_URL` y `CUSTOM_PROVIDER_API_KEY` (los tres son necesarios). Esta página muestra qué proveedores tienen una clave configurada y te permite probar cada uno haciendo clic en el botón `Test`.
...
Usa **Configuración de API** para almacenar las claves API de cada proveedor que utilices. Para Ollama, introduce la **URL base** en lugar de una clave API. Para un proveedor personalizado compatible con OpenAI (cualquier punto final que no esté en la lista integrada, como un servidor o puerta de enlace autoalojado), introduce un **nombre de proveedor**, una **URL base** (como `https://my-llm.example.com/v1`) y una **clave API**; los tres son obligatorios. La URL y el nombre se editan en línea; usa **Editar** para reemplazar la clave API. Los modelos del proveedor personalizado aparecen solo en modo **Avanzado** (Configuración → Modelos).

<br/>

> 💡 **Consejo** <br/>
> Si no deseas utilizar una clave API ni pagar por el uso, puedes [descargar Ollama](https://ollama.com) y ejecutar modelos (como `translategemma:4b`) localmente en tu máquina de forma gratuita. Alternativamente, puedes crear una cuenta gratuita de OpenRouter (no se requiere tarjeta de crédito) para usar sus modelos gratuitos, u obtener una clave API gratuita de Cerebras, Google, Groq, Mistral AI o [NVIDIA](https://build.nvidia.com/).

<br/>

- Agrega solo los proveedores que necesites. En **Configuración** > **Modelos**, el ID de cada modelo comienza con el proveedor (por ejemplo, `openrouter/openrouter/free`, `openai/gpt-4o`, `nvidia/nvidia/nemotron-nano-3-30b-a3b`, `ollama/llama3`, `MyProvider/…` para un punto final personalizado llamado `MyProvider`).
