---
translated_at: "2026-03-29T01:53:03.268Z"
...
ER-GUIDE.ms.md) · [Persian](USER-GUIDE.fa.md) · [Polish](USER-GUIDE.pl.md) · [Portuguese (PT)](USER-GUIDE.pt.md) · [Punjabi](USER-GUIDE.pa.md) · [Romanian](USER-GUIDE.ro.md) · [Russian](USER-GUIDE.ru.md) · [Slovak](USER-GUIDE.sk.md) · [Spanish](USER-GUIDE.es.md) · [Swahili](USER-GUIDE.sw.md) · [Swedish](USER-GUIDE.sv.md) · [Telugu](USER-GUIDE.te.md) · [Thai](USER-GUIDE.th.md) · [Turkish](USER-GUIDE.tr.md) · [Ukrainian](USER-GUIDE.uk.md) · [Vietnamese](USER-GUIDE.vi.md)</small>
...
Use the sidebar to navigate the app. You can collapse the sidebar to gain more space by clicking the icon next to the app logo.
  ...
  <tr>
    ...
    <td valign="top">
      ...
      <ul>
        ...
        <li><strong>History</strong> shows the usage history, including input and output text.</li><br/>
        <li><strong>User</strong> displays the logged-in user's username (web only).</li>
      </ul>
...
| **Input**                                                          | **Output**                                                                                                                  |
|--------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------|
| - Character count <br/>- Word count <br/>- Paragraph count         | - How long the task took<br/>- **TPS** (tokens per second)<br/>- Character, word, and paragraph counts<br/>- The model used |

If you're wondering about the technical terms:
...
4. Select a model from the toolbar.
5. Type or paste text into **Input**.
6. Click **Translate**.
7. View the result in **Output**.
8. Use the copy button if you'd like to copy the result.
...
Use **Rewrite** when you want to improve the wording without changing the main meaning.
...
- generating several distinct reformulations in one go (**Alternative versions**)
- making text more formal or informal (**Formal** / **Informal**)
...
> 💡 **TIP**<br/>
> When you use the "**Check Spelling & Grammar**" mode, a **Show changes** toggle appears in the output panel (next to **Copy**).
...
If your prompt list is empty, click **Load sample prompts** in the Transform workspace. The same option is always available in [**Settings** > **Transform Prompts**](#transform-prompts) on the export/import row. Both options add built-in examples so you can get started quickly.
...
- A **higher** temperature results in greater variety and creativity.
...
> ⚠️ **WARNING**<br/>
> Click **`Save`** before clicking **`Back to Run`**. If you navigate back without saving, your changes will be lost.
...
The test panel on the right allows you to try out your prompt with sample text before using it in your daily work.

This is helpful when:

- you are creating a new prompt
- you are comparing two versions of a prompt
- you want to verify tone, length, or output format
...
Use the **Dashboard** to see how much you're using the app and what it's costing (for paid models).
...
> ℹ️ **NOTE**<br/>
> If you only use **free** models, **cost** amounts may be zero and cost-focused summaries may appear empty. However, on **Summary**, **Usage over time**, and **Usage by model**, the **number of calls** (translate, rewrite, and transform) will still be displayed if there was activity during the selected period.
...
- **Summary** provides an overview of usage and cost. It includes **Usage over time** (a stacked cumulative chart of **call counts** per day for translate, rewrite, and transform functions) and **Usage by model** (total **calls per model**, including transform).
- **By Usage** breaks down activity by translation language, rewrite mode, and transform prompt.
- **By Model** displays the models you've used and their associated costs.
- **By Day** shows daily usage totals.
- **All Calls** displays the complete call history and allows you to export it.
...
This is helpful if you want to review activity outside the app or share a report.
...
To delete all data or remove records based on their age, go to [**Settings** > **Cost Tracking**](#cost-tracking). There, you'll find options to delete all stored data or only data older than a certain date.
...
Click on **History** to view the history of your actions within **Transrewrt**, including the input and output of each operation.
...
The history page allows you to export filtered data in the following formats:
...
Use **General Settings** to control typing behavior, whether execution details are stored in **History**, and appearance.
...
- **Keep execution history** controls whether each translate, rewrite, and transform stores **input and output text** for the sidebar [**History**](#history) view. Turning it off prompts for confirmation; if confirmed, stored history text is removed from the database.

- **Delete history data** allows you to remove stored text based on age (for example, older than a few months, or **all data (clear)**) using **Delete data**. This only affects saved execution text in the **History** view; it does **not** delete cost or usage totals. To remove or trim **cost** data, use [**Settings** > **Cost Tracking**](#cost-tracking).

**Appearance**

- **Show cost information on the actions** controls whether the cost per operation (if available) and the total cost are displayed on the Translate, Rewrite, and Transform output panels.
...
- **Font Family** changes the font used in the text panels.
- **Size** adjusts the font size.

**Configuration Backup**

- **Include usage data in the backup** — when enabled, the ZIP file will also include execution history and API call data.

- **Backup configuration** — creates a single ZIP file (`transrewrt-config-backup-YYYY-MM-DD_HHMMSS.zip` in UTC by default) containing `config.json`, `state.json`, optional encryption key, users, preferences, custom prompts, and usage data (if you opted in). After a successful backup, a confirmation message displays the saved file name.
- **Restore from backup** — opens a **confirmation dialog first**. Select the backup ZIP file within the dialog (**Browse** / file picker or drag-and-drop where supported), then review the options:
  - **Restore the usage data** — imports usage history from the ZIP if it was backed up with usage data included; leave this unchecked if you only want to restore settings and prompts.
  - **Clear the old usage data before restoring** — removes existing usage history from the current installation before applying the backup (optional; use this when you want a complete replacement).
...
> If you do not want to add credits to OpenRouter right away, start by enabling **Free Only** and selecting the free models (no credit card required). You can also use Ollama to run models locally without any API key.
...
Use **Settings** > **Languages** to manage the language lists used in the app.
...
When you add a custom language, it appears in the language selectors along with the built-in options.
...
API keys are configured through system or Docker environment variables — they are not entered in the web UI. This page shows which providers have a key configured and allows you to test each one by clicking the **`Test`** button.
...
- Only add the providers you need. In **Settings** > **Models**, each model ID begins with the provider (for example, `openrouter/openrouter/free`, `openai/gpt-4o`, `ollama/llama3`).
...
If something isn't working as expected, check the following points first.
...
- you only use **free models** and you are viewing **cost** figures (they may be zero); **usage** call-count charts on the **Summary** tab still require data from the selected time period
- the selected **time filter** does not include the period when calls were made — try selecting **All** to verify

If the charts remain empty after selecting **All**, confirm that calls appear in the [**History**](#history) or in the **All Calls** tab.
...
When you use models through **OpenRouter**, the app displays your actual spending as reported by OpenRouter.

For **other providers** (such as OpenAI direct, Anthropic direct, etc.), the cost is estimated using pricing data published by OpenRouter. If no matching price is found for a model, the cost will be shown as **not available** and will not be included in your running total.
...
To bring the total closer to your actual OpenRouter spend, open [**Settings** > **Cost Tracking**](#cost-tracking) and click **Sync with API key usage**.
...
**Keep execution history** might be disabled. Go to [**Settings** > **General Settings**](#general-settings) and enable it. Note that enabling this option won't restore any previously deleted history data.
...
Your session may have timed out. Log in again. If this occurs frequently, check the server configuration for session lifetime settings.
...
- If another administrator can still sign in, they can open [**Settings** > **Users**](#users), select the account, and set a **new password** there.
- If you are **locked out** but have **shell access** to the machine or container, reset the password using the helper included with the image (replace `transrewrt` if you changed the default name, and quote the password if it contains spaces or special characters):

```bash
docker exec transrewrt reset-web-password '<username>' '<new-password>'
```

The default admin username is `admin` if you haven't created other accounts. When only one argument is provided, it is treated as the new password for `admin`.

If you're running from a **source checkout** instead of Docker, use:
...
Only **administrators** can view data from all users via the **User** filter. Regular users can see only their own activity by design.
...
- Start with [**Translate**](#translate) to make sure your setup works before moving on to [**Rewrite**](#rewrite) or [**Transform**](#transform).
...
- Use [**Dashboard**](#dashboard) if you want to monitor usage and costs.
- Use [**History**](#history) to review past operations and their full input/output text.
- Export prompts regularly if you are building a prompt library that you want to keep safe (see [Transform Prompts](#transform-prompts)) or if you wish to share it with others.
