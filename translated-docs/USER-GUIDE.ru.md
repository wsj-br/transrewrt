...
- [Преобразование](#transform)
  - [Запуск существующей подсказки](#run-an-existing-prompt)
  - [Если у вас еще нет подсказок](#if-you-have-no-prompts-yet)
  - [Быстрое создание подсказки](#create-a-prompt-quickly)
  - [Изменение подсказки](#edit-a-prompt)
  - [Тестирование подсказки перед использованием](#test-a-prompt-before-using-it)
...
- [Распространенные проблемы](#common-issues)
  - [Приложение не переводит, не перезаписывает и не преобразует текст](#the-app-will-not-translate-rewrite-or-transform-text)
  ...
  - [Общая стоимость не совпадает с моим счетом от провайдера](#total-cost-does-not-match-my-provider-bill)
  - [Страница истории отсутствует на боковой панели](#the-history-page-is-missing-from-the-sidebar)
  ...
  - [На панели управления нет данных для других пользователей (веб)](#dashboard-shows-no-data-for-other-users-web)
  - [Я изменил подсказку и потерял изменения](#i-changed-a-prompt-and-lost-the-edits)
...
1. **Перефразировать…** — при отсутствии выделенного текста в выводе получите другой полный перевод того же ввода с другим wording. Модель получает каждую версию, которую вы уже имеете, так что новое wording может отличаться от всех них. Вы можете сохранить до **пяти** версий и переключаться между ними в выпадающем меню версий. При выделенном тексте **Перефразировать…** открывает альтернативы слов рядом с выделением (так же, как при щелчке правой кнопкой мыши). Без выделения **Перефразировать…** отключается, как только вы достигнете пяти версий; при наличии выделения он все еще работает на пяти версиях (только альтернативы слов, обновляя версию 5). Пока выполняется полное перефразирование, нажмите **Остановить перевод**, чтобы отменить; вывод возвращается к версии, которая была активна, когда началось перефразирование.
2. **Альтернативы слов** — выберите одно или несколько слов или короткую фразу в выводе (если вы выберете только часть слова, приложение расширит выделение до полных слов), затем щелкните правой кнопкой мыши или нажмите **Перефразировать…**. Краткий список альтернатив появляется рядом с выделением; нажмите одну, чтобы заменить ее. Каждая опция может заменить немного более широкий диапазон, чем ваше выделение (например, соседний предлог или артикль), чтобы предложение оставалось грамматически правильным. Если у вас меньше пяти версий, отредактированный вывод сохраняется как новая версия; при пяти версиях обновляется только **версия 5**. Щелчок правой кнопкой мыши без выделения ничего не делает. Нажмите **Esc** или щелкните вне списка, чтобы отменить без изменения вывода.
3. **Стоимость** — каждое полное **Перефразировать…** (без выделения) и каждый запрос альтернатив слов снова использует модель и может увеличить стоимость использования (так же, как обычный перевод).
...
Поддерживаемые провайдеры: OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras, **Ollama** (локальные модели через базовый URL) и необязательный **пользовательский OpenAI-совместимый провайдер** (имя, URL и ключ API — только в расширенном режиме). Вам нужно настроить только те провайдеры, которые вы используете.
...
Используйте **Конфигурация API** для хранения ключей API для каждого используемого вами провайдера. Для Ollama введите **базовый URL** вместо ключа API. Для пользовательского OpenAI-совместимого провайдера (например, [NVIDIA NIM](https://build.nvidia.com/)) введите **имя провайдера**, **базовый URL** (например, `https://integrate.api.nvidia.com/v1`) и **ключ API**; все три поля обязательны. URL и имя редактируются в строке; используйте **Изменить**, чтобы заменить ключ API. Модели пользовательского провайдера отображаются только в **Расширенном** режиме (Настройки → Модели).
...
- Добавляйте только те провайдеры, которые вам нужны. В **Настройки** > **Модели** каждый идентификатор модели начинается с имени провайдера (например, `openrouter/openrouter/free`, `openai/gpt-4o`, `ollama/llama3`, `NVIDIA/nvidia/nemotron-nano-3-30b-a3b` для пользовательского конечного узла с именем NVIDIA).
