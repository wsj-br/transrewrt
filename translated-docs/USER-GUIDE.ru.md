---
translation_last_updated: '2026-05-21T23:13:54.249Z'
...
По умолчанию приложение работает в режиме **Простой**: вы выбираете **навык** (например, Бесплатно (OpenRouter), Lite или Technical) и **поставщика** в разделе Настройки, не указывая идентификаторы моделей. Переключитесь на **Расширенный** режим в разделе [**Настройки** > **Основные настройки**](#general-settings), если хотите использовать классический список моделей из раздела [**Настройки** > **Модели**](#models).
...
- [Настройки](#settings)
  ...
  - [Преобразовать (вкладка настройки)](#transform-settings-tab)
  - [Пользователи](#users)
...
- В **простом** режиме **навык** — это предустановка (Бесплатно (OpenRouter), Lite, Расширенный или Technical), которая сопоставляется с моделью для выбранного вами **поставщика** (OpenRouter, OpenAI, Ollama и другие). В панели инструментов отображаются только те навыки, для которых есть сопоставление с текущим поставщиком. Вы выбираете навык при использовании функций Перевести, Переписать и Преобразовать.
- В **расширенном** режиме **модель** — это движок ИИ, который вы выбираете напрямую. Идентификаторы моделей используют **префикс поставщика** (например, `openrouter/…`, `openai/…`, `ollama/…`).
...
В **простом** режиме на панели инструментов отображается **выбор навыка** с встроенными предустановками: **Бесплатно (OpenRouter)**, **Lite**, **Расширенный** и **Technical**. Набор доступных навыков зависит от выбранного **поставщика** в разделе [**Настройки** > **Основные настройки**](#general-settings). Например, **Бесплатно (OpenRouter)** отображается только при выборе поставщика OpenRouter. Если выбран **поставщик** **Ollama**, на панели инструментов отображаются установленные локальные модели вместо навыков.
...
- **Поведение для ENTER** определяет, что происходит при нажатии `Enter`:
  - **Enter** запускает перевод или переписывание (по умолчанию).
  - **Shift + Enter** запускает перевод или переписывание; обычный **Enter** вставляет новую строку.
...
4. Выберите навык (Простой режим) или модель (Расширенный режим).
...
- **Простой** (по умолчанию): выберите **поставщика** (OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras или Ollama). Для облачных поставщиков используются встроенные предустановки навыков на панели инструментов. **Ollama** отображает модели, установленные на вашем компьютере, вместо навыков. В простом режиме раздел **Каталог навыков** показывает версию каталога и время последнего обновления; нажмите **Обновить каталог навыков**, чтобы получить актуальный список навыков из репозитория проекта (приложение также периодически проверяет обновления в фоновом режиме).
...
**Резервное копирование конфигурации** (только для настольного приложения и веб-администраторов)
...
- выбрать другой навык (Простой режим) или модель (Расширенный режим)
- использовать более короткий ввод
- отключить **Перевод в реальном времени (во время ввода)** в разделе [**Настройки** > **Основные настройки**](#general-settings)
- использовать бесплатные модели для простых задач (см. [Модели](#models))
