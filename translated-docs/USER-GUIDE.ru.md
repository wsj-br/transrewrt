---
translation_last_updated: '2026-05-17T23:33:43.438Z'
...
По умолчанию приложение работает в режиме **Простой**: вы выбираете **навык** (например, Бесплатно, Быстро или Технический) и **поставщика** в разделе Настройки, не указывая идентификаторы моделей. Переключитесь в режим **Расширенный** в разделе [**Настройки** > **Основные настройки**](#general-settings), если хотите использовать классический список моделей из раздела [**Настройки** > **Модели**](#models).
...
- [Настройки](#settings)
  ...
  - [Преобразовать (вкладка настроек)](#transform-settings)
  - [Пользователи](#users)
...
- [Распространённые проблемы](#common-issues)
  ...
  - [Сводка на панели управления выглядит пустой](#dashboard-summary-looks-empty)
  - [Стоимость отображается как "недоступно" или кажется неправильной](#cost-shows-not-available-or-seems-wrong)
  - [Общая стоимость не соответствует счёту поставщика](#total-cost-does-not-match-my-provider-bill)
...
- В режиме **Простой** **навык** — это предустановка (Бесплатно, Быстро, Расширенный, Технический, Юридический), которая соответствует модели выбранного вами **поставщика** (OpenRouter, OpenAI, Ollama и другие). Вы выбираете навык на панели инструментов в режимах Перевести, Переписать и Преобразовать.
- В режиме **Расширенный** **модель** — это движок ИИ, который вы выбираете напрямую. Идентификаторы моделей используют **префикс поставщика** (например, `openrouter/…`, `openai/…`, `ollama/…`).
- **Ключ API** (или, для Ollama — **базовый URL**) используется приложением для подключения к поставщику.

Если вы используете **настольное приложение**, добавьте ключи в разделе [**Настройки** > **Конфигурация API**](#api-config) для каждого используемого поставщика. Если вы используете только OpenRouter, см. ниже раздел [Как получить бесплатный ключ API OpenRouter](#how-to-get-an-api-key-desktop-app). Если вы не хотите использовать ключ API, вы можете установить Ollama (с сайта [ollama.com](https://ollama.com)) и использовать локальные модели, например `translategemma:4b`.
...
5. В режиме **Перевести** выберите **навык** (Простой) или **модель** (Расширенный) на панели инструментов.
6. Откройте [**Настройки** > **Языки**](#languages) и выберите **Основные языки**, чтобы наиболее часто используемые языки отображались первыми.
7. Выполните простой перевод, чтобы убедиться, что всё работает, затем попробуйте функции **Переписать** и **Преобразовать**.

Этот порядок важен. Он предотвращает наиболее распространённую проблему при первом использовании: попытку выполнить задачу до того, как приложение установит рабочее соединение с API или выберет навык/модель.
...
- Справа находятся элементы управления **выбором навыка или модели** и **языком интерфейса**.

В режиме **Простой** на панели инструментов отображается **выбор навыка** (Бесплатно, Быстро, Расширенный, Технический, Юридический и аналогичные предустановки). Навыки зависят от **Поставщика**, выбранного в разделе [**Настройки** > **Основные настройки**](#general-settings). Если **Поставщик** — **Ollama**, на панели инструментов отображаются установленные локальные модели вместо навыков.
...
1. Откройте режим **Перевести**.
2. Выберите язык в поле **С**.
3. Выберите язык в поле **На**.
4. Выберите навык (Простой) или модель (Расширенный) на панели инструментов.
...
При использовании функций **Создать запрос**, **Улучшить запрос** или **Перевести запрос** в редакторе запросов режим **Простой** предлагает тот же выбор навыков, что и функции Перевести и Переписать; режим **Расширенный** использует список моделей.
...
В режиме **Простой** выбор модели осуществляется через навыки на панели инструментов и **Поставщик** в разделе Основные настройки; вкладка **Модели** скрыта.

<br/>

> ℹ️ **ПРИМЕЧАНИЕ**<br/>
> В веб-версии каждый пользователь имеет собственную конфигурацию. Такие настройки, как опыт с ИИ, поставщик, выбранные модели или навыки, языки, общие параметры и запросы преобразования, хранятся для каждого пользователя отдельно. Внесённые вами изменения не влияют на других пользователей.
...
- **Простой** (по умолчанию): выберите **Поставщика** (OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras или Ollama). Облачные поставщики используют встроенные предустановки навыков на панели инструментов. **Ollama** отображает модели, установленные на вашем компьютере, вместо навыков.
- **Расширенный**: выбирайте отдельные модели на панели инструментов; управляйте списком в разделе [**Настройки** > **Модели**](#models).

В **веб-приложении** доступные поставщики зависят от API-ключей, заданных в серверной среде. В **настольном приложении** настройте ключи в разделе [**Конфигурация API**](#api-config).
...
- вы выбрали **навык** (Простой) или **модель** (Расширенный) на панели инструментов
- в **Простом** режиме в [**Настройки** > **Основные настройки**](#general-settings) указан **Поставщик** с рабочим ключом (или URL Ollama) и по крайней мере один навык для этого поставщика
- в **Расширенном** режиме по крайней мере одна модель указана в [**Настройки** > **Модели**](#models)
