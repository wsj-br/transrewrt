...
- **용어집** - 언어 쌍별 소스/대상 용어 쌍 저장 및 번역 중 적용하여 선택한 용어 일관성 유지; 설정에서 용어 관리(추가/편집/삭제, CSV/XLSX 가져오기 및 템플릿 내보내기)
- **기록** - 입력/출력 텍스트, 필터링 및 내보내기를 포함한 전체 실행 기록
- **쉬움 & 고급** - 쉬움 모드(기본값): 공급자별 큐레이션된 사전 설정(**무료 (OpenRouter)**, **표준**, **고급**, **기술**; 선택한 공급자에 대한 매핑이 있는 사전 설정만 표시) 모델 ID 선택 없이; 고급 모드: 구성된 공급자의 전체 모델 목록
- **모델 & 비용** - 비용 및 사용량 대시보드(요약, 모델별, 모든 통화) 내보내기 포함; OpenRouter는 실제 지출 표시, 다른 공급자는 추정치 사용
- **UI** - 다국어 인터페이스(30개 이상 언어, RTL 지원), 글꼴 등
- **웹 모드** - 관리자 역할이 있는 다중 사용자 지원
- **데스크톱** - Windows 및 Linux용 Electron 앱
- **자체 호스팅** - amd64 및 arm64(Raspberry Pi 준비 완료)용 Docker 이미지
