---
translation_last_updated: '2026-03-31T22:57:24.416Z'
...
**다른 언어로 읽기:**
[영어 (영국)](../README.md) · [포르투갈어 (브라질)](README.pt-BR.md) · [아랍어](README.ar.md) · [벵골어](README.bn.md) · [카탈루냐어](README.ca.md) · [중국어 (간체)](README.zh-CN.md) · [중국어 (번체)](README.zh-TW.md) · [크로아티아어](README.hr.md) · [체코어](README.cs.md) · [네덜란드어](README.nl.md) · [영어 (미국)](README.en-US.md) · [피리핀어](README.tl.md) · [프랑스어](README.fr.md) · [독일어](README.de.md) · [그리스어](README.el.md) · [힌디어](README.hi.md) · [헝가리어](README.hu.md) · [이탈리아어](README.it.md) · [일본어](README.ja.md) · [자와어](README.jv.md) · [한국어](README.ko.md) · [말레이어](README.ms.md) · [페르시아어](README.fa.md) · [폴란드어](README.pl.md) · [포르투갈어 (포르투갈)](README.pt.md) · [펀자브어](README.pa.md) · [루마니아어](README.ro.md) · [러시아어](README.ru.md) · [슬로바키아어](README.sk.md) · [스페인어](README.es.md) · [스와힐리어](README.sw.md) · [스웨덴어](README.sv.md) · [텔루구어](README.te.md) · [태국어](README.th.md) · [터키어](README.tr.md) · [우크라이나어](README.uk.md) · [베트남어](README.vi.md)
...
> ℹ️ **참고**  
>
> Docker에서는 `OPENROUTER_API_KEY`, `OPENAI_API_KEY`, `CEREBRAS_API_KEY` 등의 환경 변수를 사용하여 LLM 자격 증명을 설정합니다(웹 UI에서는 아님). 데스크톱(Electron)에서는 **설정 → API**에서 키를 구성합니다.
...
이는 amd64에서도 동일하게 적용되며, 다운로드한 파일 이름에 맞게 변경하세요. 자세한 내용은 [설치 → Linux (Electron)](#linux-electron)을 참조하세요.

Debian/Ubuntu에서는 Chromium이 기대하는 추가 **런타임** 라이브러리가 필요할 수 있습니다(일반적으로 완전한 데스크톱 환경에는 이미 설치되어 있음). 데스크톱 알림에는 **`libnotify4`** 를 사용하세요. **`libnotify-dev`** 는 아님(이는 소프트웨어 빌드용이며, 패키징된 AppImage 실행용이 아님):

```bash
sudo apt install libgtk-3-0 libnotify4 libnss3 libxss1 libasound2 libxtst6 xauth
```

최소 또는 사용자 정의 이미지는 여전히 누락된 `.so` 파일로 인해 실패할 수 있습니다. 오류 메시지에 나온 패키지를 설치하세요(일반적인 추가 패키지: `libatk1.0-0`, `libatk-bridge2.0-0`, `libgbm1`, `libdrm2`). 일부 환경에서는 AppImage 실행을 위해 FUSE가 필요할 수 있습니다(예: Ubuntu 22.04+의 `libfuse2`) 또는 `APPIMAGE_EXTRACT_AND_RUN=1 ./Transrewrt-….AppImage`를 사용하세요.

동일한 요약은 [설치 → Linux](#linux-electron)에서 확인할 수 있습니다.
...
>
> - **사용자 계정 컨트롤(UAC)**: "알 수 없는 게시자로부터 제공된 이 앱이 장치를 변경하도록 허용하시겠습니까?" → **예**를 클릭하세요.
> - **Microsoft Defender SmartScreen**: "Windows가 PC를 보호했습니다" → **추가 정보** 클릭 → **그래도 실행** 선택.
>
> 이는 앱이 Microsoft 또는 주요 게시자로부터 서명되지 않았기 때문에 발생합니다. 공식 GitHub 릴리스에서 다운로드한 경우 안전합니다.
> (아래 SHA256 체크섬을 확인하세요).

### Linux (Electron)

- [릴리스](https://github.com/wsj-br/transrewrt/releases)에서 일치하는 `.AppImage` 파일(`x64` 또는 `arm64`)을 다운로드하세요.
- 실행: x86_64/amd64 시스템에서는 `chmod +x Transrewrt-x.y.z-x64.AppImage && ./Transrewrt-x.y.z-x64.AppImage`, ARM64 시스템에서는 `...-arm64.AppImage` 파일명을 사용하세요.
- **Debian/Ubuntu 런타임 라이브러리** (Electron/Chromium; [빠른 시작 → Linux](#quick-start)과 동일): `sudo apt install libgtk-3-0 libnotify4 libnss3 libxss1 libasound2 libxtst6 xauth` — **`libnotify4`** 를 사용하고 `libnotify-dev`는 사용하지 마세요. 최소 설치된 시스템의 경우 터미널에서 보고된 누락된 `.so` 파일을 설치하세요. `libatk1.0-0`, `libatk-bridge2.0-0`, `libgbm1`, `libdrm2` 등의 추가 라이브러리가 종종 필요합니다. AppImage의 경우 `libfuse2` (Ubuntu 22.04+) 또는 `APPIMAGE_EXTRACT_AND_RUN=1 ./….AppImage`가 필요할 수 있습니다.
- **GPU 메시지:** 일부 시스템(특히 ARM)에서 Chromium이 GPU 또는 EGL 초기화 오류를 기록할 수 있습니다. 그러나 앱은 정상적으로 실행될 수 있습니다. 이러한 메시지를 피하려면 하드웨어 가속을 끄고 실행하세요: `TRANSREWRT_DISABLE_GPU=1 ./Transrewrt-x.y.z-x64.AppImage` (또는 사용 중인 `arm64` 파일명).
...
번역, 재작성 또는 변환 작업에는 OpenRouter의 **Body Builder** 모델(`[openrouter/bodybuilder](https://openrouter.ai/openrouter/bodybuilder)`)을 사용하지 마세요. 이 모델은 완성된 텍스트가 아닌 JSON 요청 페이로드를 반환합니다. 사용자 가이드의 [설정 → 모델](USER-GUIDE.ko.md#models)을 참조하세요.
...
| `TZ` | `Europe/London` | 서버 측 시간(로그 등)을 위한 IANA 타임존; UI는 여전히 브라우저 설정을 따릅니다. [Docker → 타임존](#docker-timezone) 참조 |
