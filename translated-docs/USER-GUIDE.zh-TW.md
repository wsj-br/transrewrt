---
translation_last_updated: '2026-05-18T15:42:36.551Z'
...
預設情況下，應用程式以**簡易**模式運行：您可以在設定中選擇一個**技能**（例如免費、快速或技術）和一個**供應商**，而不需要選擇模型 ID。如果您想要從[**設定** > **模型**](#models)中獲得經典模型列表，請切換到[**設定** > **一般設定**](#general-settings)中的**進階**模式。
...
- 在 **簡易** 模式中，**技能** 是預設設定（例如免費、快速、進階、技術性、法律），會根據您選擇的 **供應商**（OpenRouter、OpenAI、Ollama 等）對應到特定模型。您可以在「翻譯」、「重寫」和「轉換」功能的工具列中選擇技能。
- 在 **進階** 模式中，**模型** 是您直接選取的 AI 引擎。模型 ID 會使用 **供應商前綴**（例如 `openrouter/…`、`openai/…`、`ollama/…`）。
- **API 金鑰**（或對 Ollama 而言是 **基礎 URL**）是應用程式連接供應商的方式。

如果您使用的是 **桌面應用程式**，請在 [**設定** > **API 設定**](#api-config) 中為您使用的每個供應商新增金鑰。若僅使用 OpenRouter，請參閱下方的 [如何取得免費的 OpenRouter API 金鑰](#how-to-get-an-api-key-desktop-app)。如果您不想使用 API 金鑰，可以安裝 Ollama（來自 [ollama.com](https://ollama.com)）並改用本機模型，例如 `translategemma:4b`。
...
5. 在 **翻譯** 功能中，於工具列選擇 **技能**（簡易）或 **模型**（進階）。
6. 開啟 [**設定** > **語言**](#languages)，如果您希望常用語言優先顯示，請選擇您的 **頂端語言**。
7. 執行一次簡單的翻譯以確認一切正常，然後嘗試 **重寫** 和 **轉換** 功能。

此順序很重要。它能避免最常見的初次使用問題：在應用程式尚未建立有效的 API 連線或尚未選擇技能/模型之前就嘗試執行任務。
...
- 左側顯示目前頁面的名稱。
- 右側顯示 **技能或模型選擇器** 和 **介面語言** 控制項。

在 **簡易** 模式中，工具列會顯示 **技能選擇器**（免費、快速、進階、技術性、法律等預設設定）。技能取決於您在 [**設定** > **一般設定**](#general-settings) 中選擇的 **供應商**。如果 **供應商** 為 **Ollama**，工具列將列出您已安裝的本機模型，而非技能。
...
當您在提示編輯器中使用 **產生提示**、**改善提示** 或 **翻譯提示** 時，**簡易** 模式會提供與翻譯和重寫相同的技能選擇器；**進階** 模式則使用模型清單。
...
在 **簡易** 模式中，模型選擇是透過工具列中的技能和 **供應商** 在一般設定中完成；**模型** 分頁會被隱藏。

<br/>

> ℹ️ **注意**<br/>
> 在網頁版中，每位使用者都有自己的設定。諸如 AI 體驗、供應商、已選取的模型或技能、語言、一般選項以及轉換提示等設定皆為每位使用者個別儲存。您所做的變更不會影響其他使用者。
...
- **簡易**（預設）：選擇 **供應商**（OpenRouter、OpenAI、Anthropic、Google Gemini、DeepSeek、Groq、Mistral、xAI、Cerebras 或 Ollama）。雲端供應商使用工具列中的內建技能預設。**Ollama** 則列出您機器上安裝的模型，而非技能。
- **進階**：在工具列中選擇個別模型；可在 [**設定** > **模型**](#models) 中管理清單。
...
- 您已在工具列中選取 **技能**（簡易）或 **模型**（進階）
- 在 **簡易** 模式下，[**設定** > **一般設定**](#general-settings) 中已設定 **供應商** 且擁有有效的金鑰（或 Ollama URL），並為該供應商至少設定一項技能
- 在 **進階** 模式下，[**設定** > **模型**](#models) 中至少列出一個模型
- 您的 API 設定運作正常
