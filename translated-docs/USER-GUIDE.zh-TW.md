---
translation_last_updated: '2026-05-24T17:52:59.212Z'
...
預設情況下，應用程式以**簡易**模式運行：您可以在設定中選擇一個**技能**（例如免費（OpenRouter）、Lite或技術）和一個**供應商**，而無需選擇模型 ID。如果您想要從[**設定** > **一般設定**](#general-settings)中切換到**進階**模式，則可以查看經典模型列表，該列表位於[**設定** > **模型**](#models)中。
...
- 在 **簡易** 模式中，**技能** 是預設設定（例如「免費 (OpenRouter)」、「精簡」、「進階」或「技術」），會對應到您所選 **供應商**（OpenRouter、OpenAI、Ollama 等）的特定模型。只有當前供應商支援的技能才會顯示在工具列中。您可在「翻譯」、「重寫」和「轉換」時選擇技能。
- 在 **進階** 模式中，**模型** 是您直接選取的人工智慧引擎。模型 ID 會使用 **供應商前綴**（例如 `openrouter/…`、`openai/…`、`ollama/…`）。
- **API 金鑰**（或對 Ollama 而言為 **基礎 URL**）是應用程式連接該供應商的方式。
...
在 **簡易** 模式中，工具列會顯示 **技能選擇器**，內含內建預設選項：**免費 (OpenRouter)**、**精簡**、**進階** 和 **技術**。顯示哪些技能取決於您在 [**設定** > **一般設定**](#general-settings) 中選擇的 **供應商** — 例如，只有在供應商為 OpenRouter 時，才會列出 **免費 (OpenRouter)**。如果 **供應商** 設為 **Ollama**，工具列將改為列出您安裝的本機模型，而非技能。
...
- **簡易**（預設）：選擇 **供應商**（OpenRouter、OpenAI、Anthropic、Google Gemini、DeepSeek、Groq、Mistral、xAI、Cerebras 或 Ollama）。雲端供應商會使用工具列中的內建技能預設。**Ollama** 則會列出您機器上安裝的模型，而非技能。在簡易模式中，**技能目錄** 會顯示目錄版本與上次更新時間；按一下 **重新整理技能目錄** 可從專案儲存庫取得最新的技能清單（應用程式也會在背景定期自動檢查）。
