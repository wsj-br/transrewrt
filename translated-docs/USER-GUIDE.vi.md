---
translation_last_updated: '2026-03-30T00:46:39.639Z'
...
- **Chuyển đổi** - xử lý văn bản bằng các chỉ dẫn AI tùy chỉnh gọi là lời nhắc.
...
> ℹ️ **LƯU Ý**<br/>
> Transrewrt có sẵn dưới dạng ứng dụng máy tính để bàn cho Windows và Linux, và dưới dạng ứng dụng web tự lưu trữ. Hướng dẫn này tập trung vào việc sử dụng hàng ngày ứng dụng. Những nội dung chỉ áp dụng cho một phiên bản sẽ được đánh dấu rõ ràng.
...
Để sử dụng Transrewrt, bạn cần truy cập vào ít nhất một nhà cung cấp AI. Các nhà cung cấp được hỗ trợ bao gồm: [OpenRouter](https://openrouter.ai) (tích hợp nhiều mô hình), OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras và [Ollama](https://ollama.com) cho các mô hình cục bộ.

Bạn không cần chọn mô hình trả phí để bắt đầu. Ngay khi bạn thêm khóa API OpenRouter, ứng dụng sẽ tự động kích hoạt tùy chọn OpenRouter **miễn phí** tích hợp. Điều này cho phép bạn bắt đầu dịch, viết lại và chuyển đổi văn bản ngay lập tức. Ngoài ra, bạn cũng có thể lấy khóa API miễn phí từ Cerebras, Google, Groq hoặc Mistral AI.
...
Nếu bạn đang sử dụng **ứng dụng máy tính để bàn**, hãy thêm khóa trong [**Cài đặt** > **Cấu hình API**](#api-config) cho từng nhà cung cấp bạn sử dụng. Nếu chỉ sử dụng OpenRouter, hãy xem phần [Cách lấy khóa API](#how-to-get-an-api-key-desktop-app) bên dưới. Nếu bạn không muốn sử dụng khóa API, bạn có thể cài đặt Ollama (từ [ollama.com](https://ollama.com)) và sử dụng các mô hình cục bộ thay thế, ví dụ như `translategemma:4b`.

Nếu bạn đang sử dụng **phiên bản web**, người quản trị máy chủ sẽ cấu hình các nhà cung cấp thông qua các biến môi trường, do đó bạn không thể nhập trực tiếp khóa API vào ứng dụng.
...
9. Nhấp vào **Thử khóa OpenRouter** để đảm bảo khóa hoạt động.
...
1. Mở ứng dụng.
2. Nếu cần, chọn **Ngôn ngữ giao diện** của bạn từ biểu tượng hình quả địa cầu.
3. Nếu bạn đang dùng **ứng dụng máy tính để bàn**, mở [**Cài đặt** > **Cấu hình API**](#api-config), thêm khóa API cho ít nhất một nhà cung cấp (ví dụ: OpenRouter), và nhấp vào **Thử** để xác minh khóa hoạt động.
...
7. Khi đã thành công, hãy thử **Viết lại** và sau đó là **Chuyển đổi**.

Thứ tự này rất quan trọng. Nó giúp tránh vấn đề phổ biến nhất khi sử dụng lần đầu: cố gắng thực hiện tác vụ trước khi ứng dụng có kết nối API hoạt động hoặc chưa chọn mô hình.
...
Sử dụng thanh bên để di chuyển trong ứng dụng. Bạn có thể thu gọn thanh bên để có thêm không gian bằng cách nhấp vào biểu tượng bên cạnh logo ứng dụng.
...
<table>
  <tr>
    ...
    <td valign="top">
      <br/><br/>
      <ul>
        <li><strong>Dịch</strong> mở khu vực làm việc dịch.</li><br/>
        <li><strong>Viết lại</strong> mở khu vực làm việc viết lại.</li><br/>
        <li><strong>Chuyển đổi</strong> mở khu vực làm việc lời nhắc tùy chỉnh.</li><br/>
...
Thanh công cụ thay đổi nhẹ tùy thuộc vào vị trí bạn đang ở trong ứng dụng.
...
**Biểu tượng quả địa cầu + mã ngôn ngữ** thay đổi ngôn ngữ giao diện ứng dụng, ví dụ như menu và nút bấm. Nó **không** thay đổi ngôn ngữ dịch được dùng trong **Dịch**.
...
| **Đầu vào**                                                          | **Đầu ra**                                                                                                                  |
|--------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------|
| - Số ký tự <br/>- Số từ <br/>- Số đoạn văn   <br/> | - Thời gian thực hiện tác vụ<br/>- **TPS** (m?? tr??? mỗi giây)<br/>- Số ký tự, số từ và số đoạn văn<br/>- Mô hình đã dùng |

Nếu bạn thắc mắc về các thuật ngữ kỹ thuật:

- **M?? tr???** nghĩa là một đoạn văn bản nhỏ. Bạn có thể hiểu nó như một phần của từ hoặc một từ ngắn.
- **TPS** nghĩa là số lượng các đoạn văn bản đó mà mô hình xử lý mỗi giây.

<br/>

Bạn cũng có thể theo dõi chi phí cho mỗi thao tác (nếu có sẵn) và tổng chi phí, bằng cách bật tùy chọn `Hiển thị thông tin chi phí trên các hành động` tại [**Cài đặt** > **Cài đặt chung**](#general-settings).
...
- **Tự động dịch khi dán**: thực hiện dịch ngay khi bạn dán văn bản vào.  
- **Tự động sao chép kết quả vào bộ nhớ tạm**: tự động sao chép kết quả sau khi dịch thành công.  
...
- **Enter**: kiểm soát hành động xảy ra khi bạn nhấn phím `Enter`:
...
Sử dụng **Viết lại** khi bạn muốn cải thiện cách diễn đạt mà không làm thay đổi ý nghĩa chính.
...
- làm văn bản trang trọng hơn hoặc thân mật hơn (**Trang trọng** / **Thân mật**)  
...
> 💡 **MẸO**<br/>
> Khi bạn sử dụng chế độ "**Kiểm tra chính tả & ngữ pháp**", một công tắc **Hiển thị thay đổi** sẽ xuất hiện trong bảng kết quả (bên cạnh **Sao chép**).  
> Bật hoặc tắt để hiển thị hoặc ẩn các chỉnh sửa cụ thể được áp dụng cho văn bản của bạn.
...
Đây là khu vực linh hoạt nhất của ứng dụng. Bạn có thể dùng nó cho các tác vụ như:
...
3. Nếu xuất hiện ô **Đích**, hãy chọn ngôn ngữ nếu bạn muốn.  
...
Nếu danh sách lời nhắc của bạn trống, hãy nhấp **Tải các nhắc mẫu** trong không gian làm việc Chuyển đổi. Nút điều khiển này cũng luôn có sẵn trong [**Cài đặt** > **Lời nhắc chuyển đổi**](#transform-prompts) ở hàng xuất/nhập. Cả hai tùy chọn đều thêm các ví dụ tích hợp sẵn để bạn có thể bắt đầu nhanh chóng.

<br/>

> ℹ️ **LƯU Ý**<br/>
> Các lời nhắc mẫu được cung cấp bằng tiếng Anh. Sau khi tải, bạn có thể chỉnh sửa lời nhắc và sử dụng **Dịch nhắc** để dịch sang ngôn ngữ của bạn.
...
Khi bạn tạo hoặc chỉnh sửa một lời nhắc, trình soạn thảo sẽ xuất hiện ở bên trái và khu vực thử nghiệm sẽ xuất hiện ở bên phải.
...
- **Hướng dẫn lời nhắc (tùy chọn)**: gợi ý ngắn được hiển thị cho người dùng khi thực thi lời nhắc.
- **Vai trò mô hình**: vai trò tổng thể được giao cho AI, ví dụ như 'Bạn là một trợ lý hữu ích.'
- **Hướng dẫn mô hình (mỗi dòng một hướng dẫn)**: các quy tắc cụ thể mà bạn muốn AI tuân theo.
- **Mô tả đầu ra**: từ ngắn mô tả kết quả, ví dụ như 'tóm tắt' hoặc 'viết lại'.
- **Nhiệt độ (0.0 → 1.0)**: cách mô hình sẽ hành xử; xem thêm bên dưới.
- **Yêu cầu ngôn ngữ đích**: thêm bộ chọn ngôn ngữ đích khi thực thi lời nhắc.

Nếu thuật ngữ kỹ thuật **Nhiệt độ** là mới đối với bạn, hãy hình dung như sau:

- **Nhiệt độ thấp hơn** sẽ cho kết quả ổn định và dễ dự đoán hơn.
- **Nhiệt độ cao hơn** sẽ tạo ra sự đa dạng và sáng tạo hơn.

Bạn cũng có thể sử dụng:

- **`Tạo lời nhắc`** để tạo bản nháp mới từ mô tả đơn giản
...
Sử dụng các nút bộ lọc ở đầu trang để thay đổi khoảng thời gian.
...
- **Tóm tắt** cung cấp cái nhìn tổng quan về mức sử dụng và chi phí. Bao gồm **Sử dụng theo thời gian** (số **lượt gọi** tích lũy xếp chồng theo ngày cho dịch, viết lại và chuyển đổi) và **Sử dụng theo mô hình** (tổng **số lượt gọi theo mô hình**, bao gồm chuyển đổi).
...
Các bảng trên bảng điều khiển có thể xuất dữ liệu dưới định dạng:
...
Trong **Theo Mô hình** hoặc **Tất cả các cuộc gọi**, bạn có thể xóa bản ghi đã lưu cho một mô hình bằng cách nhấp vào biểu tượng "thùng rác".

> ⚠️ **CẢNH BÁO**<br/>
> Việc xóa bản ghi đã lưu không thể hoàn tác. Chỉ sử dụng tính năng này nếu bạn chắc chắn rằng bạn không cần lịch sử đó nữa.

Để xóa tất cả dữ liệu hoặc xóa bản ghi dựa trên thời gian lưu trữ, hãy truy cập [**Cài đặt** > **Theo dõi chi phí**](#cost-tracking). Tại đó, bạn sẽ tìm thấy các tùy chọn để xóa toàn bộ dữ liệu đã lưu hoặc chỉ dữ liệu cũ hơn một ngày nhất định.
...
Trang lịch sử có thể xuất dữ liệu đã lọc dưới các định dạng:
...
| Tab               | Desktop | Web (admin) | Web (người dùng thường) |
  |-------------------|:-------:|:-----------:|:------------------:|
...
Sử dụng **Cài đặt chung** để điều chỉnh hành vi khi nhập liệu, việc lưu trữ chi tiết thực thi cho **Lịch sử**, và giao diện.

**Hành vi**

- **Hành vi cho ENTER** chọn việc `Enter` thực hiện tác vụ hay chèn dòng mới.
...
ID mô hình bao gồm tiền tố nhà cung cấp (ví dụ: `openrouter/…` so với `openai/…`). Các nhãn như **OpenAI (OpenRouter)** so với **OpenAI (trực tiếp)** cho biết cách lưu lượng truy cập được định tuyến.

> ℹ️ **LƯU Ý**<br/>
> **OpenRouter Body Builder** (`openrouter/bodybuilder`) là một mô hình định tuyến, không phải mô hình trò chuyện thông thường: phản hồi của nó là JSON mô tả các nội dung yêu cầu API OpenRouter (ví dụ: mảng `requests` với `model` và `messages`). Nếu bạn sử dụng nó cho **Dịch**, **Viết lại**, hoặc **Chuyển đổi**, bảng đầu ra sẽ hiển thị JSON đó thay vì văn bản hoàn chỉnh. Hãy chọn một mô hình văn bản thông thường cho các tác vụ này. Xem [trang mô hình Body Builder](https://openrouter.ai/openrouter/bodybuilder) trên OpenRouter.

Hành động:

- Để thêm mô hình, nhấp vào **Thêm** hoặc bất kỳ đâu trong mục nhập.

- Để xóa mô hình, nhấp vào **X** bên cạnh nó trong **Các mô hình đã chọn** hoặc **Đã chọn** trên mục nhập trong Danh sách mô hình khả dụng.

- Để xóa danh sách, nhấp vào **Bỏ chọn tất cả**. Mô hình miễn phí bắt buộc sẽ vẫn được giữ lại trong danh sách.
...
Sử dụng **Cài đặt** > **Ngôn ngữ** để tổ chức danh sách ngôn ngữ được dùng trong ứng dụng.
...
Nếu bạn thêm một ngôn ngữ tùy chỉnh, nó sẽ xuất hiện trong các trình chọn ngôn ngữ cùng với các tùy chọn tích hợp.
...
- **Đồng bộ với việc sử dụng khóa API** đặt tổng số bằng với mức sử dụng được báo cáo bởi tài khoản OpenRouter của bạn (chỉ dành cho OpenRouter).
...
**Theo dõi chi phí:** Khi bạn sử dụng các mô hình OpenRouter, ứng dụng sẽ hiển thị mức sử dụng và chi tiêu thực tế dựa trên thông tin chi phí từ OpenRouter. Đối với tất cả các nhà cung cấp khác, ứng dụng sẽ ước tính chi phí dựa trên giá do OpenRouter công bố; nếu không có giá nào khả dụng, ước tính có thể bằng không.
...
> ⚠️ **CẢNH BÁO**<br/>
> Việc xóa dữ liệu không thể hoàn tác. Trước khi xóa, hãy sao lưu dữ liệu hoặc xuất dữ liệu qua [**Lịch sử**](#history)
...
Các nhà cung cấp được hỗ trợ bao gồm: OpenRouter, OpenAI, Anthropic, Google Gemini, DeepSeek, Groq, Mistral, xAI, Cerebras và **Ollama** (các mô hình cục bộ thông qua URL cơ sở). Bạn chỉ cần cấu hình các nhà cung cấp mà bạn sử dụng.
...
Sử dụng **Cấu hình API** để lưu trữ các khóa API cho từng nhà cung cấp bạn sử dụng. Với Ollama, hãy nhập **URL cơ sở** thay vì khóa API.

<br/>

> 💡 **Mẹo** <br/>
> Nếu bạn không muốn sử dụng khóa API hoặc trả phí sử dụng, bạn có thể [tải xuống Ollama](https://ollama.com) và chạy các mô hình (ví dụ như `translategemma:4b`) cục bộ trên máy của bạn miễn phí. Ngoài ra, bạn có thể tạo một tài khoản OpenRouter miễn phí (không cần thẻ tín dụng) để sử dụng các mô hình miễn phí của họ, hoặc lấy khóa API miễn phí từ Cerebras, Google, Groq hoặc Mistral AI.

<br/>

- Chỉ thêm các nhà cung cấp mà bạn cần. Trong **Cài đặt** > **Mô hình**, mỗi ID mô hình bắt đầu bằng nhà cung cấp (ví dụ: `openrouter/openrouter/free`, `openai/gpt-4o`, `ollama/llama3`).

Để thêm khóa API, nhập giá trị vào ô văn bản và nhấn **`Lưu`**. Để thay thế khóa hiện có, nhấn **`Sửa`**. Để xác minh khóa có hoạt động hay không, nhấn **`Thử`**. Với URL cơ sở Ollama, luôn nhấn **`Thử`** để kiểm tra kết nối.
...
Nhấp vào biểu tượng quả địa cầu trong [thanh công cụ](#toolbar) và chọn **Ngôn ngữ giao diện** bạn muốn.
...
Khi bạn sử dụng mô hình thông qua **OpenRouter**, ứng dụng sẽ hiển thị số tiền chi tiêu thực tế do OpenRouter báo cáo.

Đối với **các nhà cung cấp khác** (OpenAI trực tiếp, Anthropic trực tiếp, v.v.), chi phí được ước tính từ dữ liệu giá do OpenRouter công bố. Nếu không tìm thấy giá phù hợp cho một mô hình, chi phí sẽ hiển thị là **không khả dụng** và sẽ không được cộng vào tổng chi phí của bạn.
...
Tùy chọn **Giữ lịch sử thực thi** có thể đã bị tắt. Hãy mở [**Cài đặt** > **Cài đặt chung**](#general-settings) và bật nó lên. Lưu ý rằng việc bật lại tùy chọn này sẽ không phục hồi dữ liệu lịch sử đã bị xóa trước đó.
...
Phiên làm việc của bạn có thể đã hết hạn. Hãy đăng nhập lại. Nếu hiện tượng này xảy ra thường xuyên, hãy kiểm tra cấu hình máy chủ về cài đặt thời gian tồn tại của phiên.
...
- Nếu một quản trị viên khác vẫn có thể đăng nhập, họ có thể mở [**Cài đặt** > **Người dùng**](#users), chọn tài khoản, và đặt một **mật khẩu mới** tại đó.
- Nếu bạn bị **khóa tài khoản** nhưng vẫn có **quyền truy cập shell** vào máy hoặc container, hãy đặt lại mật khẩu bằng công cụ hỗ trợ đi kèm theo hình ảnh (thay `transrewrt` nếu bạn đổi tên mặc định, và đặt dấu ngoặc kép cho mật khẩu nếu nó chứa khoảng trắng hoặc ký tự đặc biệt):

```bash
docker exec transrewrt reset-web-password '<username>' '<new-password>'
```

Tên người dùng quản trị viên mặc định là `admin` nếu bạn chưa từng tạo tài khoản khác. Khi bạn chỉ truyền một đối số, nó sẽ được coi là mật khẩu mới cho `admin`.

Nếu bạn chạy từ **bản sao mã nguồn** thay vì Docker, hãy sử dụng:

```bash
pnpm run reset-web-password -- <username> <new-password>
```

Lệnh script cập nhật bản ghi người dùng trong cơ sở dữ liệu SQLite (và có thể tạo người dùng `admin` nếu bị thiếu). Sau khi đặt lại, hãy đăng nhập bằng mật khẩu mới.
...
Chỉ **quản trị viên** mới có thể xem dữ liệu từ tất cả người dùng thông qua bộ lọc **Người dùng**. Người dùng thông thường chỉ có thể xem hoạt động của chính họ theo thiết kế.
...
Khi chỉnh sửa lời nhắc, luôn nhấp vào **Lưu** trước khi nhấp vào **Quay lại Chạy**.
...
- Bắt đầu với [**Dịch**](#translate) để đảm bảo thiết lập của bạn hoạt động trước khi chuyển sang [**Viết lại**](#rewrite) hoặc [**Chuyển đổi**](#transform).
...
- Sử dụng [**Lịch sử**](#history) để xem lại các thao tác trước đó cùng toàn bộ văn bản đầu vào/đầu ra của chúng.
- Xuất lời nhắc thường xuyên nếu bạn đang xây dựng thư viện lời nhắc muốn giữ an toàn (xem [Lời nhắc chuyển đổi](#transform-prompts)) hoặc nếu bạn muốn chia sẻ với người khác.
