 ...
 * Uses src/shared/db/appSchema.js for SQL.
 */
...
module.exports = function createCustomPromptsRouter(
  ...
  router.get("/custom-prompts", (req, res) => {
    ...
    try {
      const rows = db.prepare(sql.CUSTOM_PROMPTS_GET_ALL).all();
      res.json(rows);
    } catch (err) {
  ...
  router.delete("/custom-prompts/:id", (req, res) => {
    ...
    try {
      const id = parseInt(req.params.id, 10);
      if (!Number.isFinite(id)) return res.status(400).json({ error: "Invalid id" });
      const result = db.prepare(sql.CUSTOM_PROMPTS_DELETE).run(id);
      if (result.changes === 0) return res.status(404).json({ error: "Prompt not found" });
      res.json({ success: true });
    } catch (err) {
  ...
  router.post("/custom-prompts/import", (req, res) => {
    ...
    try {
      ...
      const now = new Date().toISOString();
      if (mode === "replace") {
        db.prepare(sql.CUSTOM_PROMPTS_DELETE_ALL).run();
      }
      const insert = db.prepare(sql.CUSTOM_PROMPTS_INSERT);
      const update = db.prepare(sql.CUSTOM_PROMPTS_UPDATE_BY_NAME);
      let count = 0;
      for (const p of body) {
