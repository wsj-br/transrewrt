/**
 * Provider model catalog fetch/cache for skills editor and skill-check.
 */
...
const { canonicalForEngine } = require("./skillModelIdUtils.js");
...
function resolveCachePath(opts = {}) {
  return opts.cachePath || defaultCachePath || path.join(process.cwd(), "skills-editor-provider-catalogs.json");
}
...
async function fetchOpenRouterModelsJson(keysMap) {
  ...
  const headers = {
    ...
    "X-Title": "Transrewrt skills-catalog",
    "User-Agent":
      "Mozilla/5.0 (compatible; Transrewrt-skills-catalog/1.0; +https://github.com/wsj-br/transrewrt)",
  };
