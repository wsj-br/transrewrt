...
class ColorPicker(ctk.CTkFrame):
    ...
    def get(self):
        ...
        for n, hex_val in self.COLORS:
            if n == name:
                return hex_val
        # If not found in predefined colors, return the value itself 
        # (in case it's already a hex value)
...
class SettingsDialog(ctk.CTkToplevel):
    ...
    def setup_general_tab(self):
        ...
        # API Key
        ctk.CTkLabel(
            scroll, text="OpenRouter API Key:", font=ctk.CTkFont(weight="bold")
        ...
        # Cost Tracking
        ctk.CTkLabel(
            scroll, text="Cost Tracking:", font=ctk.CTkFont(weight="bold")
        ...
        ctk.CTkLabel(cost_frame, text=f"Total Cost: ${total_cost:.6f}").pack(
            side="left", padx=(0, 20)
        )
        ctk.CTkButton(
            cost_frame, text="Reset Cost", width=100, command=self.reset_cost
        ).pack(side="left")

        # Behavior Settings
        ctk.CTkLabel(scroll, text="Behavior:", font=ctk.CTkFont(weight="bold")).pack(
            anchor="w", pady=(20, 5)
        )
        ...
        ctk.CTkOptionMenu(
            ...
            values=["Translate", "Newline", "Shift-Translate"],
        ).pack(anchor="w", pady=5)

        self.auto_copy_var = ctk.BooleanVar(value=self.config_manager.get("auto_copy"))
        ctk.CTkCheckBox(
            scroll, text="Auto-copy result to clipboard", variable=self.auto_copy_var
        ).pack(anchor="w", pady=5)

        self.realtime_var = ctk.BooleanVar(
            value=self.config_manager.get("real_time_translation")
        )
        ctk.CTkCheckBox(
            scroll,
        ...
        self.font_family_var = ctk.StringVar(
            value=self.config_manager.get("font_family")
        )
        fonts = [
            "Segoe UI",
            "Arial",
        ...
        ctk.CTkOptionMenu(
            font_frame, variable=self.font_family_var, values=fonts, width=150
        ).pack(side="left", padx=(0, 20))

        ctk.CTkLabel(font_frame, text="Size:").pack(side="left", padx=(0, 10))
        self.font_size_var = ctk.StringVar(
            value=str(self.config_manager.get("font_size"))
        )
        ctk.CTkEntry(font_frame, textvariable=self.font_size_var, width=50).pack(
            side="left"
        )

        # Colors
        ctk.CTkLabel(scroll, text="Text Colors:", font=ctk.CTkFont(size=12)).pack(
            anchor="w", pady=(10, 5)
        )
