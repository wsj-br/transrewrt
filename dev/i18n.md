...
| **Custom extract script**   | Scans renderer source for `t("...")` / `t('...')` / `t(`...`)` and `i18n.t(...)` and writes/updates `locales/strings.json`. |
...
No separate CLI (e.g. i18next-scanner) is used; extraction and translation are done by the project’s own scripts.
...
The following sections contain the full source code of the two i18n scripts for reference.
