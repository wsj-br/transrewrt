...
| **i18n:translate** | `pnpm run i18n:translate` | Runs `scripts/generate-translations.js`. Reads `strings.json`, translates missing entries per language via OpenRouter, writes per-locale JSON files. Requires `API_KEY` for new translations.  |
