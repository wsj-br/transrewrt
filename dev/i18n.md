...
- [Annex: Script source code](#annex-script-source-code)
  ...
  - [`scripts/openrouter-script-models.js`](#scriptsopenrouter-script-modelsjs)
...
Keep placeholders unchanged in the source string; the translation script and the translator model are instructed to preserve them (e.g. `{{variable}}`, `{0}`, `%s`). With **key-as-default**, i18next does not substitute `{{...}}` in the returned string the way many tutorials show; the app often uses **`interpolateTemplate(str, vars)`** from [`src/renderer/utils/misc/formatUtils.js`](../src/renderer/utils/misc/formatUtils.js) on the result of `t("...")`:

```js
import { interpolateTemplate } from "../utils/misc/formatUtils";

interpolateTemplate(t("Hello {{name}}"), { name: userName });
```

You can still use i18next’s second-argument interpolation for keys that rely on it; prefer one consistent pattern per string.

Example with i18next vars (where supported):
...
3. **Generate translations**
  Run: `pnpm run i18n:translate`
   The script reads `strings.json`, for each language in `ui-languages.json` (except `en-GB` and `en`) translates missing entries via OpenRouter, updates `strings.json`, and writes flat JSON files under `src/renderer/locales/` (e.g. `de.json`, `pt-BR.json`). If `OPENROUTER_API_KEY` is not set, it only writes locale files from existing translations (no API calls).
4. **One-shot: extract + translate**
  ```bash
   OPENROUTER_API_KEY=<your-openrouter-key> pnpm run i18n:sync
  ```
Optional translate-script options (see [pnpm scripts](#pnpm-scripts)): `--dry-run`, `--show-strings`, `--force`, `--locale`, `--model`, `--max-tokens`.
...
| **i18n:translate** | `pnpm run i18n:translate` | Runs `scripts/generate-translations.js`. Reads `strings.json`, translates missing entries per language via OpenRouter, writes per-locale JSON files. Requires `OPENROUTER_API_KEY` for new translations.  |
...
**Logs:** When the run translates at least one string, one cost/summary line is appended to `dev/translations.log`. Unique source strings that were pending are printed to the **console** (and session transcript) after `done` and **before** the `--- totalizer ---` block (not to `translations.log`). Every run also writes a full mirror of `log` / `warn` / `err` output to `dev/generate-translations-YYYYMMDD-HHMMSS.log` (local timestamp in the filename).
...
### `scripts/openrouter-script-models.js`
