...
| `ARTIFICIAL_INTELLIGENCE_API_KEY` | Optional. Artificial Analysis Data API key used by **AI Suggestion** shortlists (intelligence index, latency/throughput). When unset, shortlists fall back to languagebench + catalog pricing only. |
...
- **AI Suggestion**: Opens a dedicated flow: (1) choose `suggestion_model` / `suggestion_model_fallback` and which presets to include (checkbox list with a master toggle; all selected by default; `free-router` never listed), with provider catalogs prefetched once per session; optional **Live timing** for fast/standard presets; (2) run page with a detailed log (benchmark shortlist from languagebench ChrF + Artificial Analysis + catalog pricing, optional live translate timing of top candidates, then OpenRouter web search via `openrouter:web_search` — see `dev/OpenRouter internet access for prompts.md`); Cancel aborts mid-run; (3) review and **Save to catalog**. Caches: `presets-editor-benchmark-cache.json` (TTL 7d), `presets-editor-timing-cache.json` (TTL 2h for live-timing measurements). Web search may incur extra cost (~$0.02 per request max at default settings, plus tokens); live timing makes real translate API calls only on cache miss. Full selection pipeline: [`../ai-suggestion-model-selection.md`](../ai-suggestion-model-selection.md).
