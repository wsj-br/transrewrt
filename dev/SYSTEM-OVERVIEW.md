...
- [LLM integration and provider support](#llm-integration-and-provider-support)
...
|------|----------------------|-----------|-------------|
| **Electron** | Single local `config.json` (path from main: user data / project / portable); provider secrets stay in main; renderer gets **sanitized** config over IPC | **IPC** `llm:stream` / `llm:abort` / `llm:models` - main process runs the shared LLM layer (Vercel AI SDK) and reaches providers | Separate window or modal |
...
│   ├── server/
│   │   ├── index.js       # Express app; resolves default presets path (repo / Docker)
│   │   ├── logger.js
│   │   ├── db/appDb.js    # Web DB init, migrations, user_preferences helpers
│   │   ├── routes/        # config, auth, apiLlm, presets, glossary, calls, customPrompts, users, …
│   │   └── utils/         # configFile, webConfigKeys, …
│   ├── shared/
│   │   ├── db/appSchema.js
│   │   ├── llm/index.js   # Vercel AI SDK bridge, keys, streaming (main + server)
...
- **Runtime detection**: [configManager](../src/renderer/utils/config/configManager.ts) uses preload IPC in Electron and [webApiClient](../src/renderer/utils/api/webApiClient.ts) in the browser. [apiService](../src/renderer/services/apiService.ts) uses `llm:stream` IPC on desktop and `fetch` + SSE to `/api/llm/stream` on web (not a generic `/api/proxy`).
- **Multi-provider LLM**: Vercel AI SDK (`ai` + `@ai-sdk/openai-compatible`) behind [shared/llm/index.js](../src/shared/llm/index.js); details in [LLM integration and provider support](#llm-integration-and-provider-support). **Security** summary in [Security and encryption](#security-and-encryption).
- **Single bundle**: One Webpack entry ([src/renderer/index.tsx](../src/renderer/index.tsx)), one production bundle for Electron and web.
