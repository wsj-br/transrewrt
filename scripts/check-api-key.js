 ...
 *
 * Usage:
 *   OPENROUTER_API_KEY=sk-or-... node scripts/check-api-key.js
 ...
 *
 * Env:
 *   OPENROUTER_API_KEY - same as server/Docker (if no --key / first arg)
...
Env: OPENROUTER_API_KEY, OPENROUTER_API_URL
`);
}
...
async function main() {
  ...
  if (!key) {
    console.error(
      "Error: API key required. Set OPENROUTER_API_KEY or pass --key <key> or the key as first argument.",
    );
    process.exit(1);
  }
