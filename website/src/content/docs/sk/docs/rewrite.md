---
title: Prepísať text
description: >-
  Zlepšite formuláciu v rovnakom jazyku – jasnosť, tón, dĺžku, gramatiku a
  ďalšie.
translation_last_updated: '2026-07-17T21:14:49.099Z'
source_file_mtime: '2026-07-17T11:53:39.333Z'
source_file_hash: ca70a1d16518bb9193c83911bfb7be66b19076c48b914b92aba4e9a17f67740f
translation_language: sk
source_file_path: src/content/docs/docs/rewrite.md
translation_models:
  - google/gemini-2.5-flash
---



Použite funkciu **Prepísať** na zlepšenie formulácie bez zmeny hlavného významu. Text zostáva v rovnakom jazyku.

![Pracovný priestor prepísania](/images/screenshots/sk/rewrite.png)

Režimy zahŕňajú:

- **Kontrola pravopisu a gramatiky**
- **Zlepšiť jasnosť**
- **Alternatívne verzie** (niekoľko preformulovaní v jednom spustení)
- **Urobiť formálnym** / **Urobiť neformálnym**
- **Skrátiť** / **Rozšíriť**
- **Urobiť technickým**

## Prepísať text

1. Otvorte **Prepísať**.
2. Vyberte **Režim**.
3. Voliteľne nastavte **Z** na jazyk vášho textu (alebo ponechajte **Rozpoznať jazyk**).
4. Zadajte alebo prilepte text do **Vstup**.
5. Kliknite na **Prepísať**.
6. Prečítajte si výsledok v **Výstup**.

:::tip
V časti **Kontrola pravopisu a gramatiky** sa vedľa tlačidla **Kopírovať** zobrazí prepínač **Zobraziť zmeny**. Prepnite ho, ak chcete zobraziť alebo skryť opravy.
:::

:::note
**Alternatívne verzie** vrátia niekoľko preformulovaní v **jednom** spustení, oddelených `----`. To sa líši od **Preformulovať…**, ktoré časom vytvára históriu verzií.
:::

## Upraviť prepis

Po úspešnom spustení sa na výstupnej strane zobrazí **Preformulovať…** a rozbaľovacia ponuka verzií (rovnaká myšlienka ako [Preložiť](/docs/translate/#refine-a-translation), ale text zostáva v rovnakom jazyku a zachováva aktuálny **Režim**):

1. **Preformulovať…** (bez výberu) – ďalšie úplné prepísanie s inou formuláciou. Až päť verzií. Kliknutím na **Zastaviť prepisovanie** zrušíte.
2. **Alternatívy slov** – vyberte text, potom kliknite pravým tlačidlom myši alebo **Preformulovať…**.
3. Každá požiadavka môže zvýšiť náklady na používanie.

## Ďalšie kroky

- [Preložiť text](/docs/translate/)
- [Transformovať pomocou výziev](/docs/transform/)
- [Bežné problémy](/docs/common-issues/)
