...
:::caution
В Docker учетные данные LLM задаются с помощью переменных среды (например, `PROVIDER_API_KEY`). Они **не** вводятся в веб-интерфейсе. На настольном компьютере ключи настраиваются в разделе **Настройки → Конфигурация API**.
:::
