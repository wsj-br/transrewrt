...
:::caution
API 키는 비밀번호처럼 취급하세요. 공유하거나 게시하거나 누구에게도 보내지 마세요. 키가 유출된 경우 제공자 웹사이트에서 삭제하고 새 키를 생성하세요.
:::
...
:::caution
OpenRouter의 **Body Builder** 모델(`openrouter/bodybuilder`)은 피하세요. 완성된 텍스트가 아닌 JSON 요청 페이로드를 반환합니다. [Settings → Models](/docs/settings/#models)를 참조하세요.
:::
...
:::caution
다른 장치나 컨테이너에서 로컬 LLM 서버를 사용하는 경우, 외부 연결을 허용하도록 구성하십시오(localhost 전용 아님).
:::
