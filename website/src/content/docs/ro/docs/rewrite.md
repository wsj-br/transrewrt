---
title: Rescrie textul
description: >-
  Îmbunătățește formularea în aceeași limbă — claritate, ton, lungime, gramatică
  și multe altele.
translation_last_updated: '2026-07-17T14:59:01.880Z'
source_file_mtime: '2026-07-17T11:53:39.333Z'
source_file_hash: ca70a1d16518bb9193c83911bfb7be66b19076c48b914b92aba4e9a17f67740f
translation_language: ro
source_file_path: src/content/docs/docs/rewrite.md
translation_models:
  - google/gemini-2.5-flash
---



Folosește **Rescrie** pentru a îmbunătăți formularea fără a schimba sensul principal. Textul rămâne în aceeași limbă.

![Spațiu de lucru Rescriere](/images/screenshots/ro/rewrite.png)

Modurile includ:

- **Verifică ortografia și gramatica**
- **Îmbunătățește claritatea**
- **Versiuni alternative** (mai multe reformulări într-o singură rulare)
- **Fă formal** / **Fă informal**
- **Scurtează** / **Extinde**
- **Fă tehnic**

## Rescrie textul

1. Deschide **Rescrie**.
2. Alege un **Mod**.
3. Opțional, setează **De la** la limba textului tău (sau lasă **Detectează limba**).
4. Tastează sau lipește textul în **Intrare**.
5. Apasă **Rescrie**.
6. Citește rezultatul în **Ieșire**.

:::tip
În **Verifică ortografia și gramatica**, un comutator **Afișează modificările** apare lângă **Copiază**. Comută-l pentru a afișa sau ascunde corecțiile.
:::

:::note
**Versiunile alternative** returnează mai multe reformulări într-o **singură** rulare, separate prin `----`. Aceasta diferă de **Reformulează…**, care construiește un istoric al versiunilor în timp.
:::

## Rafinează o rescriere

După o rulare reușită, **Reformulează…** și meniul derulant pentru versiuni apar în partea de ieșire (aceeași idee ca [Traducere](/docs/translate/#refine-a-translation), dar textul rămâne în aceeași limbă și păstrează **Modul** curent):

1. **Reformulează…** (fără selecție) — o altă rescriere completă cu o formulare diferită. Până la cinci versiuni. Apasă **Oprește rescrierea** pentru a anula.
2. **Alternative de cuvinte** — selectează textul, apoi fă clic dreapta sau **Reformulează…**.
3. Fiecare solicitare poate adăuga costuri de utilizare.

## Pașii următori

- [Traduceți text](/docs/translate/)
- [Transformați cu prompturi](/docs/transform/)
- [Probleme comune](/docs/common-issues/)
